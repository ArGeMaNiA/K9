#include <QDate>
#include "k9_devir_islemi_batch_class.h"
#include "ui_k9_devir_islemi_batch.h"
#include "adak_sql.h"
...
/**************************************************************************************
                   OPEN_K9_DEVIR_ISLEMI_BATCH
***************************************************************************************/

void OPEN_K9_DEVIR_ISLEMI_BATCH (QWidget * parent)
{
    K9_DEVIR_ISLEMI_BATCH * B = new K9_DEVIR_ISLEMI_BATCH(parent);
    B->EXEC( NOT_FULL_SCREEN );
}

/**************************************************************************************
                   K9_DEVIR_ISLEMI_BATCH::K9_DEVIR_ISLEMI_BATCH
***************************************************************************************/

K9_DEVIR_ISLEMI_BATCH::K9_DEVIR_ISLEMI_BATCH(QWidget *parent) :  BATCH_KERNEL (parent), m_ui(new Ui::K9_DEVIR_ISLEMI_BATCH)
{
    m_ui->setupUi      (this);
...
/**************************************************************************************
                   K9_DEVIR_ISLEMI_BATCH::SETUP_FORM
***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "K9-DEVIR-ISLEMI" );

    REGISTER_BUTTON_WIDGET(m_ui->widget_batch_buttons);

    SET_SETTING_NAME    ( "K9_DEVIR_ISLEMI_BATCH" );
...
/**************************************************************************************
                   K9_DEVIR_ISLEMI_BATCH::CHECK_VAR
***************************************************************************************/

int K9_DEVIR_ISLEMI_BATCH::CHECK_VAR(QObject * object)
{
...
/**************************************************************************************
                   K9_DEVIR_ISLEMI_BATCH::CHECK_RUN
***************************************************************************************/

int K9_DEVIR_ISLEMI_BATCH::CHECK_RUN()
{
    if ( m_ui->searchEdit_arsiv_firma_kodu->GET_TEXT().isEmpty() EQ true ) {
...
/**************************************************************************************
                   K9_DEVIR_ISLEMI_BATCH::RUN_BATCH
***************************************************************************************/
void K9_DEVIR_ISLEMI_BATCH::RUN_BATCH()
{
    ...
    K9_LOG_KAYDI_EKLE (K9_YONETIM , LOG_K9_DEVIR , -1 , log_detaylari);
...
/**************************************************************************************
                  K9_DEVIR_ISLEMI_BATCH::BILGILENDIR
***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::BILGILENDIR (QString mesaj)
{
...
/**************************************************************************************
      K9_DEVIR_ISLEMI_BATCH::ESKI_YILA_AIT_BILGILERI_ARSIV_FIRMAYA_KOPYALA
***************************************************************************************/


void K9_DEVIR_ISLEMI_BATCH::ESKI_YILA_AIT_BILGILERI_ARSIV_FIRMAYA_KOPYALA()
{
    /***********************        ADAK_GRUPLAR  *******************/

    K9_COPY_TABLES ( DB , ARSIV_DB , "adak_gruplar" , "");
    ...
    K9_COPY_TABLES ( DB , ARSIV_DB , "adak_kaydin_gruplari" , "");
    ...
    /***********************        K9_KULLANICI_YETKILERI  *******************/

    K9_COPY_TABLES(DB , ARSIV_DB , "k9_kullanici_yetkileri","");

    BILGILENDIR (QString(" <font color = \"red\">%1</font> - K9 Kullanıcı Yetkileri Tablosu , Arşiv Firmaya Kopyalandı.<br>").arg(QTime::currentTime().toString("hh:mm:ss")));



    /***********************        K9_SABIT_DEGERLER     *******************/

    K9_COPY_TABLES(DB ,ARSIV_DB , "k9_sabit_degerler","");

    BILGILENDIR (QString(" <font color = \"red\">%1</font> - K9 Sabit Değerler Tablosu,Arşiv Firmaya Kopyalandı.<br>").arg(QTime::currentTime().toString("hh:mm:ss")));
    ...
    K9_UPDATE_TANIMLANMAMIS_ENT_HESAPLARI    ( ARSIV_DB );
    ...
    K9_SUBE_ENT_HESAPLARINI_GUNCELLE         ( SUBE_GET_SUBE_ID(),ARSIV_DB );
    ...
    CARI_ENT_FISLERINI_OLUSTUR      ( K9_PROGRAMI , ARSIV_DB );
    ...
    BNK_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    CEK_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    STK_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    FAT_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    PRK_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    PRS_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    DMR_ENT_FISLERINI_OLUSTUR       ( K9_PROGRAMI , ARSIV_DB );
    ...
    SM_MAKBUZ_ENT_FISLERINI_OLUSTUR ( K9_PROGRAMI , ARSIV_DB);
    ...
    CARI_ACILIS_FISI_OLUSTUR                  ( 1 );
    CARI_ACILIS_FISI_OLUSTUR                  ( 0 );

    BNK_ACILIS_FISI_OLUSTUR                   ( 1 );
    BNK_ACILIS_FISI_OLUSTUR                   ( 0 );

    CEK_SENET_ACILIS_BORDROSU_OLUSTUR         ( 1 );
    CEK_SENET_ACILIS_BORDROSU_OLUSTUR         ( 0 );

    STOK_ACILIS_FISI_OLUSTUR                  ( 1 );
    STOK_ACILIS_FISI_OLUSTUR                  ( 0 );

    MUH_ACILIS_FISI_OLUSTUR                   ( 1 );
    MUH_ACILIS_FISI_OLUSTUR                   ( 0 );

    MUH_KAPANIS_FISI_OLUSTUR                  ( 1 );
    MUH_KAPANIS_FISI_OLUSTUR                  ( 0 );
}

/**************************************************************************************
          K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMANIN_MUHASEBE_YILINI_GUNCELLE
***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMANIN_MUHASEBE_YILINI_GUNCELLE()
{
    SQL_QUERY query(DB);

    QDate k9_array_son_tarih = QDate (m_muhasebe_yili + 1 , 01,01).addDays(K9_ARRAY_SIZE-2);

    query.PREPARE_UPDATE("k9_sabit_degerler",
                         "sabit_deger_id",
                         "mali_yil_ilk_tarih    , "
                         "mali_yil_son_tarih    , "
                         "k9_array_son_tarih     ",
    ...
    query.SET_VALUE(":k9_array_son_tarih"       , k9_array_son_tarih.toString("yyyy.MM.dd"));
...
/**************************************************************************************
         K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMADAN_ESKI_YILIN_FISLERINI_SIL
***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMADAN_ESKI_YILIN_FISLERINI_SIL()
{
    SQL_QUERY query        ( DB );
    ...
    double partiye_giren_urun_miktari[K9_ARRAY_SIZE];
    double partiden_cikan_urun_miktari[K9_ARRAY_SIZE];
    ...
    if (query.SELECT() NE 0 ) {
        while (query.NEXT()) {
            ...
            if ( query_select.SELECT() NE 0 ) {
                ...
                UNPACK_DOUBLE_ARRAY(query_select.VALUE(0).toString() , partiye_giren_urun_miktari  , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query_select.VALUE(1).toString() , partiden_cikan_urun_miktari , K9_ARRAY_SIZE);
...
/************************************************************************************************/
/*                  K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMADAN_ESKI_YILA_AIT_PRS_BORDROLARI_SIL();    */
/************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMADAN_ESKI_YILA_AIT_PRS_BORDROLARI_SIL()
{
...
/************************************************************************************************/
/*                  K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMANIN_FIS_NUMARALARINI_GUNCELLE();              */
/************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CALISAN_FIRMANIN_FIS_NUMARALARINI_GUNCELLE()
{
...
/**********************************************************************************************/
/*                 K9_DEVIR_ISLEMI_BATCH::ADRES_MODULUNU_DEVRET();                             */
/**********************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::ADRES_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "adr_adresler" , "");
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "adr_telefonlar" , "");
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "adr_emailler" ,"");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "adr_yetkili_bilgileri" , "");
    ...
    K9_COPY_TABLES(DB, ARSIV_DB , "adr_not_defteri" , "");
...
/**************************************************************************************************/
/*                 K9_DEVIR_ISLEMI_BATCH::BANKA_MODULUNU_DEVRET();                              */
/**************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::BANKA_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_hesabin_kredi_kartlari" , "");
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_kk_arrayler"            , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_hesabin_poslari" , "");
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_pos_arrayler"    , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_hesaplar" , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_makrolar" , "");
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_makro_satirlari" , "");
    ...
    column_names << "ent_borc_array" << "ent_alacak_array" <<"ent_olmayan_borc_array"
            <<"ent_olmayan_alacak_array" << "toplam_borc_array" << "toplam_alacak_array";
    ...
    column_names << "toplam_kk_tahsilat_borc_array" << "toplam_kk_thslat_alacak_array"
            << "ent_kk_tahsilat_borc_array" << "ent_kk_tahsilat_alacak_array" <<
            "ent_olm_kk_tahsilat_borc_array" << "ent_olm_kk_thslat_alacak_array";
    ...
    column_names << "toplam_kk_odeme_borc_array" << "toplam_kk_odeme_alacak_array"
            ...
            << "ent_olm_kk_odeme_borc_array" << "ent_olm_kk_odeme_alacak_array";
...
/************************************************************************************************/
/*             K9_DEVIR_ISLEMI_BATCH::BNK_FISLERINI_KOPYALA();                               */
/************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::BNK_FISLERINI_KOPYALA()
{
    ...
    where_list << QString::number(BANKA_MODULU)<< QString::number(K9_PROGRAMI) << QString::number(K9_PROGRAMI);


    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_hareket_fisler" , where_str , where_list);
    ...
    select_devredilecek_firma.SET_VALUE(":program_id_1" , K9_PROGRAMI);
    select_devredilecek_firma.SET_VALUE(":program_id_2" , K9_PROGRAMI);
    ...
    if ( select_devredilecek_firma.SELECT() EQ 0 ) {
        ...
        while (select_devredilecek_firma.NEXT()) {
            ...
            K9_COPY_TABLES ( DB , ARSIV_DB , "bnk_hareket_fis_satirlari" , QString("fis_id = :arg_%1").arg(0),line_where_list);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_makro_fisler" , where_str , where_list);
    ...
    if ( select_devredilecek_firma.SELECT() NE 0 ) {
        ...
        while ( select_devredilecek_firma.NEXT() ) {
            ...
            K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_makro_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , line_where_list);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_virman_fisler" , where_str , where_list);
    ...
    while ( select_devredilecek_firma.NEXT() ) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "bnk_virman_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , line_where_list);
...
/***************************************************************************************/
/*                 K9_DEVIR_ISLEMI_BATCH::BNK_ACILIS_FISI_OLUSTUR();                  */
/***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::BNK_ACILIS_FISI_OLUSTUR(int entegre_fis_mi)
{
    ...
    query_arsiv_db.PREPARE_SELECT("bnk_hesaplar" ,
                                   "hesap_no_id,doviz_id,ent_borc_array,"
                                   "ent_alacak_array , ent_olmayan_borc_array,"
                                   "ent_olmayan_alacak_array","");
    ...
    double borc_array   [K9_ARRAY_SIZE];
    double alacak_array [K9_ARRAY_SIZE];

    double pos_borc_array[K9_ARRAY_SIZE];
    double pos_alacak_array[K9_ARRAY_SIZE];

    double kk_borc_array[K9_ARRAY_SIZE];
    double kk_alacak_array[K9_ARRAY_SIZE];
    ...
    while ( query_arsiv_db.NEXT() ) {
        ...
        if ( entegre_fis_mi EQ 1 ) {

            UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_borc_array").toString(),borc_array , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_alacak_array").toString() , alacak_array , K9_ARRAY_SIZE);

            s_query_arsiv_db.PREPARE_SELECT("bnk_pos_arrayler" , "ent_kk_tahsilat_borc_array,"
                                            "ent_kk_tahsilat_alacak_array" , "hesap_no_id = :hesap_no_id");
            s_query_arsiv_db.SET_VALUE(":hesap_no_id" , hesap_no_id);

            if ( s_query_arsiv_db.SELECT() > 0 ) {
                s_query_arsiv_db.NEXT();

                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_kk_tahsilat_borc_array").toString(),pos_borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_kk_tahsilat_alacak_array").toString() , pos_alacak_array , K9_ARRAY_SIZE);

            }
            s_query_arsiv_db.PREPARE_SELECT("bnk_kk_arrayler" , "ent_kk_odeme_borc_array,"
                                            "ent_kk_odeme_alacak_array" , "hesap_no_id = :hesap_no_id");
            s_query_arsiv_db.SET_VALUE(":hesap_no_id" , hesap_no_id);
            ...
            if ( s_query_arsiv_db.SELECT() > 0 ) {
                ...
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_kk_odeme_borc_array").toString() , kk_borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_kk_odeme_alacak_array").toString() , kk_alacak_array , K9_ARRAY_SIZE);

            }
        }
        else {

            UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_olmayan_borc_array").toString(),borc_array , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_olmayan_alacak_array").toString() , alacak_array , K9_ARRAY_SIZE);

            s_query_arsiv_db.PREPARE_SELECT("bnk_pos_arrayler" , "ent_olm_kk_tahsilat_borc_array,"
                                            "ent_olm_kk_thslat_alacak_array" , "hesap_no_id = :hesap_no_id");
            s_query_arsiv_db.SET_VALUE(":hesap_no_id" , hesap_no_id);

            if ( s_query_arsiv_db.SELECT() NE 0 ) {

                s_query_arsiv_db.NEXT();

                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_olm_kk_tahsilat_borc_array").toString(),pos_borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_olm_kk_thslat_alacak_array").toString() , pos_alacak_array , K9_ARRAY_SIZE);
            }

            s_query_arsiv_db.PREPARE_SELECT("bnk_kk_arrayler" , "ent_olm_kk_odeme_borc_array,"
                                            "ent_olm_kk_odeme_alacak_array" , "hesap_no_id = :hesap_no_id");
            s_query_arsiv_db.SET_VALUE(":hesap_no_id" , hesap_no_id);
            ...
            if ( s_query_arsiv_db.SELECT() NE 0 ) {
                ...
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_olm_kk_odeme_borc_array").toString() , kk_borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(s_query_arsiv_db.VALUE("ent_olm_kk_odeme_alacak_array").toString() , kk_alacak_array , K9_ARRAY_SIZE);
            }
        }
        ...
        BNK_HRK_FIS->program_id                       = K9_PROGRAMI;
...
/*************************************************************************************************/
/*                   K9_DEVIR_ISLEMI_BATCH::CARI_MODULUNU_DEVRET();                             */
/*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CARI_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "car_hesaplar" , "");
    ...
    column_names << "ent_borc_array" << "ent_alacak_array" << "toplam_borc_array"
            << "toplam_alacak_array" << "ent_olmayan_borc_array" << "ent_olmayan_alacak_array";
...
/*************************************************************************************************/
/*                 K9_DEVIR_ISLEMI_BATCH::CARI_FISLERI_KOPYALA();                               */
/*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CARI_FISLERI_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);

    //K9 programina ait olan cari modulu entegrasyon fisleri devredilmeyecek
    ...
    where_list << m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd");
    where_list << QString::number(CARI_MODULU)<< QString::number(K9_PROGRAMI) << QString::number(K9_PROGRAMI);


    K9_COPY_TABLES(DB ,ARSIV_DB , "car_fisler" , where_str , where_list);
    ...
    while ( select_devredilecek_firma.NEXT()) {
        ...
        K9_COPY_TABLES(DB , ARSIV_DB , "car_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , where_line_list);
...
/********************************************************************/
/*       K9_DEVIR_ISLEMI_BATCH::CARI_ACILIS_FISI_OLUSTUR();         */
/********************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CARI_ACILIS_FISI_OLUSTUR(int entegre_fis_mi)
{

    SQL_QUERY select_arsiv_firma(ARSIV_DB);

    //Entegre olan ve entegre olmayan bakiyeler icin olmak uzere iki tane acilis fisi olusturulur.

    select_arsiv_firma.PREPARE_SELECT("car_hesaplar " , "cari_hesap_id,ent_borc_array , ent_alacak_array, "
                                      "ent_olmayan_borc_array , ent_olmayan_alacak_array ");
    ...
    if ( entegre_fis_mi EQ 1 ) {
        CARI_FIS->kaydedilecek_sirket = 1;
    }
    else {
        CARI_FIS->kaydedilecek_sirket = 0;
    }
    ...
    double      toplam_borc[K9_ARRAY_SIZE];
    double      toplam_alacak[K9_ARRAY_SIZE];
    ...
    CARI_FIS_SATIRI->kaydedilecek_sirket = CARI_FIS->kaydedilecek_sirket;
    ...
    while (select_arsiv_firma.NEXT() EQ true) {
        ...
        if ( entegre_fis_mi EQ 1 ) {
            ...
            UNPACK_DOUBLE_ARRAY(select_arsiv_firma.VALUE("ent_borc_array").toString()   , toplam_borc   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(select_arsiv_firma.VALUE("ent_alacak_array").toString() , toplam_alacak , K9_ARRAY_SIZE );
        }
        else {
            UNPACK_DOUBLE_ARRAY(select_arsiv_firma.VALUE("ent_olmayan_borc_array").toString()   , toplam_borc   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(select_arsiv_firma.VALUE("ent_olmayan_alacak_array").toString() , toplam_alacak , K9_ARRAY_SIZE );
        }
...
/*************************************************************************************************/
/*              K9_DEVIR_ISLEMI_BATCH::CEK_SENET_MODULUNU_DEVRET();                             */
/*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CEK_SENET_MODULUNU_DEVRET()
{
...
/*************************************************************************************************/
/*               K9_DEVIR_ISLEMI_BATCH::CEKLER_TABLOSUNU_KOPYALA();                             */
/*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CEKLER_TABLOSUNU_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        K9_COPY_TABLES(DB , ARSIV_DB ,"cek_cekler_senetler" ,QString("cek_senet_id = :arg_%1").arg(0) , where_line_list);

        where_line_list << m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd");

        K9_COPY_TABLES(DB , ARSIV_DB ,"cek_hareketleri" , QString("cek_senet_id = :arg_%1 AND "
...
/************************************************************************************************/
/*              K9_DEVIR_ISLEMI_BATCH::CEK_BORDROLARINI_KOPYALA();                             */
/************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CEK_BORDROLARINI_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    where_list << m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd");
    where_list << QString::number(CEKSENET_MODULU)<< QString::number(K9_PROGRAMI) << QString::number(K9_PROGRAMI);

    K9_COPY_TABLES(DB ,ARSIV_DB , "cek_bordrolar" , where_str , where_list);
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "cek_bordro_satirlari" , QString("bordro_id = :arg_%1").arg(0) , line_where_list );
    }
...
/*********************************************************************/
/*    K9_DEVIR_ISLEMI_BATCH::CEK_SENET_ACILIS_BORDROSU_OLUSTUR();   */
/*********************************************************************/

void K9_DEVIR_ISLEMI_BATCH::CEK_SENET_ACILIS_BORDROSU_OLUSTUR(int entegre_fis_mi)
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY select_query(DB);
    ...
    select_devredilecek_firma.SET_VALUE(":kaydedilecek_sirket" , entegre_fis_mi);
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        int kaydedilecek_sirket      = select_devredilecek_firma.VALUE(4).toInt();
        double cek_senet_tutari   = select_devredilecek_firma.VALUE(5).toDouble();
        ...
        if ( musteri_cek_senedi_mi EQ true) {
        ...
        int uygun_bordro_id = FIND_UYGUN_ACILIS_BORDROSU(bordro_islem_turu,cek_senet_turu,hesap_no_id,kaydedilecek_sirket);
        ...
        CEK_BORDRO_SATIRI->program_id            = K9_PROGRAMI;
        CEK_BORDRO_SATIRI->kaydedilecek_sirket      = kaydedilecek_sirket;
...
/*************************************************************************************************/
/*                   K9_DEVIR_ISLEMI_BATCH::MUHASEBE_MODULUNU_DEVRET();                         */
/*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::MUHASEBE_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "muh_hesaplar" , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "muh_yansitma_hesaplari" , "");
...
/*******************************************************************************/
/*                  K9_DEVIR_ISLEMI_BATCH::MUHASEBE_FISLERINI_KOPYALA         */
/*******************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::MUHASEBE_FISLERINI_KOPYALA (  )
{
    SQL_QUERY select_db_query         ( DB );

    //K9_PROGRAMINA ait olan entegrasyon fisleri aktarilmayacak.K9 ONAR fonksiyonu kullanilarak ent fisleri tekrardan olusturulacak
    ...
    where_list << m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd");
    where_list << QString::number(MUHASEBE_MODULU)<< QString::number(K9_PROGRAMI) << QString::number(K9_PROGRAMI);

    K9_COPY_TABLES(DB ,ARSIV_DB , "muh_fisler" , where_str , where_list);
    ...
    select_db_query.SET_VALUE(":program_id_1" , K9_PROGRAMI);
    select_db_query.SET_VALUE(":program_id_2" , K9_PROGRAMI);
    ...
    while ( select_db_query.NEXT() EQ true ) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "muh_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , line_where_list);
    }
...
/****************************************************************/
/*           K9_DEVIR_ISLEMI_BATCH::MUH_ACILIS_FISI_OLUSTUR        */
/****************************************************************/

void K9_DEVIR_ISLEMI_BATCH::MUH_ACILIS_FISI_OLUSTUR(int entegre_fis_mi)
{
    SQL_QUERY arsiv_query(ARSIV_DB);

    arsiv_query.PREPARE_SELECT("muh_hesaplar" , "hesap_id,ent_borc_array,ent_alacak_array,"
                               "ent_olmayan_borc_array,ent_olmayan_alacak_array" ,
    ...
    MUHASEBE_FISI->program_id       = K9_PROGRAMI;
    MUHASEBE_FISI->kaydedilecek_sirket = entegre_fis_mi;
    ...
    double borc_bakiyesi  [K9_ARRAY_SIZE];
    double alacak_bakiyesi [K9_ARRAY_SIZE];
    ...
    while ( arsiv_query.NEXT() EQ true ) {
        ...
        if ( entegre_fis_mi  EQ 1 ) {
            UNPACK_DOUBLE_ARRAY ( arsiv_query.VALUE("ent_borc_array").toString() , borc_bakiyesi   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY ( arsiv_query.VALUE("ent_alacak_array").toString() , alacak_bakiyesi , K9_ARRAY_SIZE);
        }
        else {
            UNPACK_DOUBLE_ARRAY ( arsiv_query.VALUE("ent_olmayan_borc_array").toString() , borc_bakiyesi   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY ( arsiv_query.VALUE("ent_olmayan_alacak_array").toString() , alacak_bakiyesi , K9_ARRAY_SIZE);
        }
        ...
        MUH_FIS_SATIRI->program_id       = K9_PROGRAMI;
        MUH_FIS_SATIRI->modul_id         = MUHASEBE_MODULU;
        MUH_FIS_SATIRI->kaydedilecek_sirket       = entegre_fis_mi;
...
/********************************************************************/
/*            K9_DEVIR_ISLEMI_BATCH::KAPANIS_FISI_OLUSTUR          */
/********************************************************************/

void K9_DEVIR_ISLEMI_BATCH::MUH_KAPANIS_FISI_OLUSTUR( int entegre_fis_mi)
{
...
/********************************************************************************************/
/*           K9_DEVIR_ISLEMI_BATCH::ISLETME_DEFTERI_MODULUNU_DEVRET();                     */
/*********************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::ISLETME_MODULUNU_DEVRET()
{
...
/***********************************************************************************/
/*              K9_DEVIR_ISLEMI_BATCH::ISLETME_DEFTERI_FISLERINI_KOPYALA          */
/***********************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::ISLETME_DEFTERI_FISLERINI_KOPYALA ( )
{
    SQL_QUERY select_db_query(DB);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "isl_fisi" , QString("fis_tarihi < :arg_%1").arg(0) , where_list);
    ...
    while ( select_db_query.NEXT() EQ true ) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "isl_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , where_list);
...
/***********************************************************************************/
/*                 K9_DEVIR_ISLEMI_BATCH::DOVIZ_MODULUNU_DEVRET();                 */
/***********************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::DOVIZ_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "dvz_degiskenler" , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "dvz_dovizler" , "");
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "dvz_kurlar" , "");
...
/******************************************************************************/
/*               K9_DEVIR_ISLEMI_BATCH::SUBE_DEPO_UNITE_DEVRET();             */
/******************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::SUBE_DEPO_UNITE_DEVRET()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "sub_subeler" , "");
...
/************************************************************************************/
/*            K9_DEVIR_ISLEMI_BATCH::STOK_MODULUNU_DEVRET();                        */
/************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STOK_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES ( DB ,ARSIV_DB , "stk_urunler" , "");
...
/***********************************************************************************/
/*                  K9_DEVIR_ISLEMI_BATCH::STOK_FISLERINI_KOPYALA();               */
/***********************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STOK_FISLERINI_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "stk_fisler" , QString("fis_tarihi < :arg_%1").arg(0) ,where_list );
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "stk_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , where_list);
...
/****************************************************************************************************/
/*            K9_DEVIR_ISLEMI_BATCH::STK_DEPODAKI_URUNLER_TABLOSUNU_KOPYALA();                      */
/****************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STK_DEPODAKI_URUNLER_TABLOSUNU_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY insert_arsiv_firma(ARSIV_DB);
    SQL_QUERY sql_query(DB);

    double ent_depoya_giren[K9_ARRAY_SIZE];
    double ent_depodan_cikan[K9_ARRAY_SIZE];

    double ent_olmayan_depoya_giren[K9_ARRAY_SIZE];
    double ent_olmayan_depo_cikan[K9_ARRAY_SIZE];

    double toplam_depoya_giren[K9_ARRAY_SIZE];
    double toplam_depodan_cikan[K9_ARRAY_SIZE];


    double calisan_firma_ent_depoya_giren[K9_ARRAY_SIZE];
    double calisan_firma_ent_depodan_cikan[K9_ARRAY_SIZE];

    double calisan_firma_ent_olmayan_depoya_giren[K9_ARRAY_SIZE];
    double calisan_firma_ent_olmayan_depo_cikan[K9_ARRAY_SIZE];

    double calisan_firma_toplam_depoya_giren[K9_ARRAY_SIZE];
    double calisan_firma_toplam_depodan_cikan[K9_ARRAY_SIZE];

    select_devredilecek_firma.PREPARE_SELECT("stk_depodaki_urunler" , "depo_id,urun_id,kayit_id,ent_depoya_giren_array ,"
                                             "ent_depodan_cikan_array,ent_olmayan_depoya_giren_array,"
                                             "ent_olmayan_depo_cikan_array, toplam_depoya_giren_array,"
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_depoya_giren_array").toString(),ent_depoya_giren,K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_depodan_cikan_array").toString(),ent_depodan_cikan,K9_ARRAY_SIZE);

        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_olmayan_depoya_giren_array").toString(),ent_olmayan_depoya_giren,K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_olmayan_depo_cikan_array").toString(),ent_olmayan_depo_cikan,K9_ARRAY_SIZE);

        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("toplam_depoya_giren_array").toString() , toplam_depoya_giren  , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("toplam_depodan_cikan_array").toString(), toplam_depodan_cikan , K9_ARRAY_SIZE);


        double yil_sonu_ent_depoya_giren  = ent_depoya_giren[gun_no];
        double yil_sonu_ent_depodan_cikan = ent_depodan_cikan[gun_no];

        double yil_sonu_ent_olmayan_depoya_giren = ent_olmayan_depoya_giren[gun_no];
        double yil_sonu_ent_olmayan_depo_cikan = ent_olmayan_depo_cikan[gun_no];
        ...
        for ( int i = 0 ; i < K9_ARRAY_SIZE ; i++) {
            calisan_firma_ent_depoya_giren[i]   = 0;
            calisan_firma_ent_depodan_cikan[i]  = 0;

            calisan_firma_ent_olmayan_depoya_giren[i]  = 0;
            calisan_firma_ent_olmayan_depo_cikan[i] = 0;
        ...
        if ( yil_sonu_ent_depoya_giren > yil_sonu_ent_depodan_cikan) {
            calisan_firma_ent_depoya_giren[0] = yil_sonu_ent_depoya_giren - yil_sonu_ent_depodan_cikan;
        }
        else {
            calisan_firma_ent_depodan_cikan[0] = yil_sonu_ent_depodan_cikan - yil_sonu_ent_depoya_giren;
        }

        if ( yil_sonu_ent_olmayan_depoya_giren > yil_sonu_ent_olmayan_depo_cikan ) {
            calisan_firma_ent_olmayan_depoya_giren[0] = yil_sonu_ent_olmayan_depoya_giren - yil_sonu_ent_olmayan_depo_cikan;
        }
        else {
            calisan_firma_ent_olmayan_depo_cikan[0] = yil_sonu_ent_olmayan_depo_cikan - yil_sonu_ent_olmayan_depoya_giren;
        }
        ...
        for ( int i = gun_no + 1 ; i < K9_ARRAY_SIZE ; i++) {

            gun_indis_no++;

            calisan_firma_ent_depoya_giren[gun_indis_no]           = (ent_depoya_giren[i]          - ent_depoya_giren[i-1]           ) + calisan_firma_ent_depoya_giren           [gun_indis_no-1];
            calisan_firma_ent_depodan_cikan[gun_indis_no]          = (ent_depodan_cikan[i]         - ent_depodan_cikan[i-1]          ) + calisan_firma_ent_depodan_cikan          [gun_indis_no-1];

            calisan_firma_ent_olmayan_depoya_giren[gun_indis_no]   = (ent_olmayan_depoya_giren[i]  - ent_olmayan_depoya_giren[i-1]   ) + calisan_firma_ent_olmayan_depoya_giren   [gun_indis_no-1];
            calisan_firma_ent_olmayan_depo_cikan[gun_indis_no]     = (ent_olmayan_depo_cikan[i]    - ent_olmayan_depo_cikan[i-1]  )    + calisan_firma_ent_olmayan_depo_cikan     [gun_indis_no-1];
        ...
        for (int i = gun_indis_no+1 ; i < K9_ARRAY_SIZE ; i++) {
            calisan_firma_ent_depoya_giren[i]          = calisan_firma_ent_depoya_giren[gun_indis_no];
            calisan_firma_ent_depodan_cikan[i]         = calisan_firma_ent_depodan_cikan[gun_indis_no];

            calisan_firma_ent_olmayan_depoya_giren[i]  = calisan_firma_ent_olmayan_depoya_giren[gun_indis_no];
            calisan_firma_ent_olmayan_depo_cikan[i] = calisan_firma_ent_olmayan_depo_cikan[gun_indis_no];
        ...
        sql_query.PREPARE_UPDATE("stk_depodaki_urunler","kayit_id",
                                 "ent_depoya_giren_array ,"
                                 "ent_depodan_cikan_array , "
                                 "ent_olmayan_depoya_giren_array ,"
                                 "ent_olmayan_depo_cikan_array,"
                                 "toplam_depoya_giren_array ,"
                                 "toplam_depodan_cikan_array ",
                                 "kayit_id = :kayit_id");
        sql_query.SET_VALUE(":ent_depoya_giren_array"          , PACK_DOUBLE_ARRAY(ent_depoya_giren          , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_depodan_cikan_array"         , PACK_DOUBLE_ARRAY(ent_depodan_cikan         , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_olmayan_depoya_giren_array"  , PACK_DOUBLE_ARRAY(ent_olmayan_depoya_giren  , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_olmayan_depo_cikan_array" , PACK_DOUBLE_ARRAY(ent_olmayan_depo_cikan , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":toplam_depoya_giren_array"       , PACK_DOUBLE_ARRAY(toplam_depoya_giren       , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":toplam_depodan_cikan_array"      , PACK_DOUBLE_ARRAY(toplam_depodan_cikan      , K9_ARRAY_SIZE));
        ...
        for (int i = gun_no + 1 ; i < K9_ARRAY_SIZE ; i++) {
            ent_depoya_giren[i]          = yil_sonu_ent_depoya_giren;
            ent_depodan_cikan[i]         = yil_sonu_ent_depodan_cikan;

            ent_olmayan_depoya_giren[i]  = yil_sonu_ent_olmayan_depoya_giren;
            ent_olmayan_depo_cikan[i] = yil_sonu_ent_olmayan_depo_cikan;
        ...
        insert_arsiv_firma.PREPARE_INSERT("stk_depodaki_urunler" , "kayit_id" , "depo_id,urun_id,"
                                          ...
                                          "ent_olmayan_depoya_giren_array , ent_olmayan_depo_cikan_array,"
        ...
        insert_arsiv_firma.SET_VALUE(":ent_depoya_giren_array"          , PACK_DOUBLE_ARRAY(ent_depoya_giren,K9_ARRAY_SIZE));
        insert_arsiv_firma.SET_VALUE(":ent_depodan_cikan_array"         , PACK_DOUBLE_ARRAY(ent_depodan_cikan,K9_ARRAY_SIZE));
        insert_arsiv_firma.SET_VALUE(":ent_olmayan_depoya_giren_array"  , PACK_DOUBLE_ARRAY(ent_olmayan_depoya_giren,K9_ARRAY_SIZE));
        insert_arsiv_firma.SET_VALUE(":ent_olmayan_depo_cikan_array" , PACK_DOUBLE_ARRAY(ent_olmayan_depo_cikan,K9_ARRAY_SIZE));
        insert_arsiv_firma.SET_VALUE(":toplam_depoya_giren_array"       , PACK_DOUBLE_ARRAY(toplam_depoya_giren,K9_ARRAY_SIZE));
        insert_arsiv_firma.SET_VALUE(":toplam_depodan_cikan_array"      , PACK_DOUBLE_ARRAY(toplam_depodan_cikan,K9_ARRAY_SIZE));
...
/************************************************************************************/
/*     K9_DEVIR_ISLEMI_BATCH::STK_SATIRDAKI_SERI_NUMARALARI_TABLOSUNU_KOPYALA();    */
/************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STK_SATIRDAKI_SERI_NUMARALARI_TABLOSUNU_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
...
/**************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::STK_URUNUN_PARTILERI_TABLOSUNU_KOPYALA();          */
/**************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STK_URUNUN_PARTILERI_TABLOSUNU_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY insert_arsiv_firma(ARSIV_DB);
    SQL_QUERY sql_query(DB);

    double  toplam_partiye_giren[K9_ARRAY_SIZE];
    double  toplam_partiden_cikan[K9_ARRAY_SIZE];

    double ent_partiye_giren[K9_ARRAY_SIZE];
    double ent_partiden_cikan[K9_ARRAY_SIZE];

    double ent_olm_partiye_giren[K9_ARRAY_SIZE];
    double ent_olm_partiden_cikan[K9_ARRAY_SIZE];

    double  calisan_firma_toplam_partiye_giren[K9_ARRAY_SIZE];
    double  calisan_firma_toplam_partiden_cikan[K9_ARRAY_SIZE];

    double calisan_firma_ent_partiye_giren[K9_ARRAY_SIZE];
    double calisan_firma_ent_partiden_cikan[K9_ARRAY_SIZE];

    double calisan_firma_ent_olm_partiye_giren[K9_ARRAY_SIZE];
    double calisan_firma_ent_olm_partiden_cikan[K9_ARRAY_SIZE];
    ...
    select_devredilecek_firma.PREPARE_SELECT("stk_urunun_partileri ","parti_id,depo_id,urun_id,parti_kodu,urun_uretim_tarihi,"
                                             ...
                                             "toplam_partiden_cikan_array,ent_partiye_giren_array,"
                                             "ent_partiden_cikan_array , ent_olm_partiye_giren_array,"
                                             "ent_olm_partiden_cikan_array");
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("toplam_partiye_giren_array").toString()  , toplam_partiye_giren    , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("toplam_partiden_cikan_array").toString() , toplam_partiden_cikan   , K9_ARRAY_SIZE);

        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_partiye_giren_array").toString()     , ent_partiye_giren       , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_partiden_cikan_array").toString()    , ent_partiden_cikan      , K9_ARRAY_SIZE);

        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_olm_partiye_giren_array").toString() , ent_olm_partiye_giren   , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_devredilecek_firma.VALUE("ent_olm_partiden_cikan_array").toString(), ent_olm_partiden_cikan  , K9_ARRAY_SIZE);
        ...
        double yil_sonu_ent_partiye_giren     = ent_partiye_giren[gun_no];
        double yil_sonu_ent_partiden_cikan    = ent_partiden_cikan[gun_no];

        double yil_sonu_ent_olm_partiye_giren = ent_olm_partiye_giren[gun_no];
        double yil_sonu_ent_olm_partiden_cikan= ent_olm_partiden_cikan[gun_no];
        ...
        for (int i = 0 ; i < K9_ARRAY_SIZE ; i++) {
            ...
            calisan_firma_ent_partiye_giren[i]      = 0;
            calisan_firma_ent_partiden_cikan[i]     = 0;

            calisan_firma_ent_olm_partiye_giren[i]  = 0;
            calisan_firma_ent_olm_partiden_cikan[i] = 0;
        ...
        if ( yil_sonu_ent_partiye_giren > yil_sonu_ent_partiden_cikan ) {
            calisan_firma_ent_partiye_giren[0] = yil_sonu_ent_partiye_giren - yil_sonu_ent_partiden_cikan;
        }
        else {
            calisan_firma_ent_partiden_cikan[0] = yil_sonu_ent_partiden_cikan - yil_sonu_ent_partiye_giren;
        }

        if ( yil_sonu_ent_olm_partiye_giren > yil_sonu_ent_olm_partiden_cikan ) {
            calisan_firma_ent_olm_partiye_giren[0] = yil_sonu_ent_olm_partiye_giren - yil_sonu_ent_olm_partiden_cikan;
        }
        else {
            calisan_firma_ent_olm_partiden_cikan[0] = yil_sonu_ent_olm_partiden_cikan - yil_sonu_ent_olm_partiye_giren;
        }
        ...
        for (int i = gun_no + 1 ; i < K9_ARRAY_SIZE ; i++) {
            ...
            calisan_firma_ent_partiye_giren[gun_indis_no]   = (ent_partiye_giren[i] -
                ent_partiye_giren[i-1]) + calisan_firma_ent_partiye_giren[gun_indis_no-1];

            calisan_firma_ent_partiden_cikan[gun_indis_no]  = (ent_partiden_cikan[i] -
                ent_partiden_cikan[i-1]) + calisan_firma_ent_partiden_cikan[gun_indis_no-1];


            calisan_firma_ent_olm_partiye_giren[gun_indis_no]   = (ent_olm_partiye_giren[i] -
                ent_olm_partiye_giren[i-1]) + calisan_firma_ent_olm_partiye_giren[gun_indis_no-1];

            calisan_firma_ent_olm_partiden_cikan[gun_indis_no]  = (ent_olm_partiden_cikan[i] -
                ent_olm_partiden_cikan[i-1]) + calisan_firma_ent_olm_partiden_cikan[gun_indis_no-1];
        ...
        for (int i = gun_indis_no+1 ; i < K9_ARRAY_SIZE ; i++) {
            ...
            calisan_firma_ent_partiye_giren[i]       = calisan_firma_ent_partiye_giren[gun_indis_no];
            calisan_firma_ent_partiden_cikan[i]      = calisan_firma_ent_partiden_cikan[gun_indis_no];

            calisan_firma_ent_olm_partiye_giren[i]   = calisan_firma_ent_olm_partiye_giren[gun_indis_no];
            calisan_firma_ent_olm_partiden_cikan[i]  = calisan_firma_ent_olm_partiden_cikan[gun_indis_no];
        ...
        sql_query.PREPARE_UPDATE("stk_urunun_partileri" , "parti_id",
                                 "toplam_partiye_giren_array ,"
                                 "toplam_partiden_cikan_array,"
                                 "ent_partiye_giren_array   ,"
                                 "ent_partiden_cikan_array   ,"
                                 "ent_olm_partiye_giren_array ,"
                                 "ent_olm_partiden_cikan_array ",
                                 "parti_id = :parti_id");
        sql_query.SET_VALUE(":toplam_partiye_giren_array"   , PACK_DOUBLE_ARRAY(toplam_partiye_giren  , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":toplam_partiden_cikan_array"  , PACK_DOUBLE_ARRAY(toplam_partiden_cikan , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_partiye_giren_array"      , PACK_DOUBLE_ARRAY(ent_partiye_giren     , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_partiden_cikan_array"     , PACK_DOUBLE_ARRAY(ent_partiden_cikan    , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_olm_partiye_giren_array"  , PACK_DOUBLE_ARRAY(ent_olm_partiye_giren , K9_ARRAY_SIZE));
        sql_query.SET_VALUE(":ent_olm_partiden_cikan_array" , PACK_DOUBLE_ARRAY(ent_olm_partiden_cikan, K9_ARRAY_SIZE));
        ...
        if ( islem_gormus_parti_id_list.contains(QVariant(parti_id).toString()) EQ true ) {
            ...
            for (int i = gun_no + 1 ; i < K9_ARRAY_SIZE ; i++) {
                ...
                ent_partiye_giren[i]      = yil_sonu_ent_partiye_giren;
                ent_partiden_cikan[i]     = yil_sonu_ent_partiden_cikan;

                ent_olm_partiye_giren[i]  = yil_sonu_ent_olm_partiye_giren;
                ent_olm_partiden_cikan[i] = yil_sonu_ent_olm_partiden_cikan;
            }
            ...
            insert_arsiv_firma.PREPARE_INSERT("stk_urunun_partileri" , "parti_id" , "parti_id,depo_id,"
                                              ...
                                              "ent_olm_partiye_giren_array , ent_olm_partiden_cikan_array ");
            ...
            insert_arsiv_firma.SET_VALUE(":toplam_partiye_giren_array"  , PACK_DOUBLE_ARRAY ( toplam_partiye_giren    , K9_ARRAY_SIZE));
            insert_arsiv_firma.SET_VALUE(":toplam_partiden_cikan_array" , PACK_DOUBLE_ARRAY ( toplam_partiden_cikan   , K9_ARRAY_SIZE));
            insert_arsiv_firma.SET_VALUE(":ent_partiye_giren_array"     , PACK_DOUBLE_ARRAY ( ent_partiye_giren       , K9_ARRAY_SIZE ));
            insert_arsiv_firma.SET_VALUE(":ent_partiden_cikan_array"    , PACK_DOUBLE_ARRAY ( ent_partiden_cikan      , K9_ARRAY_SIZE ));
            insert_arsiv_firma.SET_VALUE(":ent_olm_partiye_giren_array" , PACK_DOUBLE_ARRAY ( ent_olm_partiye_giren   , K9_ARRAY_SIZE  ));
            insert_arsiv_firma.SET_VALUE(":ent_olm_partiden_cikan_array", PACK_DOUBLE_ARRAY ( ent_olm_partiden_cikan  , K9_ARRAY_SIZE));
...
/*************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::STK_URUNUN_SERI_NUMARALARI_TABLOSUNU_KOPYALA();   */
/*************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STK_URUNUN_SERI_NUMARALARI_TABLOSUNU_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY insert_arsiv_firma(ARSIV_DB);
...
/*****************************************************************************************/
/*             K9_DEVIR_ISLEMI_BATCH::IRS_IADE_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA();                        */
/*****************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::IRS_IADE_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA(int irs_satir_id)
{
    SQL_QUERY select_devredilecek_firma(DB);
...
/*****************************************************************************************/
/*             K9_DEVIR_ISLEMI_BATCH::STOK_ACILIS_FISI_OLUSTUR();                        */
/*****************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::STOK_ACILIS_FISI_OLUSTUR(int entegre_fis_mi)
{
    ...
    for (int i = 0 ; i < depolar_list.size() ; i++) {
        ...
        query_arsiv_db.PREPARE_SELECT("stk_depodaki_urunler" , "urun_id,ent_depoya_giren_array,"
                             "ent_depodan_cikan_array,ent_olmayan_depoya_giren_array,"
                             "ent_olmayan_depo_cikan_array" , "depo_id = :depo_id");
        ...
        if ( query_arsiv_db.SELECT() NE 0 ) {
            while ( query_arsiv_db.NEXT()) {
                ...
                if ( entegre_fis_mi EQ 1 ) {
                    UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_depoya_giren_array").toString() , depoya_giren_urun_miktari , K9_ARRAY_SIZE);
                    UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_depodan_cikan_array").toString() , depodan_cikan_urun_miktari , K9_ARRAY_SIZE);
                }
                else {
                    UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_olmayan_depoya_giren_array").toString() , depoya_giren_urun_miktari , K9_ARRAY_SIZE);
                    UNPACK_DOUBLE_ARRAY(query_arsiv_db.VALUE("ent_olmayan_depo_cikan_array").toString() , depodan_cikan_urun_miktari , K9_ARRAY_SIZE);
                }
                ...
                if (old_depo_id NE depo_id) {
                    ...
                    insert_devredilecek_firma.PREPARE_INSERT("stk_fisler" , "fis_id" , "fis_no,fis_tarihi,aciklama,"
                                                             ...
                                                             "kaydedilecek_sirket,base_fis_id,iade_irsaliyesi_mi");
                    ...
                    insert_devredilecek_firma.SET_VALUE(":program_id"   , K9_PROGRAMI);
                    insert_devredilecek_firma.SET_VALUE(":kaydedilecek_sirket" , entegre_fis_mi);
                ...
                if ( stok_takip_sekli EQ ENUM_PARTI_TAKIBI) {
                    ...
                    sql_query.PREPARE_SELECT("stk_urunun_partileri ","parti_id , ent_partiye_giren_array,"
                                             "ent_partiden_cikan_array,ent_olm_partiye_giren_array,"
                                             "ent_olm_partiden_cikan",
                    ...
                    if ( sql_query.SELECT() NE 0 ) {
                        ...
                        while (sql_query.NEXT()) {
                             ...
                             if ( entegre_fis_mi EQ 1 ) {
                                 UNPACK_DOUBLE_ARRAY(sql_query.VALUE("ent_partiye_giren_array").toString() , partiye_giren_urun_miktari ,K9_ARRAY_SIZE);
                                 UNPACK_DOUBLE_ARRAY(sql_query.VALUE("ent_partiden_cikan_array").toString() , partiden_cikan_urun_miktari , K9_ARRAY_SIZE);
                             }
                             else {
                                 UNPACK_DOUBLE_ARRAY(sql_query.VALUE("ent_olm_partiye_giren_array").toString() , partiye_giren_urun_miktari ,K9_ARRAY_SIZE);
                                 UNPACK_DOUBLE_ARRAY(sql_query.VALUE("ent_olm_partiden_cikan_array").toString() , partiden_cikan_urun_miktari , K9_ARRAY_SIZE);
                             }
                            ...
                            insert_devredilecek_firma.PREPARE_INSERT("stk_fis_satirlari" , "fis_satiri_id",
                                                                     ...
                                                                     "program_id , base_record_id,kaydedilecek_sirket");
...
/***************************************************************************************/
/*            K9_DEVIR_ISLEMI_BATCH::FATURA_MODULUNU_DEVRET();                         */
/***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::FATURA_MODULUNU_DEVRET()
{
...
/***************************************************************************************/
/*                   K9_DEVIR_ISLEMI_BATCH::FATURALARI_KOPYALA();                      */
/***************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::FATURALARI_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma (DB);
    ...
    where_list << m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd");
    where_list << QString::number(FATURA_MODULU)<< QString::number(K9_PROGRAMI) << QString::number(K9_PROGRAMI);


    K9_COPY_TABLES(DB ,ARSIV_DB , "fat_faturalar" , where_str , where_list);
    ...
    select_devredilecek_firma.SET_VALUE(":program_id_1" , K9_PROGRAMI);
    select_devredilecek_firma.SET_VALUE(":program_id"   , K9_PROGRAMI);
    ...
    while (select_devredilecek_firma.NEXT()) {
        ...
        K9_COPY_TABLES ( DB ,ARSIV_DB , "fat_fatura_satirlari" , QString("fis_id = :arg_%1").arg(0) ,
                              where_list);
...
/*******************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::FAT_IRS_BAGLANTISI_TABLOSUNU_KOPYALA();                */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::FAT_IRS_BAGLANTISI_TABLOSUNU_KOPYALA(int fatura_id)
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY insert_arsiv_firma(ARSIV_DB);
...
/*********************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::FAT_IRS_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA();             */
/*********************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::FAT_IRS_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA(int fatura_satiri_id)
{
    SQL_QUERY select_devredilecek_firma(DB);
    SQL_QUERY insert_arsiv_firma(ARSIV_DB);
...
/*********************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::FAT_IADE_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA();             */
/*********************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::FAT_IADE_SATIR_BAGLANTISI_TABLOSUNU_KOPYALA(int fatura_satiri_id)
{
    SQL_QUERY select_devredilecek_firma (DB);
...
/*******************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::PERAKENDE_MODULUNU_DEVRET();                            */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::PERAKENDE_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "prk_gelir_gider_turleri" ,"");
...
/*******************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::PERAKENDE_MODULU_FISLERINI_KOPYALA();                   */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::PERAKENDE_MODULU_FISLERINI_KOPYALA()
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "prk_fisler" , QString("fis_tarihi < :arg_%1").arg(0) , where_list);
    ...
    while ( select_devredilecek_firma.NEXT() ) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "prk_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , where_list);
...
/*******************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::PERSONEL_MODULUNU_DEVRET();                           */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::PERSONEL_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "prs_sabit_degerler" , "");
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "prs_isten_ayrilanlar" ,
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "prs_firma_odenekleri" , "");

    K9_COPY_TABLES(DB , ARSIV_DB , "prs_kesinti_odenek_tanimlar" , "");
...
/******************************************************************************************/
/*                K9_DEVIR_ISLEMI_BATCH::PRS_BORDROLAR_TABLOSUNU_KOPYALA();                    */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::PRS_BORDROLAR_TABLOSUNU_KOPYALA()
{
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "prs_bordrolar" , QString("bordro_kesim_tarihi < '%1'").arg(m_calisan_firma_acilis_tarihi.toString("yyyy.MM.dd")));
    ...
    while ( query_db.NEXT() ) {
        ...
        K9_COPY_TABLES(DB , ARSIV_DB , "prs_bordro_personelleri" , QString("bordro_id = %1").arg(bordro_id));

        K9_COPY_TABLES(DB , ARSIV_DB , "prs_brd_kesinti_odenekleri" , QString("bordro_id = %1").arg(bordro_id));
        ...
        if ( s_query_db.SELECT() NE 0 ) {
            ...
            K9_COPY_TABLES(DB ,ARSIV_DB , "prs_puantaj" , QString("puantaj_id = %1").arg(s_query_db.VALUE(0).toInt()));

            K9_COPY_TABLES(DB ,ARSIV_DB , "prs_eksik_gun_nedenleri" , QString("personel_id = %1").arg(s_query_db.VALUE(1).toInt()));
        }
    }
...
/*******************************************************************************************/
/*          K9_DEVIR_ISLEMI_BATCH::PRS_PERSONELLER_TABLOSUNU_KOPYALA();                    */
/******************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::PRS_PERSONELLER_TABLOSUNU_KOPYALA()
{

    K9_COPY_TABLES(DB ,ARSIV_DB , "prs_personeller" , "");
    ...
    K9_COPY_TABLES(DB , ARSIV_DB , "prs_personel_kisisel_blgler" ,"");

    //PERSONEL NOT BILGILERI
    K9_COPY_TABLES(DB ,ARSIV_DB , "prs_personel_notlari" ,"" );

    //PERSONEL NUFUS BILGILERI

    K9_COPY_TABLES(DB ,ARSIV_DB , "prs_personel_nufus_blgler" ,"" );

    //PERSONEL AILE UYELERI

    K9_COPY_TABLES(DB ,ARSIV_DB , "prs_personel_aile_uyeleri" ,"" );

    //PERSONEL EK ODENEKLER/KESINTILER

    K9_COPY_TABLES(DB ,ARSIV_DB , "prs_ek_kesintiler_odenekler" ,"" );
...
/******************************************************************************************/
/*                K9_DEVIR_ISLEMI_BATCH::FIND_UYGUN_ACILIS_BORDROSU()                    */
/******************************************************************************************/

int K9_DEVIR_ISLEMI_BATCH::FIND_UYGUN_ACILIS_BORDROSU(int bordro_islem_turu, int cek_senet_turu,
                                                      int hesap_no_id,int kaydedilecek_sirket)
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    QString where_str = "bordro_turu =:bordro_turu "
                        ...
                        "bordro_islem_turu = :bordro_islem_turu AND kaydedilecek_sirket = :kaydedilecek_sirket";
    ...
    select_devredilecek_firma.SET_VALUE(":kaydedilecek_sirket"  , kaydedilecek_sirket);
...
/********************************************************************************************/
/*             K9_DEVIR_ISLEMI_BATCH::CIRO_EDILEN_CEK_ODENDI_KABUL_EDILECEK_MI()           */
/********************************************************************************************/

bool K9_DEVIR_ISLEMI_BATCH::CIRO_EDILEN_CEK_ODENDI_KABUL_EDILECEK_MI(int cek_senet_id , QDate islem_tarihi)
{
    SQL_QUERY select_devredilecek_firma(DB);

    select_devredilecek_firma.PREPARE_SELECT("k9_sabit_degerler " , "cek_ciro_sonrasi_gun_sayisi");
...
/******************************************************************************/
/*       K9_DEVIR_ISLEMI_BATCH::CEK_SENET_ARSIV_FIRMADA_ISLEM_GORDU_MU()      */
/******************************************************************************/

bool K9_DEVIR_ISLEMI_BATCH::CEK_SENET_ARSIV_FIRMADA_ISLEM_GORDU_MU(int cek_senet_id)
{
    SQL_QUERY select_devredilecek_firma(DB);
...
/*********************************************************************************/
/*        K9_DEVIR_ISLEMI_BATCH::CEK_SENET_CALISAN_FIRMADA_ISLEM_GORDU_MU()      */
/*********************************************************************************/

bool K9_DEVIR_ISLEMI_BATCH::CEK_SENET_CALISAN_FIRMADA_ISLEM_GORDU_MU(int cek_senet_id)
{
    SQL_QUERY select_devredilecek_firma(DB);
...
/***********************************************************************************/
/*           K9_DEVIR_ISLEMI_BATCH::GET_SUBELER_VE_SUBENIN_DEPOLARI_LIST()         */
/***********************************************************************************/

QStringList K9_DEVIR_ISLEMI_BATCH::GET_DEPOLAR_LIST()
{
...
/**********************************************************************************/
/*           K9_DEVIR_ISLEMI_BATCH::GET_YILA_GORE_SERI_NUMARALARI_ID_LIST()       */
/**********************************************************************************/

QStringList K9_DEVIR_ISLEMI_BATCH::GET_YILA_GORE_SATIRDAKI_SERI_NUMARALARI_ID_LIST(QDate fis_tarihi)
{
    SQL_QUERY query(DB);
    SQL_QUERY select_query(DB);
    QStringList seri_numaralari_id_list;
...
QStringList K9_DEVIR_ISLEMI_BATCH::GET_YILA_GORE_SATIRDAKI_PARTI_ID_LIST(QDate fis_tarihi)
{
    SQL_QUERY query(DB);
...
void K9_DEVIR_ISLEMI_BATCH::HESAP_BAKIYELERINI_SIFIRLA(QString p_table_name, QString id_column_name,
                                                       QStringList column_names,
                                                       int values_size , ADAK_SQL * P_GELEN_DB)
{
    SQL_QUERY query(P_GELEN_DB);

    double    toplam_borc_alacak[K9_ARRAY_SIZE];
    ...
    for ( int i = 0 ; i < values_size ; i++ ) {
        ...
        query.SET_VALUE(QString(":%1").arg(column_names.at(i)) , PACK_DOUBLE_ARRAY(toplam_borc_alacak   , K9_ARRAY_SIZE));
...
void K9_DEVIR_ISLEMI_BATCH::DEMIRBAS_MODULUNU_DEVRET ()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "dmr_demirbaslar" , "");
    K9_COPY_TABLES(DB ,ARSIV_DB , "dmr_satilan_demirbaslar" , "");
...
/*************************************************************************************************
                                 DEMIRBAS_MODULU_FISLERINI_KOPYALA
*************************************************************************************************/
void  K9_DEVIR_ISLEMI_BATCH::DEMIRBAS_MODULU_FISLERINI_KOPYALA ()
{
    SQL_QUERY select_devredilecek_firma(DB);
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "dmr_fisler" , QString("fis_tarihi < :arg_%1").arg(0) , where_list);
    ...
    while ( select_devredilecek_firma.NEXT() ) {
        ...
        K9_COPY_TABLES(DB ,ARSIV_DB , "dmr_fis_satirlari" , QString("fis_id = :arg_%1").arg(0) , where_list);
...
void K9_DEVIR_ISLEMI_BATCH::SMM_MODULUNU_DEVRET()
{
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "smm_hizmetler" , "");
    ...
    K9_COPY_TABLES(DB ,ARSIV_DB , "smm_makbuzlar" , QString("makbuz_tarihi < :arg_%1").arg(0) , where_list);
...
/*************************************************************************************************
                             K9_DEVIR_ISLEMI_BATCH::VERITABANI_BOS_MU
*************************************************************************************************/

bool K9_DEVIR_ISLEMI_BATCH::VERITABANI_BOS_MU( ADAK_SQL *P_DB )
{
    //! Veritabani bos mu ?
    SQL_QUERY query( P_DB );

    //! VERI TABANINDA KAYIT OLUP OLMADIGINA BAKILIYOR.
    query.PREPARE_SELECT( "k9_sabit_degerler", "sabit_deger_id");
    if ( query.SELECT() NE 0 ) {
        return false;
    }
    ...
    query.PREPARE_SELECT( "k9_kullanici_yetkileri", "kullanici_id");
...
/*************************************************************************************************
                             K9_DEVIR_ISLEMI_BATCH::SEARCH_EDIT_CLICKED
*************************************************************************************************/

void K9_DEVIR_ISLEMI_BATCH::SEARCH_EDIT_CLICKED(QWidget *widget, QLineEdit *line_edit)
{
    Q_UNUSED( line_edit );
