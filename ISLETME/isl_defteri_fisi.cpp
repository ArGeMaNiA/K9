...
void ISL_DEFTERI_FISI::SETUP_FORM()
{
    ...
    m_ui->reklam_viewer_dikey->SHOW_REKLAM( "GELIR-VE-GIDER-FISLERI" );
    ...
    M_ISL_FISI->program_id = K9_PROGRAMI;
...
int ISL_DEFTERI_FISI::LEAVE_RECORD()
{
    if (m_log_kaydi_eklensin_mi EQ true ) {
        ...
        if(m_kayit_eklendi EQ true) {
            K9_LOG_KAYDI_EKLE ( ISLETME_MODULU , LOG_ISL_FISLER , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if(m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE ( ISLETME_MODULU , LOG_ISL_FISLER , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if(m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE ( ISLETME_MODULU , LOG_ISL_FISLER , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
