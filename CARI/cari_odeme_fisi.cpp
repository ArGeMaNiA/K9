...
#include "k9_uretilen_fis_secimi_open.h"
#include "k9_yetki.h"
#include "k9_log.h"
...
void CARI_ODEME_FISI::SETUP_FORM()
{
    ...
    M_FIS_DETAYLARI      = new K9_ENT_DETAYLARI_STRUCT;
    M_ISLEM_DETAYLARI    = new K9_ISLEM_DETAYLARI;
    ...
    if( K9_KULLANICININ_CALISTIGI_SIRKET() EQ 1 ){
        ...
        M_FIS->kaydedilecek_sirket = SIRKET_1;
    }else {
        m_ent_filtresi_turu =  FIS_HER_IKI_SIRKET;
        M_FIS->kaydedilecek_sirket = SIRKET_2;
    }

    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket, K9_GET_KAYDEDILECEK_SIRKET_STRING( K9_KULLANICININ_CALISTIGI_SIRKET() ));
...
void CARI_ODEME_FISI::NEW_FIS_RECORD()
{
    ...
    M_FIS->program_id          = K9_PROGRAMI;
    M_FIS->modul_id            = CARI_MODULU;
    M_FIS->kaydedilecek_sirket = K9_KULLANICININ_CALISTIGI_SIRKET();
...
int CARI_ODEME_FISI::GET_FIS_RECORD ( int fis_id )
{
    ...
    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket, K9_GET_KAYDEDILECEK_SIRKET_STRING( M_FIS->kaydedilecek_sirket ));
    ...
    if ( sql_query.SELECT() NE 0 ) {
        ...
        if ( m_ent_filtresi_turu EQ FIS_BIRINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(5).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(6).toString() , alacak_array , K9_ARRAY_SIZE);
        }
        else if ( m_ent_filtresi_turu EQ FIS_IKINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(7).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(8).toString() , alacak_array , K9_ARRAY_SIZE);
        }
        else {
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(3).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(sql_query.VALUE(4).toString() , alacak_array , K9_ARRAY_SIZE);
        }

        double hesap_borc   = borc_array[K9_SON_BAKIYE];
        double hesap_alacak = alacak_array[K9_SON_BAKIYE];
   ...
   if ( m_makbuz_para_birimi_id EQ m_cari_para_birimi_id  ) {
    ...
    while ( select_query.NEXT() EQ true ) {
        ...
        combobox_odeme_turu->setCurrentIndex            ( combobox_odeme_turu->findText ( K9_GET_TAH_ODM_SEKLI_STRING(select_query.VALUE(3).toInt(),ENUM_CARI_ODEME_FISI)));
        ...
        int islem_turu = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_odeme_turu->currentText());
...
int CARI_ODEME_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
    ...
    if( object EQ m_ui->comboBox_fis_filtresi )
    {
    ...
    else if ( object EQ m_ui->comboBox_sirket ) {
        M_FIS->kaydedilecek_sirket = K9_GET_KAYDEDILECEK_SIRKET_ENUM( m_ui->comboBox_sirket->currentText() );
    }
    else if ( object EQ m_ui->searchedit_cari_hesap_kodu ) {
        ...
        if ( m_ent_filtresi_turu EQ FIS_BIRINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(5).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(6).toString() , alacak_array , K9_ARRAY_SIZE);
        }
        else if ( m_ent_filtresi_turu EQ FIS_IKINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(7).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(8).toString() , alacak_array , K9_ARRAY_SIZE);
        }
        else {
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(3).toString() , borc_array   , K9_ARRAY_SIZE);
            UNPACK_DOUBLE_ARRAY(select_query.VALUE(4).toString() , alacak_array , K9_ARRAY_SIZE);
        }
...
int CARI_ODEME_FISI::CHECK_LINE_VAR(int row_number, QObject * object)
{
    ...
    int islem_turu = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_odeme_sekli->currentText());
    ...
    if ( object EQ combobox_odeme_sekli) {
        ...
        int odeme_sekli = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_odeme_sekli->currentText() );
       ...
       if (lineedit_cek_senet_id->text().toInt() EQ 0 ) {
        ...
        if ( islem_turu EQ ENUM_NAKIT ) {
            ...
            if ( M_FIS->kaydedilecek_sirket EQ 1 ) {
                SQL_QUERY query( DB );

                query.PREPARE_SELECT( "k9_sabit_degerler", "max_nakit_giris_cikis_tutari","sabit_deger_id = :sabit_deger_id" );
...
int CARI_ODEME_FISI::CHECK_ADD_LINE(int fis_id, int row_number)
{
    ...
    int islem_turu = K9_GET_TAH_ODM_SEKLI_ENUM(combo_box->currentText());
    ...
    switch (islem_turu) {
        case ENUM_KREDI_KARTI :
        {
            ...
            if (lineedit_kredi_karti_id->text().toInt() EQ 0 ) {
                if ( OPEN_K9_KK_ODEME_BATCH(satir_id,M_ISLEM_DETAYLARI,CARI_MODULU,this ) EQ false ) {
                    return ADAK_FAIL;
                }
        ...
        case ENUM_EFT      :
        {
            ...
            if (lineedit_hesap_no_id->text().toInt() EQ 0 ) {
                ...
                if ( OPEN_K9_EFT_BATCH(ENUM_CARI_ODEME_FISI ,islem_turu , satir_id ,
...
void CARI_ODEME_FISI::ADD_LINE ( int p_fis_id, int p_row_number )
{
    ...
    M_FIS_SATIRI->islem_turu                = K9_GET_TAH_ODM_SEKLI_ENUM (combobox_odeme_turu->currentText() );
    ...
    M_FIS_SATIRI->kaydedilecek_sirket       = M_FIS->kaydedilecek_sirket;
...
void CARI_ODEME_FISI::UPDATE_LINE ( int fis_id, int row_number )
{
    ...
    int new_islem_turu = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_odeme_turu->currentText() );
...
int CARI_ODEME_FISI::CHECK_DELETE_LINE(int fis_id, int row_number)
{
    ...
    if ( K9_GET_TAH_ODM_SEKLI_ENUM(combobox_odeme_turu->currentText()) EQ ENUM_NAKIT ) {
        return ADAK_OK;
    }
...
void  CARI_ODEME_FISI::LINE_SEARCH_EDIT_CLICKED (int row_number,QWidget * ,QLineEdit *lineedit )
{
    ...
    if ( odeme_sekli EQ ENUM_MUSTERI_CEKI OR odeme_sekli EQ ENUM_KENDI_CEKIMIZ) {
        ...
        if (odeme_sekli EQ ENUM_MUSTERI_CEKI) {
            cek_id = OPEN_CEK_SENET_ARAMA ( ENUM_MUSTERI_CEKI , ENUM_CEK , true , m_cari_hesap_id ,
                                            m_makbuz_para_birimi_id ,m_ent_filtresi_turu, this );
        }
        else {
            cek_id = OPEN_CEK_SENET_ARAMA ( ENUM_KENDI_CEKIMIZ , ENUM_CEK , false , 0 ,
                                            m_makbuz_para_birimi_id ,m_ent_filtresi_turu, this);
        }
    ...
    if ( odeme_sekli EQ ENUM_MUSTERI_SENEDI OR odeme_sekli EQ ENUM_KENDI_SENEDIMIZ ) {
        ...
        if (odeme_sekli EQ ENUM_MUSTERI_SENEDI ) {
            senet_id = OPEN_CEK_SENET_ARAMA(ENUM_MUSTERI_SENEDI,ENUM_SENET,true, m_cari_hesap_id,
                                            m_makbuz_para_birimi_id,m_ent_filtresi_turu,this);
        }
        else {
            senet_id = OPEN_CEK_SENET_ARAMA(ENUM_KENDI_SENEDIMIZ,ENUM_SENET,false,1,
                                            m_makbuz_para_birimi_id,m_ent_filtresi_turu,this);
        }
...
void CARI_ODEME_FISI::SEARCH_EDIT_CLICKED(QWidget * widget,QLineEdit * lineedit)
{
    ...
    int hesap_id = OPEN_CARI_KART_ARAMA ( lineedit->text(), SAHIS_ARTI_FIRMA, this, 1, K9_PROGRAMI, CARI_MODULU );
...
{ 
    if ( p_button EQ m_ui->push_button_odeme_makbuzu_yazdir ) {
        ...
        M_FIS->yazdirildi_mi = 1;
         ...
         K9_URETILEN_FIS_SEC( K9_PROGRAMI, CARI_MODULU, p_record_id, this );
    }
}
...
int CARI_ODEME_FISI::CHANGER_BUTTON_CLICKED ( QAbstractButton * p_button )
{
    if ( p_button EQ m_ui->pushButton_detaylar ) {
        ...
        M_FIS_DETAYLARI->muh_fis_no = K9_GET_FIS_NO ( MUHASEBE_MODULU , muh_fis_id);
        ...
        for ( int i = 0 ; i < cek_fis_id_list.size() ; i++ ) {
           ...
           if ( bordo_turu EQ ENUM_MUSTERI_CEK_BORDROSU ) {
                M_FIS_DETAYLARI->cek_fis_no = K9_GET_FIS_NO(CEKSENET_MODULU , cek_senet_fis_id);
           }
           else if( bordo_turu EQ ENUM_MUSTERI_SENET_BORDROSU ){
               M_FIS_DETAYLARI->senet_fis_no = K9_GET_FIS_NO(CEKSENET_MODULU , cek_senet_fis_id);
           }
           else if( bordo_turu EQ ENUM_FIRMA_CEK_BORDROSU) {
               M_FIS_DETAYLARI->firma_cek_fis_no = K9_GET_FIS_NO(CEKSENET_MODULU , cek_senet_fis_id);
           }
           else if( bordo_turu EQ ENUM_FIRMA_SENET_BORDROSU ){
               M_FIS_DETAYLARI->firma_senet_fis_no = K9_GET_FIS_NO(CEKSENET_MODULU , cek_senet_fis_id);
           }
        }
        ...
        if ( query.SELECT() NE 0 ) {
            while ( query.NEXT() ) {
                ...
                if ( hesap_no_id NE 0 ) {
                    ...
                    int banka_fis_no = K9_GET_FIS_NO(BANKA_MODULU , bnk_fis_id);
        ...
        OPEN_K9_ENT_DETAYLARI_BATCH(M_FIS_DETAYLARI , true , this);
...
int CARI_ODEME_FISI::LINE_CHANGER_BUTTON_CLICKED(QAbstractButton *button, int row_number)
{
    ...
    case ENUM_HAVALE:
    case ENUM_EFT:
        if ( OPEN_K9_EFT_BATCH(ENUM_CARI_ODEME_FISI,islem_turu ,fis_satiri_id ,m_makbuz_para_birimi_id,
    ...
    case ENUM_KREDI_KARTI:
        ...
        if ( OPEN_K9_KK_ODEME_BATCH(fis_satiri_id , M_ISLEM_DETAYLARI ,CARI_MODULU, this ) EQ true ) {
            QLineEdit * lineedit_kredi_karti_id = ( QLineEdit * ) m_ui->tablewidget_fis_satirlari->cellWidget(row_number,KREDI_KARTI_ID_COLUMN);
            lineedit_kredi_karti_id->setText(QVariant(M_ISLEM_DETAYLARI->kredi_karti_id).toString());
            QLineEdit * lineedit_hesap_no_id = (QLineEdit * ) m_ui->tablewidget_fis_satirlari->cellWidget(row_number,HESAP_NO_ID_COLUMN);
            lineedit_hesap_no_id->setText(QVariant(M_ISLEM_DETAYLARI->bnk_hesap_no_id).toString());
...
int CARI_ODEME_FISI::LEAVE_RECORD()
{
    if ( m_log_kaydi_eklenecek_mi EQ true ) {
        ...
        if(m_kayit_eklendi EQ true) {
            K9_LOG_KAYDI_EKLE ( CARI_MODULU , LOG_CARI_FISLER , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if(m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE ( CARI_MODULU , LOG_CARI_FISLER , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if(m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE ( CARI_MODULU , LOG_CARI_FISLER , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
        DB->COMMIT_TRANSACTION();
    }
    return ADAK_OK;
...
/**************************************************************************************
                   CARI_ODEME_FISI::SHOW_REKLAM
***************************************************************************************/

void CARI_ODEME_FISI::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("CARI_ODEME_FISI");
}
