...
void CARI_KART_ARAMA::SETUP_FORM()
{
    ...
    if ( m_program_id EQ K9_PROGRAMI AND m_modul_id NE PERSONEL_MODULU ) {
...
void CARI_KART_ARAMA::SEARCH()
{
    ...
    f_query.PREPARE_SELECT( "car_hesaplar","cari_hesap_id, cari_hesap_kodu, adres_id,"
                                           "para_birimi_id ,toplam_borc_array, toplam_alacak_array, ent_borc_array, ent_alacak_array" );
    ...
    if ( m_program_id EQ K9_PROGRAMI ) {
    ...
    while (f_query.NEXT() EQ true ) {
        ...
        UNPACK_DOUBLE_ARRAY(f_query.VALUE("toplam_borc_array").toString() , toplam_borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(f_query.VALUE("toplam_alacak_array").toString() , toplam_alacak_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(f_query.VALUE("ent_borc_array").toString() , ent_borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(f_query.VALUE("ent_alacak_array").toString() , ent_alacak_array , K9_ARRAY_SIZE);

        double toplam_borc   = 0.00;
        double toplam_alacak = 0.00;

        if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ SIRKET_1 ) {
            toplam_borc   = ROUND ( ent_borc_array  [ K9_SON_BAKIYE ] );
            toplam_alacak = ROUND ( ent_alacak_array[ K9_SON_BAKIYE ] );
        }
        else {
            toplam_borc   = ROUND ( toplam_borc_array  [ K9_SON_BAKIYE ] );
            toplam_alacak = ROUND ( toplam_alacak_array[ K9_SON_BAKIYE ] );
        }
