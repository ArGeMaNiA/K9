...
void CARI_TAHSILAT_FISI::NEW_FIS_RECORD()
{
    ...
    M_FIS->kaydedilecek_sirket = K9_KULLANICININ_CALISTIGI_SIRKET(); // SIRKET_1 or SIRKET_2
...
int CARI_TAHSILAT_FISI::GET_FIS_RECORD ( int fis_id )
{
    ...
    while ( select_query.NEXT() EQ true ) {
        ...
        combobox_tahsilat_sekli->setCurrentIndex        ( combobox_tahsilat_sekli->findText ( K9_GET_TAH_ODM_SEKLI_STRING(select_query.VALUE(3).toInt(),ENUM_CARI_TAHSILAT_FISI ) ) );
        ...
        combobox_tahsilat_sekli->setCurrentIndex(combobox_tahsilat_sekli->findText(K9_GET_TAH_ODM_SEKLI_STRING(islem_turu,ENUM_CARI_TAHSILAT_FISI)));
...
int CARI_TAHSILAT_FISI::CHECK_LINE_VAR(int row_number, QObject * object)
{
    ...
    if ( object EQ combobox_tahsilat_sekli ) {
        ...
        int islem_turu = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tahsilat_sekli->currentText());
        ...
        if ( K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tahsilat_sekli->currentText() ) EQ ENUM_NAKIT ) {
        ...
        if ( K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tahsilat_sekli->currentText()) EQ ENUM_NAKIT ) {
            ...
            if ( M_FIS->kaydedilecek_sirket EQ SIRKET_1 ) {
                SQL_QUERY query( DB );
    ...
    switch (islem_turu) {
        ...
        case ENUM_KREDI_KARTI    :
            if ( SATIR_KAYDI_YAPILABILIR_MI(row_number,islem_turu) EQ false ) {
                ...
                if ( OPEN_K9_KK_TAHSILAT_BATCH(satir_id ,M_ISLEM_DETAYLARI ,CARI_MODULU,this ) EQ false ) {
                    return ADAK_FAIL;
                }
                ISLEM_DETAYLARINI_GIZLI_SUTUNLARA_YAZ(row_number);
        ...
        case ENUM_EFT      :
            ...
            if ( SATIR_KAYDI_YAPILABILIR_MI( row_number,islem_turu) EQ false ) {
                ...
                if ( OPEN_K9_EFT_BATCH(ENUM_CARI_TAHSILAT_FISI , islem_turu ,satir_id ,
...
void CARI_TAHSILAT_FISI::ADD_LINE ( int fis_id, int row_number )
{
    ...
    M_FIS_SATIRI->islem_turu                            = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tahsilat_sekli->currentText() );
    ...
    // m_ui->lineedit_muh_fis_no->setText ( QVariant ( K9_GET_FIS_NO ( MUHASEBE_MODULU, M_FIS->muh_fis_id ) ).toString() );
...
void CARI_TAHSILAT_FISI::SEARCH_EDIT_CLICKED ( QWidget * widget,QLineEdit * lineedit)
{
    ...
    int  cari_hesap_id = OPEN_CARI_KART_ARAMA ( lineedit->text(),SAHIS_ARTI_FIRMA , this ,1 , K9_PROGRAMI, CARI_MODULU );
...
void CARI_TAHSILAT_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_record_id )
{
    if ( p_button EQ m_ui->push_button_tahsilat_makbuzu_yazdir ) {
        ...
        K9_URETILEN_FIS_SEC( M_FIS->program_id, M_FIS->modul_id, p_record_id, this );
    }
...
int CARI_TAHSILAT_FISI::LINE_CHANGER_BUTTON_CLICKED(QAbstractButton *button, int row_number)
{
    ...
    switch (islem_turu ) {
        ...
        case ENUM_HAVALE :
        case ENUM_EFT:
            if ( OPEN_K9_EFT_BATCH( ENUM_CARI_TAHSILAT_FISI , islem_turu ,fis_satiri_id ,
            ...
            break;
        case ENUM_KREDI_KARTI:
            if ( OPEN_K9_KK_TAHSILAT_BATCH(fis_satiri_id,M_ISLEM_DETAYLARI ,CARI_MODULU, this) EQ true ) {
                ISLEM_DETAYLARINI_GIZLI_SUTUNLARA_YAZ(row_number);
...
/**************************************************************************************
                   CARI_TAHSILAT_FISI::SHOW_REKLAM
***************************************************************************************/

void CARI_TAHSILAT_FISI::SHOW_REKLAM()
{
    m_ui->reklamViewer->setParent(this);
    m_ui->reklamViewer->SHOW_REKLAM("CARI_TAHSILAT_FISI");
    m_ui->reklamViewer->lower();
}
