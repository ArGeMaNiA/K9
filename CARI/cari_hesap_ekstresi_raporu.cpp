...
QString CARI_HESAP_EKSTRESI_RAPORU::GET_HEADER()
{
    ...
    ADD_ROW_TO_TABLE(QStringList()<< K9_GET_FIRMA_ISMI()<<"<b>CARİ HESAP EKSTRESİ"<<GET_REPORT_TOP_RIGHT_HEADER());
...
QString CARI_HESAP_EKSTRESI_RAPORU::GET_BODY()
{
    ...
    for ( ; ; ) {
        ...
        UNPACK_DOUBLE_ARRAY(cari_hesap.at(4) , toplam_borc_array   , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(cari_hesap.at(5) , toplam_alacak_array , K9_ARRAY_SIZE);

        int bas_gun_no = MALI_YIL_ARRAY_INDIS ( M_CARI_RV->baslangic_tarihi);
        int bts_gun_no = MALI_YIL_ARRAY_INDIS ( M_CARI_RV->bitis_tarihi);

        if ( M_CARI_RV->nakli_yekun EQ true ) {
...
QStringList CARI_HESAP_EKSTRESI_RAPORU::GET_CARI_HESAPLAR_LIST()
{
    ...
    while (select_query.NEXT()) {
        ...
        if ( M_CARI_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
            toplam_borc   = select_query.VALUE(4).toString();
            toplam_alacak = select_query.VALUE(5).toString();
        }
        else if ( M_CARI_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
            toplam_borc   = select_query.VALUE(6).toString();
            toplam_alacak = select_query.VALUE(7).toString();
        }
        else if ( M_CARI_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
            toplam_borc   = select_query.VALUE(8).toString();
            toplam_alacak = select_query.VALUE(9).toString();
        }
...
QStringList CARI_HESAP_EKSTRESI_RAPORU::GET_CARI_HESAP_ISLEMLERI  ( int cari_hesap_id )
{
    ...
    if ( M_CARI_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        select_query.AND_EKLE("kaydedilecek_sirket = :kaydedilecek_sirket " );
        select_query.SET_VALUE( ":kaydedilecek_sirket", 1 );
    }
    else if ( M_CARI_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
        select_query.AND_EKLE("kaydedilecek_sirket = :kaydedilecek_sirket " );
        select_query.SET_VALUE( ":kaydedilecek_sirket", 0 );
    }
    ...
    if ( M_CARI_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        select_query.AND_EKLE("car_fis_satirlari.kaydedilecek_sirket = :kaydedilecek_sirket " );
        select_query.SET_VALUE( ":kaydedilecek_sirket", 1 );
    }
    else if ( M_CARI_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ){
        select_query.AND_EKLE("car_fis_satirlari.kaydedilecek_sirket = :kaydedilecek_sirket " );
