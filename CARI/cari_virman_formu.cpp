...
void CARI_VIRMAN_FORMU::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "CARI-VIRMAN-FORMU");
    ...
    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_fis_filtresi, K9_GET_FIS_FILTRESI_STRING( m_entegre_filtresi ));
...
int CARI_VIRMAN_FORMU::GET_RECORD ( int record_id )
{
    ...
    select_query.PREPARE_SELECT ( "car_fisler",
                                  "fis_no , fis_tarihi , aciklama, doviz_kuru,kaydedilecek_sirket,"
                                  "program_id , modul_id ",
                                  "fis_id = :fis_id" );
    ...
    M_FIS->kaydedilecek_sirket           = select_query.VALUE ( "kaydedilecek_sirket"   ).toInt();
...
int CARI_VIRMAN_FORMU::ADD_RECORD ()
{
    ...
    K9_LOG_KAYDI_EKLE ( CARI_MODULU,LOG_CARI_FISLER, LOG_ISLEM_ADD, log_detaylari );

    double temel_para_birimi_tutari = 0.0;

    if ( m_borclu_hesap_para_birimi_id EQ DVZ_GET_TEMEL_PARA_BIRIMI_ID()) {
        temel_para_birimi_tutari = m_ui->commaEdit_tutar->GET_DOUBLE();
    }
    else {
        temel_para_birimi_tutari = ROUND ( m_ui->commaEdit_tutar->GET_DOUBLE() * m_ui->commaEdit_kur->GET_DOUBLE() );
    }
...
void CARI_VIRMAN_FORMU::UPDATE_RECORD (int record_id)
{
    ...
    K9_LOG_KAYDI_EKLE ( CARI_MODULU , LOG_CARI_FISLER , LOG_ISLEM_UPDATE , log_detaylari );
...
void CARI_VIRMAN_FORMU::DELETE_RECORD ( int record_id )
{
    ...
    K9_LOG_KAYDI_EKLE ( CARI_MODULU , LOG_CARI_FISLER , LOG_ISLEM_DELETE , log_detaylari );
...
int CARI_VIRMAN_FORMU::CHANGER_BUTTON_CLICKED(QAbstractButton *button)
{
    ...
    OPEN_K9_ENT_DETAYLARI_BATCH(M_FIS_DETAYLARI , false,this);
...
void CARI_VIRMAN_FORMU::SAVER_BUTTON_CLICKED(QAbstractButton *button, int record_id)
{
    if ( button EQ m_ui->pushButton_yazdir ) {
        OPEN_REPORT_SHOWER(GET_CARI_FISI_YAZDIR_RAPORU(record_id , ENUM_CARI_VIRMAN_FORMU),nativeParentWidget());
         ...
         K9_URETILEN_FIS_SEC( K9_PROGRAMI, CARI_MODULU, record_id, this );
    }
}
