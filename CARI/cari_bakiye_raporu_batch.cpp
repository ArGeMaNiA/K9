...
int CARI_BAKIYE_RAPORU_BATCH::CHECK_RUN()
{
    ...
    if ( K9_MALI_YIL_TARIH_ARALIGI_KONTROLU( m_ui->adakDate_bsl_tarihi, m_ui->adakDate_bts_tarihi ) NE ADAK_OK ) {
        return ADAK_FAIL;
    }
...
void CARI_BAKIYE_RAPORU_BATCH::RUN_BATCH()
{
    ...
    CARI_RV->entegre_filtresi = K9_GET_RAPOR_SIRKET_FILTRESI_ENUM(m_ui->comboBox_sirket_turu->currentText());
    ...
    if ( CARI_RV->nakli_yekun EQ true ) {
    ...
    if ( CARI_RV->bakiyesi_hesaplar EQ true ) {
        if ( CARI_RV->calismamis_hesaplar EQ true OR CARI_RV->nakli_yekun EQ true) {
            log_detaylari.append ( " , ");
        }
    ...
    K9_LOG_KAYDI_EKLE ( m_modul_id,LOG_CARI_RAPORLAR ,LOG_ISLEM_RAPOR_ALINDI,log_detaylari);
