...

{
    ...
    if ( M_CARI_RV->CARI_ID_LIST.size() NE 0 ) {
        ...
        if( !( M_CARI_RV->program_id EQ K9_PROGRAMI AND M_CARI_RV->modul_id EQ CARI_MODULU ) ) {
...
QString CARI_BAKIYE_RAPORU::GET_HEADER()
{
   ...
   ADD_ROW_TO_TABLE ( QStringList() << K9_GET_FIRMA_ISMI() << "<b>CARİ BAKİYE RAPORU</b>" << GET_REPORT_TOP_RIGHT_HEADER() );
...
QString CARI_BAKIYE_RAPORU::GET_BODY()
{
    ...
    if ( m_tum_hesaplar_basildi EQ true ) {
         return NULL;
    }
    ...
    for ( ; ; ) {
        ...
        if ( query.SELECT() NE 0 ) {
            ...
            if ( M_CARI_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
                ...
                UNPACK_DOUBLE_ARRAY(query.VALUE(2).toString() , toplam_borc_array , K9_ARRAY_SIZE );
                UNPACK_DOUBLE_ARRAY(query.VALUE(3).toString() , toplam_alacak_array , K9_ARRAY_SIZE);
            }
            else if ( M_CARI_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY(query.VALUE(4).toString() , toplam_borc_array , K9_ARRAY_SIZE );
                UNPACK_DOUBLE_ARRAY(query.VALUE(5).toString() , toplam_alacak_array , K9_ARRAY_SIZE);
            }
        }
