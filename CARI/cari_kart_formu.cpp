...
void CARI_KART_FORMU::SETUP_FORM()
{
    ...
    if ( K9_KULLANICI_CARI_HAREKETLERINI_GOREBILIR_MI() EQ 0 ) {
...
int CARI_KART_FORMU::GET_RECORD ( int fis_id )
{
    ...
    m_ui->textEdit_gruplar->setText ( GRP_KAYDIN_GRUPLARINI_BUL ( K9_PROGRAMI , CARI_MODULU, fis_id, &m_grup_idleri, new QStringList, new QStringList ) );
...
int CARI_KART_FORMU::ADD_RECORD()
{
    ...
    cari_hesap_bilgileri.program_id             = K9_PROGRAMI;
    ...
    GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI,CARI_MODULU , fis_id , &m_grup_idleri);
...
{
    Q_UNUSED ( fis_id );
    if ( m_modul_id NE CARI_MODULU AND m_program_id NE K9_PROGRAMI ) {
...
void CARI_KART_FORMU::DELETE_RECORD ( int fis_id )
{
    ...
    GRP_KAYDIN_GRUPLARINI_SIL(K9_PROGRAMI,CARI_MODULU , fis_id);
    m_muh_hesap_id = 0;
}
...
{
   return OPEN_CARI_KART_ARAMA ( "", m_cari_kart_turu , this, 0, K9_PROGRAMI, CARI_MODULU );// SAHIS + FIRMA
}
...
{
    return   CARI_FIND_FIRST_RECORD( m_cari_kart_turu, K9_PROGRAMI, CARI_MODULU );
}
...
{
    return   CARI_FIND_LAST_RECORD( m_cari_kart_turu, K9_PROGRAMI, CARI_MODULU );
}
...
{
    return CARI_FIND_PREV_RECORD( m_ui->lineedit_cari_hesap_kodu->text(), m_cari_kart_turu, K9_PROGRAMI, CARI_MODULU );
}
...
{
    return CARI_FIND_NEXT_RECORD( m_ui->lineedit_cari_hesap_kodu->text(), m_cari_kart_turu, K9_PROGRAMI, CARI_MODULU );
}
...
void CARI_KART_FORMU::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_adres_id  )
{
    if ( p_button EQ m_ui->push_button_not_defteri) {
        OPEN_CARI_NOT_DEFTERI( p_adres_id, CARI_MODULU, K9_PROGRAMI, this );
    }
...
int CARI_KART_FORMU::CHANGER_BUTTON_CLICKED(QAbstractButton * button)
{
    if ( button EQ m_ui->push_button_gruplar ) {
        if ( OPEN_GRUP_SECIM ( K9_PROGRAMI,CARI_MODULU , &m_grup_idleri, DB, this ) EQ true ) {
...
/**************************************************************************************
                   CARI_KART_FORMU::SHOW_REKLAM
***************************************************************************************/

void CARI_KART_FORMU::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("CARI_KART_FORMU");
}
