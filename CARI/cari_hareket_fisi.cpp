...
void CARI_HAREKET_FISI::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM("CARI-HAREKET");
    ...
    if ( m_modul_id EQ CARI_MODULU ) {
        if ( m_fis_turu EQ ENUM_CARI_HAREKET_FISI AND K9_KULLANICI_ENT_BILGILERINI_DEGISTIREBILIR_MI() EQ 1 ) {
...
{    
    ...
    f_query.PREPARE_SELECT ( "car_fisler",
                             ...
                             "doviz_tutar_bilgileri, program_id, modul_id, kaydedilecek_sirket, "
    ...
    M_FIS->kaydedilecek_sirket          = f_query.VALUE ( "kaydedilecek_sirket"   ).toInt();
...
int CARI_HAREKET_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
    ...
    else if ( object EQ m_ui->limitedTextEdit_aciklama ) {
...
int CARI_HAREKET_FISI::FIND_FIS_RECORD()
{
    ...
    if( m_modul_id EQ 0 ) {
        query_str.append( " AND program_id != " + QString::number( K9_PROGRAMI ) );
    }
    else {
...
int CARI_HAREKET_FISI::FIND_FIRST_FIS_RECORD()
{
    ...
    if ( m_modul_id NE 0 ) {
        ...
        select_query.SET_VALUE(":program_id" , K9_PROGRAMI);
    }

    if ( kaydedilecek_sirket > -1 ) {
...
int  CARI_HAREKET_FISI::CHANGER_BUTTON_CLICKED ( QAbstractButton * p_button )
{
    ...
    if ( p_button EQ m_ui->button_ent_detaylari ) {
        ...
        int muh_fis_no = K9_GET_FIS_NO(MUHASEBE_MODULU , M_FIS->muh_fis_id);
