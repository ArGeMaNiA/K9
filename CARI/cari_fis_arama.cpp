...
void CARI_FIS_ARAMA::SETUP_FORM()
{
    ...
    K9_FILL_COMBOBOX_FIS_FILTRESI(m_ui->comboBoxfis_filtresi);

    m_entegre_filtresi = FIS_BIRINCI_SIRKET;

    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ 0 ) {
    ...
    m_ui->comboBoxfis_filtresi->setCurrentIndex(m_ui->comboBoxfis_filtresi->findText(K9_GET_FIS_FILTRESI_STRING(m_entegre_filtresi)));
...
int CARI_FIS_ARAMA::CHECK_VAR ( QObject * p_object )
{
    ...
    else if ( p_object EQ m_ui->comboBoxfis_filtresi ) {
        m_entegre_filtresi = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBoxfis_filtresi->currentText());
    }
