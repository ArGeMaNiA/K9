...
void CEK_SENET_FORMU::SETUP_FORM()
{
    ...
    if ( K9_KULLANICI_SIRKET_DEGISTIRE_BILIR_MI() EQ 1 ) {
...
void CEK_SENET_FORMU::CLEAR_FORM_MEMBERS()
{
   ...
   m_ui->lineedit_portfoy_no->setText("*");
    ...
    M_CEK_SENET->kaydedilecek_sirket = -1;
...
void CEK_SENET_FORMU::NEW_RECORD()
{
    ...
    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ 1 ) {
        m_entegre_olsun_mu = 1;
    }
    else {
        m_entegre_olsun_mu = 0;
    }

    FIND_AND_SET_COMBOBOX_TEXT(m_ui->comboBox_sirket , K9_GET_KAYDEDILECEK_SIRKET_STRING(m_entegre_olsun_mu));

    if ( K9_KULLANICI_SIRKET_DEGISTIRE_BILIR_MI() EQ 0 ) {
...
int CEK_SENET_FORMU::GET_RECORD ( int cek_senet_id )
{
    ...
    M_CEK_SENET->kaydedilecek_sirket   = sql_query.VALUE("kaydedilecek_sirket").toInt();

    FIND_AND_SET_COMBOBOX_TEXT(m_ui->comboBox_sirket , K9_GET_KAYDEDILECEK_SIRKET_STRING(sql_query.VALUE("kaydedilecek_sirket").toInt()));

    m_entegre_olsun_mu = M_CEK_SENET->kaydedilecek_sirket;
...
{
    int entegre_filtresi = -1;
    if ( m_entegre_olsun_mu EQ 1 ) {
        entegre_filtresi = FIS_BIRINCI_SIRKET;
    }
    else {
        entegre_filtresi = FIS_HER_IKI_SIRKET;
    }

    if ( m_cek_senet_turu EQ ENUM_CEK ) {
        return OPEN_CEK_SENET_ARAMA ( ENUM_KENDI_CEKIMIZ,ENUM_CEK,false,-1,-1,entegre_filtresi,this );
    }
    return OPEN_CEK_SENET_ARAMA ( ENUM_KENDI_SENEDIMIZ,ENUM_SENET,false,-1,-1,entegre_filtresi,this );
}
...
int CEK_SENET_FORMU::CHECK_VAR ( QObject * object )
{
    ...
    if ( object EQ m_ui->combobox_banka_hesap_kodu  ) {
        ...
        m_entegre_olsun_mu = K9_GET_KAYDEDILECEK_SIRKET_ENUM(m_ui->comboBox_sirket->currentText());
    }
...
int CEK_SENET_FORMU::CHECK_ADD ()
{
    ...
    int value = DB->LOCK_ROW ( "sabit_deger_id", "k9_sabit_degerler", QString ( "sabit_deger_id = 1" ) );
    if ( value NE ADAK_OK ) {
        return ADAK_FAIL;
    }
    ...
    if ( QString ( m_ui->lineedit_portfoy_no->text().at(0)) NE "*" ) {
        ...
        if ( sql_query.SELECT() NE 0 ) {
            DB->UNLOCK_ROW ( "sabit_deger_id","k9_sabit_degerler", QString ( "sabit_deger_id = 1" ) );
...
int CEK_SENET_FORMU::ADD_RECORD ()
{
    ...
    M_CEK_SENET->kaydedilecek_sirket   = m_entegre_olsun_mu;
    ...
    K9_LOG_KAYDI_EKLE ( CEKSENET_MODULU , LOG_CEK_SENETLER , LOG_ISLEM_ADD , log_detaylari ) ;
...
int CEK_SENET_FORMU::CHECK_UPDATE ( int record_id )
{
    ...
    int lock_return_value = DB->LOCK_ROW (  "sabit_deger_id", "k9_sabit_degerler", QString ( "sabit_deger_id = 1" ));
...
void CEK_SENET_FORMU::UPDATE_RECORD (int cek_senet_id)
{
    ...
    K9_LOG_KAYDI_EKLE ( CEKSENET_MODULU , LOG_CEK_SENETLER , LOG_ISLEM_UPDATE , log_detaylari );
...
void CEK_SENET_FORMU::DELETE_RECORD ( int cek_senet_id )
{
    ...
    K9_LOG_KAYDI_EKLE ( CEKSENET_MODULU , LOG_CEK_SENETLER , LOG_ISLEM_DELETE , log_detaylari );
