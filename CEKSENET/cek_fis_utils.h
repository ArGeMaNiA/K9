...
//! ENTEGRASYON
bool    CEK_SENET_ENT_ICIN_MUH_HESAPLARINI_BELIRLE ( int bordro_satiri_id, int *borclu_hesap_id, int *alacakli_hesap_id, bool p_k9_onar_cagirdi, ADAK_SQL * P_GELEN_DB = NULL);
void    CEK_SENET_ENT_FISLERINI_OLUSTUR( CEK_BORDRO_STRUCT * P_BORDRO, CEK_BORDRO_SATIRI_STRUCT * P_BORDRO_SATIRI, int p_bordro_satiri_id , bool p_k9_onar_cagirdi, ADAK_SQL * P_GELEN_DB = NULL );
void    CEK_SENET_ENT_FIS_SATIRLARINI_SIL( CEK_BORDRO_STRUCT * P_BORDRO , int p_bordro_satiri_id);


int     CEK_GET_SENET_ENTEGRASYON_TURU ( int cek_senet_id, int modul_id, bool p_k9_onar_cagirdi = false, ADAK_SQL * P_GELEN_DB = NULL );
