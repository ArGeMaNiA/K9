...
int CEK_BORDROSU_EKLE ( CEK_BORDRO_STRUCT  *  P_BORDRO , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_INSERT ( "cek_bordrolar", "bordro_id",
                           ...
                           "musteri_cek_senedi_mi ,base_fis_id , kaydedilecek_sirket , muh_fis_id" );
    ...
    query.SET_VALUE      ( ":kaydedilecek_sirket"   , P_BORDRO->kaydedilecek_sirket    );
    ...
    if ( P_BORDRO->muh_fis_id EQ 0 AND P_BORDRO->modul_id EQ CEKSENET_MODULU ) {
        ...
        MUHASEBE_FISI->kaydedilecek_sirket = P_BORDRO->kaydedilecek_sirket;
                             ...
                             bool p_base_fis_satiri_eklenecek , int p_fis_satiri_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    bool k9_onar_cagirdi = !p_base_fis_satiri_eklenecek;
    if( modul_id EQ CARI_MODULU ) {
       k9_onar_cagirdi = p_base_fis_satiri_eklenecek;
    }
    ...
    if ( p_base_fis_satiri_eklenecek EQ true ) {
        ...
        query.SET_VALUE     ( ":kaydedilecek_sirket"     , P_BORDRO_SATIRI->kaydedilecek_sirket   );
        ...
        //! K9 ONARDAN GELDIGIN DE EKLEMEMESI GEREKIR.
    ...
    //! k9 0nardan gelmis ise iceride hareketler geriye alinacak
    ...
    if ( entegrasyon_fis_satiri_guncellenecek EQ true ) {
        ...
        if ( cek_senet_entegrasyon_turu EQ ENUM_MUHASEBE_ENTEGRASYONU ) {
            ...
            for ( int i = 0 ; i < muh_satirlar_list.size() ; i++ ) {
                ...
                MUHASEBE_FIS_SATIRI->kaydedilecek_sirket = P_BORDRO_SATIRI->kaydedilecek_sirket;
            ...
            CARI_FIS->kaydedilecek_sirket = P_BORDRO->kaydedilecek_sirket;
            ...
            for ( int i = 0 ; i < car_satirlar_list.size() ; i++ ) {
                ...
                CARI_FIS_SATIRI->kaydedilecek_sirket = P_BORDRO_SATIRI->kaydedilecek_sirket;
            ...
            for ( int i = 0 ; i < MUH_ROW_ID_LIST.size() ; i++ ) {
                ...
                MUH_FIS_SATIRI->kaydedilecek_sirket = P_BORDRO_SATIRI->kaydedilecek_sirket;
            ...
            BANKA_FISI->kaydedilecek_sirket = P_BORDRO->kaydedilecek_sirket;
            ...
            for ( int i = 0 ; i < BNK_ROW_ID_LIST.size() ; i++ ) {
                ...
                BNK_FIS_SATIRI->kaydedilecek_sirket = P_BORDRO_SATIRI->kaydedilecek_sirket;
...
void CEK_BORDRO_GUNCELLE ( CEK_BORDRO_STRUCT * P_BORDRO , int p_bordro_id )
{
    ...
    query.PREPARE_SELECT("cek_bordrolar",
                         ...
                         "program_id , modul_id,muh_fis_id,kaydedilecek_sirket ",
                         "bordro_id = :bordro_id");
    query.SET_VALUE(":bordro_id" , p_bordro_id);
    ...
    int old_entegre_olsun_mu  = query.VALUE(10).toInt();
    ...
    if ( old_entegre_olsun_mu NE P_BORDRO->kaydedilecek_sirket ) {
        fis_satirlari_guncellenecek = true;
    }
    ...
    if ( ent_fisleri_silinecek EQ true ) {
        ...
        if ( entegrasyon_turu EQ ENUM_CARI_ENTEGRASYON ) {
            ...
            if ( car_fis_id > 0 ) {
                ...
                CARI_FISI->kaydedilecek_sirket = P_BORDRO->kaydedilecek_sirket;
    ...
    if ( fis_satirlari_guncellenecek EQ true) {
        ...
        if ( query.SELECT() NE 0 ) {
            while ( query.NEXT() ) {
                ...
                s_query.SET_VALUE(":kaydedilecek_sirket"         , P_BORDRO->kaydedilecek_sirket);
...
void CEK_BORDRO_BILGILERI_OKU(CEK_BORDRO_STRUCT *P_BORDRO, int p_bordro_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("cek_bordrolar",
                         ...
                         "bordro_para_birimi_id , doviz_kuru , parite , base_fis_id, "
                         "kaydedilecek_sirket,muh_fis_id ",
                         "bordro_id = :bordro_id");
    query.SET_VALUE(":bordro_id" , p_bordro_id);
    ...
    P_BORDRO->kaydedilecek_sirket      = query.VALUE(16).toInt();
    P_BORDRO->muh_fis_id            = query.VALUE(17).toInt();
}
/**************************************************************************************
                             ...
                             int p_bordro_satiri_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    P_BORDRO_SATIRI->kaydedilecek_sirket      = query.VALUE(12).toInt();
}
...
void CEK_SENET_ENT_FISLERINI_OLUSTUR( CEK_BORDRO_STRUCT *P_BORDRO, CEK_BORDRO_SATIRI_STRUCT *P_BORDRO_SATIRI,
                                           int p_bordro_satiri_id, bool p_k9_onar_cagirdi, ADAK_SQL * P_GELEN_DB )

{
    ...
    if ( CEK_SENET_ENT_ICIN_MUH_HESAPLARINI_BELIRLE ( p_bordro_satiri_id, &borclu_hesap_id, &alacakli_hesap_id, p_k9_onar_cagirdi ,USE_DB ) EQ false ) {
        return;
    }
    ...
    int cek_senet_entegrasyon_turu = CEK_GET_SENET_ENTEGRASYON_TURU( P_BORDRO_SATIRI->cek_senet_id, P_BORDRO->modul_id, p_k9_onar_cagirdi, USE_DB );
...
int CEK_GET_SENET_ENTEGRASYON_TURU ( int cek_senet_id , int modul_id , bool p_k9_onar_cagirdi, ADAK_SQL * P_GELEN_DB)
{
    ADAK_SQL * USE_DB;
...
bool CEK_SENET_ENT_ICIN_MUH_HESAPLARINI_BELIRLE ( int bordro_satiri_id, int *borclu_hesap_id, int *alacakli_hesap_id, bool p_k9_onar_cagirdi, ADAK_SQL * P_GELEN_DB )
{
...
void CEK_CLEAR_BORDRO_SATIRI_STRUCT(CEK_BORDRO_SATIRI_STRUCT *P_BORDRO_SATIRI)
{
    ...
    P_BORDRO_SATIRI->kaydedilecek_sirket      = -1;
