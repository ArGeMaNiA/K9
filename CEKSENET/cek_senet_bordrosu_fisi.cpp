...
{    
    ...
    M_ENTEGRASYON_DETAYLARI = new K9_ENT_DETAYLARI_STRUCT;
    ...
    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ SIRKET_1 ) {
        ...
        M_BORDRO->kaydedilecek_sirket = SIRKET_1;
    }
    else {
        m_entegre_filtresi = FIS_HER_IKI_SIRKET;
        M_BORDRO->kaydedilecek_sirket = SIRKET_2;
    }

    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket, K9_GET_KAYDEDILECEK_SIRKET_STRING( M_BORDRO->kaydedilecek_sirket ));
...
void CEK_SENET_BORDROSU_FISI::NEW_FIS_RECORD()
{
    ...
    M_BORDRO->kaydedilecek_sirket = K9_KULLANICININ_CALISTIGI_SIRKET();
...
int CEK_SENET_BORDROSU_FISI::GET_FIS_RECORD ( int bordro_id )
{
    ...
    sql_query.PREPARE_SELECT ( "cek_bordrolar",
                               ...
                               "kaydedilecek_sirket,bordro_islem_turu,musteri_cek_senedi_mi,cek_bordrosu_mu ",
                               "bordro_id = :bordro_id" );
    ...
    M_BORDRO->kaydedilecek_sirket   = sql_query.VALUE(12).toInt();
...
int CEK_SENET_BORDROSU_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
      if ( object EQ m_ui->searchEdit_cari_hesap_kodu ) {
        ...
        M_BORDRO->kaydedilecek_sirket = K9_GET_KAYDEDILECEK_SIRKET_ENUM( m_ui->comboBox_sirket->currentText() );
    }
...
int CEK_SENET_BORDROSU_FISI::ADD_FIS_RECORD()
{
    ...
    M_BORDRO->program_id            = K9_PROGRAMI;
...
int CEK_SENET_BORDROSU_FISI::FIND_FIRST_FIS_RECORD()
{
   ...
   if ( m_entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
       sql_query += QString ( " AND kaydedilecek_sirket = 1 ");
   }
   else if ( m_entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
       sql_query += QString ( " AND kaydedilecek_sirket = 0 ");
   }
...
int CEK_SENET_BORDROSU_FISI::FIND_PREV_FIS_RECORD()
{
    ...
    if ( m_entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        query += QString (" AND kaydedilecek_sirket = 1 ");
    }
    else if ( m_entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
        query += QString (" AND kaydedilecek_sirket = 0 ");
    }
...
void CEK_SENET_BORDROSU_FISI::ADD_LINE ( int bordro_id, int row_number)
{
    ...
    M_BORDRO_SATIRI->kaydedilecek_sirket = M_BORDRO->kaydedilecek_sirket;
...
void CEK_SENET_BORDROSU_FISI::LINE_SEARCH_EDIT_CLICKED(int row_number,QWidget * widget,QLineEdit * line_edit)
{
    ...
    int cek_id = OPEN_CEK_SENET_ARAMA ( m_bordro_islem_turu, m_cek_mi_senet_mi, m_musteri_cek_senedi_mi,
                                        hesap_id,m_bordro_para_birimi_id,m_entegre_filtresi, this );
        ...
        if ( record_id > 0 ) {
            ...
            if ( entegrasyon_turu EQ ENUM_MUHASEBE_ENTEGRASYONU ) {
                ...
                int muh_fis_id = MUH_GET_ENT_FIS_ID(M_BORDRO->program_id , M_BORDRO->modul_id,
                                                    record_id);
                M_ENTEGRASYON_DETAYLARI->muh_fis_no = K9_GET_FIS_NO(MUHASEBE_MODULU , muh_fis_id);
            }
                ...
                M_ENTEGRASYON_DETAYLARI->cari_fis_no = K9_GET_FIS_NO(CARI_MODULU , cari_fis_id);
                ...
                M_ENTEGRASYON_DETAYLARI->banka_fis_no << QVariant(K9_GET_FIS_NO(BANKA_MODULU , bnk_fis_id)).toString();

            }
        }
        OPEN_K9_ENT_DETAYLARI_BATCH ( M_ENTEGRASYON_DETAYLARI , false,this);
...
void CEK_SENET_BORDROSU_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_record_id )
{
    if ( p_button EQ m_ui->button_yazdir ) {
        ...
        K9_URETILEN_FIS_SEC( K9_PROGRAMI, CEKSENET_MODULU, p_record_id, this );
    }
...
int CEK_SENET_BORDROSU_FISI::LEAVE_RECORD()
{
    if ( m_log_kaydi_eklenmeli_mi EQ true ) {
        ...
        if ( m_kayit_eklendi EQ true ) {
            K9_LOG_KAYDI_EKLE ( CEKSENET_MODULU,LOG_CEK_BORDROSU,LOG_ISLEM_ADD,m_log_detaylari );
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE( CEKSENET_MODULU,LOG_CEK_BORDROSU,LOG_ISLEM_DELETE,m_log_detaylari );
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
             K9_LOG_KAYDI_EKLE ( CEKSENET_MODULU,LOG_CEK_BORDROSU,LOG_ISLEM_UPDATE,m_log_detaylari );
        }
...
void CEK_SENET_BORDROSU_FISI::SHOW_REKLAM()
{
    m_ui->reklamViewer->SHOW_REKLAM("CEK-SENET-BORDROSU-FISI");
}
