...
int MUH_HESAP_EKLEME_FISI::GET_FIS_RECORD ( int p_fis_id )
{
    ...
    while ( query.NEXT() EQ true ) {
        ...
        lineEdit_gruplar->setText(GRP_KAYDIN_GRUPLARINI_BUL(K9_PROGRAMI,MUHASEBE_MODULU,query.VALUE(0).toInt(),&grup_idleri,new QStringList(), new QStringList()));
...
int MUH_HESAP_EKLEME_FISI::ADD_FIS_RECORD()
{
    ...
    m_muh_hesap_plani->m_added_record << m_ui->line_edit_hesap_kodu->text()
                                      ...
                                      << "0"
                                      << MUH_GET_BORC_ARRAY  ( p_hesap_id , m_muh_hesap_plani->m_entegrasyon_durumu)+" "
                                      << MUH_GET_ALACAK_ARRAY( p_hesap_id , m_muh_hesap_plani->m_entegrasyon_durumu)+" "
    ...
    K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_HESAP_PLANI , LOG_ISLEM_ADD , log_detaylari );
...
void MUH_HESAP_EKLEME_FISI::UPDATE_FIS_RECORD( int p_fis_id )
{
    ...
    m_muh_hesap_plani->m_updated_record << m_ui->line_edit_hesap_kodu->text() <<  m_ui->lineEdit_hesap_ismi->text()
                       << QVariant(alt_hesap_sayisi).toString() << MUH_GET_ALACAK_ARRAY( p_fis_id , m_muh_hesap_plani->m_entegrasyon_durumu )
                       +" " << MUH_GET_BORC_ARRAY( p_fis_id , m_muh_hesap_plani->m_entegrasyon_durumu)+" "<<  QVariant ( p_fis_id ).toString();
    ...
    K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU,LOG_MUH_HESAP_PLANI,LOG_ISLEM_UPDATE,log_detaylari );
...
void MUH_HESAP_EKLEME_FISI::ADD_LINE ( int fis_id, int row_number )
{
    ...
    if ( m_ana_hesap_aktarilmalimi EQ true ) {
        ...
        sql_query.PREPARE_SELECT("muh_hesaplar" , "parent_id, hesap_seviyesi ,hesap_kodu , hesap_ismi , tam_hesap_kodu, "
                                 "toplam_borc_array,toplam_alacak_array,ent_borc_array,ent_alacak_array, "
                                 "ent_olmayan_borc_array,ent_olmayan_alacak_array" , "hesap_id = :hesap_id");
        ...
        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(5).toString(),toplam_borc_array   ,K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(6).toString(),toplam_alacak_array ,K9_ARRAY_SIZE );

        double ent_borc_array     [ K9_ARRAY_SIZE ];
        double ent_alacak_array   [ K9_ARRAY_SIZE ];

        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(7).toString(),ent_borc_array   ,K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(8).toString(),ent_alacak_array ,K9_ARRAY_SIZE );

        double ent_olmayan_borc   [ K9_ARRAY_SIZE ];
        double ent_olmayan_alacak [ K9_ARRAY_SIZE ];

        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(9).toString() ,ent_olmayan_borc   ,K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(10).toString(),ent_olmayan_alacak ,K9_ARRAY_SIZE );
        ...
        update_query.SET_VALUE (  ":ent_olmayan_borc_array"     , PACK_DOUBLE_ARRAY ( ent_olmayan_borc,K9_ARRAY_SIZE )    );
        update_query.SET_VALUE (  ":ent_olmayan_alacak_array"   , PACK_DOUBLE_ARRAY ( ent_olmayan_alacak,K9_ARRAY_SIZE )  );
        update_query.UPDATE();
    ...
    if ( m_ui->tablewidget_alt_hesaplar->item(row_number,GRUP_IDLERI) NE NULL ) {
        ...
        GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , MUHASEBE_MODULU , yeni_hesap_id , &grup_idleri  );
    }

    m_ana_hesap_aktarilmalimi = false;

    m_muh_hesap_plani->m_added_record << yeni_alt_hesap_kodu << yeni_alt_hesap_ismi << "0" <<
            MUH_GET_BORC_ARRAY( yeni_hesap_parent_id , m_muh_hesap_plani->m_entegrasyon_durumu) << " " << MUH_GET_ALACAK_ARRAY( yeni_hesap_parent_id, m_muh_hesap_plani->m_entegrasyon_durumu) << " " << QVariant ( yeni_hesap_parent_id ).toString();

    m_muh_hesap_plani->m_updated_record << m_ana_hesap_kodu << m_ui->lineEdit_hesap_ismi->text() << QVariant ( alt_hesap_sayisi ).toString() << MUH_GET_BORC_ARRAY(yeni_hesap_id , m_muh_hesap_plani->m_entegrasyon_durumu) << " " << MUH_GET_ALACAK_ARRAY(yeni_hesap_id, m_muh_hesap_plani->m_entegrasyon_durumu)+ " " << QVariant(yeni_hesap_parent_id).toString();
...
void MUH_HESAP_EKLEME_FISI::UPDATE_LINE ( int fis_id, int row_number )
{
    ...
    m_muh_hesap_plani->m_updated_line <<   hesap_kodu << hesap_ismi
                       <<QVariant ( sql_query.SELECT() ).toString() << MUH_GET_BORC_ARRAY( hesap_id, m_muh_hesap_plani->m_entegrasyon_durumu )
                       <<" " << MUH_GET_ALACAK_ARRAY( hesap_id , m_muh_hesap_plani->m_entegrasyon_durumu) << " " << QVariant ( hesap_id ).toString();
    ...
    if ( m_ui->tablewidget_alt_hesaplar->item(row_number,GRUP_IDLERI) NE NULL ) {
        ...
        GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , MUHASEBE_MODULU , hesap_id , &grup_idleri);
    }
...
void MUH_HESAP_EKLEME_FISI::DELETE_LINE ( int fis_id, int row_number )
{
    ...
    if ( sql_query.SELECT() > 0 ) {
        ...
        if ( sql_query.SELECT() > 0 ) {
            ...
            K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_HESAP_PLANI , LOG_ISLEM_DELETE , log_detaylari );
        }
    }
...
int MUH_HESAP_EKLEME_FISI::LINE_CHANGER_BUTTON_CLICKED(QAbstractButton *button, int row_number)
{
    ...
    if ( button EQ button_grup_secimi ) {
        ...
        if ( OPEN_GRUP_SECIM ( K9_PROGRAMI , MUHASEBE_MODULU , &idler,DB, this ) EQ true ) {
