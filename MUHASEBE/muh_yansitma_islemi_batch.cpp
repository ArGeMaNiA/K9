...
void MUH_YANSITMA_ISLEMI_BATCH::SETUP_FORM()
{
    ...
    m_ui->comboBox_sirket_turu->setCurrentIndex(m_ui->comboBox_sirket_turu->findText(K9_GET_KAYDEDILECEK_SIRKET_STRING( K9_KULLANICININ_CALISTIGI_SIRKET() )));
...
void MUH_YANSITMA_ISLEMI_BATCH::RUN_BATCH ()
{
    ...
    MUHASEBE_FISI->kaydedilecek_sirket       = K9_GET_KAYDEDILECEK_SIRKET_ENUM(m_ui->comboBox_sirket_turu->currentText());
    ...
    while ( sql_query.NEXT() EQ true ) {
        ...
        query.PREPARE_SELECT("muh_hesaplar" , "ent_borc_array , ent_alacak_array , ent_olmayan_borc_array,"
        ...
        if ( query.SELECT() NE 0 ) {
            ...
            if ( K9_GET_KAYDEDILECEK_SIRKET_ENUM ( m_ui->comboBox_sirket_turu->currentText() ) EQ SIRKET_1 ) {
                UNPACK_DOUBLE_ARRAY(query.VALUE(0).toString() , BORC_BAKIYESI , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(1).toString() , ALACAK_BAKIYESI , K9_ARRAY_SIZE);
            }
            else {
                UNPACK_DOUBLE_ARRAY(query.VALUE(2).toString() , BORC_BAKIYESI , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(3).toString() , ALACAK_BAKIYESI , K9_ARRAY_SIZE);
            }
        }
        ...
        MUH_FIS_SATIRI->kaydedilecek_sirket       = MUHASEBE_FISI->kaydedilecek_sirket;
