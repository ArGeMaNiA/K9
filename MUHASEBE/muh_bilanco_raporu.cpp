...
QString BILANCO_RAPORU::GET_HEADER()
{
    ...
    ADD_ROW_TO_TABLE     ( QStringList()  << K9_GET_FIRMA_ISMI() << "<b>BİLANÇO TABLOSU</b>" << GET_REPORT_TOP_RIGHT_HEADER() );
...
void BILANCO_RAPORU::GRUBUN_HESAPLARINI_BILANCO_HESAPLARINA_EKLE ( int grup_indisi,QString grup_kodu )
{
    ...
    sql_query.PREPARE_SELECT("muh_hesaplar" , "hesap_id,tam_hesap_kodu,hesap_ismi,toplam_borc_array,"
                             "toplam_alacak_array , ent_borc_array , ent_alacak_array ,"
                             "ent_olmayan_borc_array , ent_olmayan_alacak_array" , "hesap_seviyesi = :hesap_seviyesi");
    ...
    if ( sql_query.SELECT() > 0 ) {
        ...
        while ( sql_query.NEXT() ) {
            ...
            if ( M_MUH_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(3).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(4).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
            }
            else if ( M_MUH_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(5).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(6).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
            }
            else if ( M_MUH_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(7).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(8).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
            }
            ...
            if ( grup_kodu EQ "59") {
                ...
                MUH_GET_FROM_GELIR_TABLOSU_DONEM_NET_KARI_ZARARI(donem_kari , donem_zarari,
                                                             M_MUH_RV->entegre_filtresi ,bts_gun_no);
...
void BILANCO_RAPORU::BILANCOYA_FARK_BAKIYESINI_EKLE(int indis_no , bool aktif_varlik_satiri_mi)
{
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "toplam_borc_array , toplam_alacak_array , ent_borc_array , ent_alacak_array,"
                         "ent_olmayan_borc_array , ent_olmayan_alacak_array" , "tam_hesap_kodu >= :tam_hesap_kodu_1 AND "
    ...
    while ( query.NEXT() ) {
        ...
        if ( M_MUH_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_borc_array").toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_alacak_array").toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
        }
        else if ( M_MUH_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_borc_array").toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_alacak_array").toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
        }
        else if ( M_MUH_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_olmayan_borc_array").toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_olmayan_alacak_array").toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
        }
