...
void MUH_MIZAN_RAPORU_BATCH::SETUP_FORM()
{
    ...
    sql_query.PREPARE_SELECT("k9_kullanici_yetkileri" , "mizan_nakli_yekun" ,
...
int MUH_MIZAN_RAPORU_BATCH::CHECK_VAR ( QObject * object )
{
    ...
    if ( object EQ m_ui->adakDate_baslangic_tarihi OR object EQ m_ui->adakDate_bts_tarihi ) {
        ...
        OPEN_GRUP_SECIM(K9_PROGRAMI,MUHASEBE_MODULU,&m_grup_idleri,DB,this,true);
...
int MUH_MIZAN_RAPORU_BATCH::CHECK_RUN()
{
    ...
    if ( K9_MALI_YIL_TARIH_ARALIGI_KONTROLU( m_ui->adakDate_baslangic_tarihi, m_ui->adakDate_bts_tarihi ) NE ADAK_OK ) {
        return ADAK_FAIL;
    }
...
void MUH_MIZAN_RAPORU_BATCH::RUN_BATCH()
{
    ...
    if ( MUH_RV->calismamis_hesaplar EQ true ) {
        if ( MUH_RV->nakli_yekun EQ true ) {
    ...
    if ( MUH_RV->bakiyesiz_hesaplar EQ true ) {
        if ( MUH_RV->calismamis_hesaplar EQ true OR MUH_RV->nakli_yekun EQ true ) {
            log_detaylari.append ( " , " );
        }
