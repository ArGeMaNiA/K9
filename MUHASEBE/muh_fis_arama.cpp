...
void MUH_FIS_ARAMA::SETUP_FORM()
{
    ...
    m_ui->frame_filtre->setVisible( false );

    if( K9_KULLANICI_SIRKET_DEGISTIRE_BILIR_MI() EQ 1 ){
        m_ui->frame_filtre->setVisible( true );
    }

    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket, K9_GET_FIS_FILTRESI_STRING( m_ent_filtre_turu ) );    
...
void MUH_FIS_ARAMA::SEARCH()
{
    ...
    int entegre_filtresi = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBox_sirket->currentText());

    int kaydedilecek_sirket = -1;

    if( K9_KULLANICI_SIRKET_DEGISTIRE_BILIR_MI() EQ 1 ) {
        switch ( entegre_filtresi ){
        case FIS_BIRINCI_SIRKET: {

                kaydedilecek_sirket = 1;
                sql_query.AND_EKLE  ( "kaydedilecek_sirket = :kaydedilecek_sirket " );
                sql_query.SET_VALUE(":kaydedilecek_sirket" , kaydedilecek_sirket);
