...
QString HESAP_EKSTRESI_RAPORU::GET_HEADER()
{
    ...
    ADD_ROW_TO_TABLE      ( QStringList() << K9_GET_FIRMA_ISMI() << "<b>HESAP EKSTRESİ</b>" << GET_REPORT_TOP_RIGHT_HEADER() );
...
QString HESAP_EKSTRESI_RAPORU::GET_BODY()
{
    ...
    if ( m_raporlanacak_kayit_var_mi EQ false ) {
        ...
        if ( M_MUH_RV->nakli_yekun ) {
            ...
            if ( sql_query.SELECT() > 0) {
                ...
                if ( M_MUH_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(0).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(1).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
                }
                else if ( M_MUH_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(2).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(3).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
                }
                else if ( M_MUH_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(4).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
                    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(5).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
                }

                hesap_kodu_adi = "<p align=center style=\"font-size:85%; font-weight:bold \">" +
                                 sql_query.VALUE(6).toString() + " " +
                                 sql_query.VALUE(7).toString() + "</p>";
            }
    ...
    for ( ; ; ) {
        ...
        UNPACK_DOUBLE_ARRAY ( M_MUH_HESAPLAR [ m_last_hesap_index ].borc_bakiyesi  ,borc_bakiyesi  , K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( M_MUH_HESAPLAR [ m_last_hesap_index ].alacak_bakiyesi,alacak_bakiyesi, K9_ARRAY_SIZE );
...
bool HESAP_EKSTRESI_RAPORU::FILL_STRUCT()
{
    ...
    QString fields2 = " toplam_borc_array, toplam_alacak_array, ent_borc_array, ent_alacak_array,"
            ...
            for ( int i = liste_indis_no ; i < M_TUM_HESAPLAR_LIST.size() ; i++ ) {
                ...
                if ( sorgu_hesap_kodu EQ liste_hesap_kodu ) {
                    ...
                    if ( M_MUH_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
                        ...
                        borc_bakiyesi   = sql_query.VALUE(9).toString();
                        alacak_bakiyesi = sql_query.VALUE(10).toString();
                    }
                    else if ( M_MUH_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
                        borc_bakiyesi   = sql_query.VALUE(11).toString();
                        alacak_bakiyesi = sql_query.VALUE(12).toString();
                    }
    ...
    if ( M_MUH_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        sql_query.AND_EKLE  ( "muh_fis_satirlari.kaydedilecek_sirket = :kaydedilecek_sirket" );
...
void HESAP_EKSTRESI_RAPORU::FILL_TUM_HESAPLAR_LIST()
{
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "hesap_id,tam_hesap_kodu,hesap_ismi,parent_id,hesap_seviyesi,tali_hesap, "
                         ...
                         "ent_olmayan_alacak_array" , where_str);
    ...
    while ( query.NEXT() ) {
        ...
        if ( M_MUH_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
            ...
            borc_bakiyesi_array     = query.VALUE(8).toString();
            alacak_bakiyesi_array   = query.VALUE(9).toString();
        }
        else if ( M_MUH_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
            borc_bakiyesi_array     = query.VALUE(10).toString();
            alacak_bakiyesi_array   = query.VALUE(11).toString();
        }
