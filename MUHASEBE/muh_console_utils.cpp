...
void MUH_HESABININ_BAKIYELERINI_ARTTIR ( int hesap_id, int gun_no,
                                              double borc_tutari, double alacak_tutari, int kaydedilecek_sirket, ADAK_SQL * P_DB )
{
    if  ( hesap_id EQ 0 ) {
        return;
    }
    ...
    double ent_borc_array             [ K9_ARRAY_SIZE ];
    double ent_alacak_array           [ K9_ARRAY_SIZE ];

    double ent_olmayan_borc_array     [ K9_ARRAY_SIZE ];
    double ent_olmayan_alacak_array   [ K9_ARRAY_SIZE ];
    ...
    double ent_olmayan_borc;
    double ent_olmayan_alacak;
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "toplam_borc_array , toplam_alacak_array , ent_borc_array,"
                         "ent_alacak_array,ent_olmayan_borc_array , ent_olmayan_alacak_array, "
                         "parent_id" , "hesap_id =:hesap_id");
    ...
    UNPACK_DOUBLE_ARRAY(query.VALUE("toplam_borc_array").toString() , toplam_borc_array , K9_ARRAY_SIZE);

    UNPACK_DOUBLE_ARRAY(query.VALUE("ent_borc_array").toString() , ent_borc_array , K9_ARRAY_SIZE);

    UNPACK_DOUBLE_ARRAY(query.VALUE("ent_olmayan_borc_array").toString() , ent_olmayan_borc_array , K9_ARRAY_SIZE);

    UNPACK_DOUBLE_ARRAY(query.VALUE("toplam_alacak_array").toString() , toplam_alacak_array , K9_ARRAY_SIZE);

    UNPACK_DOUBLE_ARRAY(query.VALUE("ent_alacak_array").toString() , ent_alacak_array , K9_ARRAY_SIZE);

    UNPACK_DOUBLE_ARRAY(query.VALUE("ent_olmayan_alacak_array").toString() , ent_olmayan_alacak_array , K9_ARRAY_SIZE);
    ...
    for ( int i = gun_no ; i < K9_ARRAY_SIZE ; i++ ) {
        ...
        if ( borc_tutari NE 0.00 ) {
            ...
            if ( kaydedilecek_sirket EQ SIRKET_1 ){
                ent_borc           = ent_borc_array[i] + borc_tutari;
                ent_borc_array[i]  = ROUND ( ent_borc  );
            }
            else {
                ent_olmayan_borc          = ent_olmayan_borc_array[i] + borc_tutari;
                ent_olmayan_borc_array[i] = ROUND ( ent_olmayan_borc );
            }
        }
        if ( alacak_tutari NE 0.00 ) {
            ...
            if ( kaydedilecek_sirket EQ SIRKET_1 ) {
                ent_alacak           = ent_alacak_array[i] + alacak_tutari;
                ent_alacak_array [i] = ROUND( ent_alacak   );
            }
            else {
                ent_olmayan_alacak          = ent_olmayan_alacak_array[i] + alacak_tutari;
                ent_olmayan_alacak_array[i] = ROUND ( ent_olmayan_alacak );
            }
        }
    }
    ...
    query.PREPARE_UPDATE("muh_hesaplar","hesap_id" ,
                         "toplam_borc_array , ent_borc_array ,"
                         "ent_olmayan_borc_array  , toplam_alacak_array ,"
                         "ent_alacak_array , ent_olmayan_alacak_array ",
                         "hesap_id = :hesap_id");
    query.SET_VALUE(":toplam_borc_array"        , PACK_DOUBLE_ARRAY(toplam_borc_array        , K9_ARRAY_SIZE));
    query.SET_VALUE(":ent_borc_array"           , PACK_DOUBLE_ARRAY(ent_borc_array           , K9_ARRAY_SIZE));
    query.SET_VALUE(":ent_olmayan_borc_array"   , PACK_DOUBLE_ARRAY(ent_olmayan_borc_array   , K9_ARRAY_SIZE));
    query.SET_VALUE(":toplam_alacak_array"      , PACK_DOUBLE_ARRAY(toplam_alacak_array      , K9_ARRAY_SIZE));
    query.SET_VALUE(":ent_alacak_array"         , PACK_DOUBLE_ARRAY(ent_alacak_array         , K9_ARRAY_SIZE));
    query.SET_VALUE(":ent_olmayan_alacak_array" , PACK_DOUBLE_ARRAY(ent_olmayan_alacak_array , K9_ARRAY_SIZE));
    query.SET_VALUE(":hesap_id"                 , hesap_id);
    query.UPDATE();
    ...
    if ( parent_id NE 0 ) {
        ...
        MUH_HESABININ_BAKIYELERINI_ARTTIR ( parent_id ,gun_no,borc_tutari ,
                                                 alacak_tutari, kaydedilecek_sirket, USE_DB );
    }
...
void MUH_HESABININ_BAKIYELERINI_AZALT ( int hesap_id , int gun_no , double borc_tutari ,
                                             double alacak_tutari,  int kaydedilecek_sirket, ADAK_SQL * P_DB )
{
    if ( hesap_id EQ 0 ) {
        return;
    }
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "toplam_borc_array , toplam_alacak_array ,"
                         ...
                         "ent_olmayan_alacak_array,parent_id" , "hesap_id = :hesap_id");
    ...
    if ( parent_id NE 0 ) {
        ...
        MUH_HESABININ_BAKIYELERINI_AZALT ( parent_id ,gun_no,borc_tutari ,
                                                alacak_tutari, kaydedilecek_sirket, USE_DB );
    }
...
QStringList MUH_GET_HESAPLAR ( int p_entegre_filtresi, const QString p_baslangic_hesabi, const QString p_bitis_hesabi )
{
    ...
    query_muh_hesaplar.PREPARE_SELECT( "muh_hesaplar" , "hesap_id, tam_hesap_kodu, hesap_ismi, parent_id, hesap_seviyesi, "
                                       "tali_hesap, toplam_borc_array, toplam_alacak_array, "
                                       "ent_borc_array, ent_alacak_array, ent_olmayan_borc_array ,ent_olmayan_alacak_array",
                                       where_str);
    ...
    while ( query_muh_hesaplar.NEXT() EQ true ) {
        ...
        if ( p_entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
        ...
        else if ( p_entegre_filtresi EQ FIS_BIRINCI_SIRKET ){
            hesap_borc_bakiye   = query_muh_hesaplar.VALUE("ent_borc_array").toString();
            hesap_alacak_bakiye = query_muh_hesaplar.VALUE("ent_alacak_array").toString();
        }
        else if ( p_entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
            hesap_borc_bakiye   = query_muh_hesaplar.VALUE("ent_olmayan_borc_array").toString();
            hesap_alacak_bakiye = query_muh_hesaplar.VALUE("ent_olmayan_alacak_array").toString();
        }
...
QString MUH_GET_BORC_ARRAY ( int hesap_id  , int fis_filtresi )
{
    SQL_QUERY sql_query ( DB );

    double  BORC_BAKIYESI [ K9_ARRAY_SIZE ];

    if (fis_filtresi EQ FIS_HER_IKI_SIRKET) {
        sql_query.PREPARE_SELECT ( "muh_hesaplar","toplam_borc_array", "hesap_id=:hesap_id" );
    }
    else if (fis_filtresi EQ FIS_BIRINCI_SIRKET) {
        sql_query.PREPARE_SELECT ( "muh_hesaplar","ent_borc_array","hesap_id=:hesap_id" );
    }
    else if ( fis_filtresi EQ FIS_IKINCI_SIRKET ){
        sql_query.PREPARE_SELECT ( "muh_hesaplar","ent_olmayan_borc_array","hesap_id=:hesap_id" );
    }
    ...
    UNPACK_DOUBLE_ARRAY( sql_query.VALUE(0).toString(), BORC_BAKIYESI, K9_ARRAY_SIZE );

    return ( VIRGUL_EKLE ( QVariant( BORC_BAKIYESI[K9_SON_BAKIYE]).toString() ) );
}
...
QString MUH_GET_ALACAK_ARRAY ( int hesap_id , int fis_filtresi )
{
    SQL_QUERY sql_query ( DB );
    ...
    if (fis_filtresi EQ FIS_HER_IKI_SIRKET) {
        ...
        sql_query.PREPARE_SELECT ( "muh_hesaplar","ent_alacak_array","hesap_id=:hesap_id" );
    }
    else if ( fis_filtresi EQ FIS_IKINCI_SIRKET ){
        sql_query.PREPARE_SELECT ( "muh_hesaplar","ent_olmayan_alacak_array","hesap_id=:hesap_id" );
    }
    ...
    UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(0).toString(), ALACAK_BAKIYESI, K9_ARRAY_SIZE );

    return ( VIRGUL_EKLE ( QVariant ( ALACAK_BAKIYESI[K9_SON_BAKIYE ] ).toString() ) );
}
...
void MUH_GET_FROM_GELIR_TABLOSU_DONEM_NET_KARI_ZARARI(double &donem_kari, double &donem_zarari , int entegre_filtresi,int gun_no)
{
    donem_kari   = 0;
    donem_zarari = 0;
    ...
    for ( int i = 0 ; i < gelir_tablosu_hesap_gruplari_list.size() ; i++ ) {
        ...
        while ( query.NEXT() ) {
            ...
            if ( entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY(query.VALUE(0).toString() , borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(1).toString() , alacak_array , K9_ARRAY_SIZE);
            }
            else if ( entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY(query.VALUE(2).toString() , borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(3).toString() , alacak_array , K9_ARRAY_SIZE);
            }
            else if ( entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
                UNPACK_DOUBLE_ARRAY(query.VALUE(4).toString() , borc_array , K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(5).toString() , alacak_array , K9_ARRAY_SIZE);
            }
            int hesap_id           = query.VALUE(6).toInt();
            QString tam_hesap_kodu = query.VALUE(7).toString();

            double borc_bakiye   = 0;
            double alacak_bakiye = 0;

            MUH_GET_FROM_GIDER_HESABI_ISLENMEMIS_BAKIYE(hesap_id , borc_bakiye , alacak_bakiye,entegre_filtresi,gun_no);
...
void MUH_GET_FROM_GIDER_HESABI_ISLENMEMIS_BAKIYE
        (int gelir_tablosu_hesabi_id, double &borc_bakiye, double &alacak_bakiye , int entegre_filtresi,int gun_no)
{
    ...
    query.PREPARE_SELECT("muh_yansitma_hesaplari,muh_hesaplar" , "tam_hesap_kodu, "
                         "toplam_borc_array , toplam_alacak_array ,"
                         "ent_borc_array , ent_alacak_array ,"
                         "ent_olmayan_borc_array , ent_olmayan_alacak_array" , "bilanco_gelir_hesabi_id = :bilanco_gelir_hesabi_id AND "
    ...
    if ( entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
        UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_borc_array").toString() , borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_alacak_array").toString() , alacak_array , K9_ARRAY_SIZE);
    }
    else if ( entegre_filtresi EQ FIS_BIRINCI_SIRKET) {
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_borc_array").toString() , borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_alacak_array").toString() , alacak_array , K9_ARRAY_SIZE);
    }
    else {
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_olmayan_borc_array").toString() , borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_olmayan_alacak_array").toString() , alacak_array , K9_ARRAY_SIZE);
    }
...
int MUH_HESAPLAR_TABLOSUNU_OKU(MUH_HESAP_STRUCT *P_HESAP,int p_hesap_id)
{
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "parent_id,hesap_seviyesi,"
                         ...
                         "ent_borc_array,ent_alacak_array,toplam_borc_array,"
                         "toplam_alacak_array,ent_olmayan_borc_array,"
                         "ent_olmayan_alacak_array" , "hesap_id = :hesap_id");
    ...
    P_HESAP->ent_borc_array      = query.VALUE(6).toString();
    P_HESAP->ent_alacak_array    = query.VALUE(7).toString();
    P_HESAP->toplam_borc_array   = query.VALUE(8).toString();
    P_HESAP->toplam_alacak_array = query.VALUE(9).toString();
    P_HESAP->ent_olm_borc_array  = query.VALUE(10).toString();
    P_HESAP->ent_olm_alacak_array= query.VALUE(11).toString();
...
void MUH_CLEAR_HESAP_STRUCT(MUH_HESAP_STRUCT *P_HESAP)
{
    ...
    P_HESAP->ent_alacak_array.clear();
    P_HESAP->ent_borc_array.clear();
    P_HESAP->ent_olm_alacak_array.clear();
    P_HESAP->ent_olm_borc_array.clear();
