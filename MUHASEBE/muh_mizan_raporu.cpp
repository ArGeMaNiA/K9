...
MIZAN_RAPORU::MIZAN_RAPORU ( QList<int> grup_idleri,MUH_RAPOR_VARS * rapor_vars ) : REPORT_KERNEL ( "MUH_MIZAN_RAPORU" )
{
    ...
    M_MUH_HESAPLAR = MUH_GET_HESAPLAR ( M_MUH_RV->entegre_filtresi, M_MUH_RV->bas_hesap_kodu, M_MUH_RV->bts_hesap_kodu );

    m_hesap_idleri = GRP_GRUBA_EKLI_KAYITLARI_BUL(K9_PROGRAMI,MUHASEBE_MODULU,grup_idleri);
...
QString MIZAN_RAPORU::GET_HEADER()
{
    ...
    if ( filters_1.isEmpty() EQ true ) {
        if ( M_MUH_RV->bakiyesiz_hesaplar EQ 1 ) {
            filters_1.append ( "Bakiyesiz" );
        }
        if ( M_MUH_RV->calismamis_hesaplar EQ 1 ) {
...
void MIZAN_RAPORU::YAZDIRILACAK_MUHASEBE_HESAPLARINI_BELIRLE ()
{
    ...
    for ( int i = 0; i < M_MUH_HESAPLAR.size(); i++ ) {
        ...
        UNPACK_DOUBLE_ARRAY ( QVariant ( HESAP_BILGILERI.at(6)).toString() , borc_bakiyesi   , K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( QVariant ( HESAP_BILGILERI.at(7)).toString() , alacak_bakiyesi , K9_ARRAY_SIZE );
