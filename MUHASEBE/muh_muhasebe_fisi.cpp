...
void MUH_MUHASEBE_FISI::SHOW_REKLAM()
{
    m_ui->reklamViewer->SHOW_REKLAM("MUHASEBE-FISI");
}
...
void MUH_MUHASEBE_FISI::SETUP_FORM()
{
    ...
    M_HESABIN_BORC_BAKIYESI   = new double [ K9_ARRAY_SIZE ];
    M_HESABIN_ALACAK_BAKIYESI = new double [ K9_ARRAY_SIZE ];
    ...
    m_ui->comboBox_fis_filtresi->setCurrentIndex(m_ui->comboBox_fis_filtresi->findText(K9_GET_FIS_FILTRESI_STRING( m_ent_filtre_turu)));
    ...
    FIND_AND_SET_COMBOBOX_TEXT(m_ui->comboBox_sirket_turu , K9_GET_KAYDEDILECEK_SIRKET_STRING(M_FIS->kaydedilecek_sirket));
...
void MUH_MUHASEBE_FISI::NEW_FIS_RECORD ()
{
    ...
    FIND_AND_SET_COMBOBOX_TEXT ( m_ui->comboBox_fis_filtresi, K9_GET_FIS_FILTRESI_STRING( m_ent_filtre_turu ) );
...
int MUH_MUHASEBE_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
    ...
    else if( object EQ m_ui->comboBox_fis_filtresi ) {

        m_ent_filtre_turu = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBox_fis_filtresi->currentText());
    ...
    else if ( object EQ m_ui->comboBox_sirket_turu ) {
        M_FIS->kaydedilecek_sirket = K9_GET_KAYDEDILECEK_SIRKET_ENUM(m_ui->comboBox_sirket_turu->currentText());
    }
...
int MUH_MUHASEBE_FISI::FIND_FIS_RECORD ()
{
    ...
    int kaydedilecek_sirket = -1;

    if ( m_ent_filtre_turu EQ FIS_BIRINCI_SIRKET ) {
        kaydedilecek_sirket = 1;
    }
    else if ( m_ent_filtre_turu EQ FIS_IKINCI_SIRKET ) {
        kaydedilecek_sirket = 0;
    }
    ...
    if ( kaydedilecek_sirket NE -1 ) {
        query.AND_EKLE("kaydedilecek_sirket = :kaydedilecek_sirket");
        query.SET_VALUE(":kaydedilecek_sirket" , kaydedilecek_sirket);
    }
...
int MUH_MUHASEBE_FISI::CHECK_LINE_VAR ( int row_number, QObject * object )
{
    ...
    if ( object EQ searchedit_hesap_kodu ) {
        ...
        int entegre_filtresi = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBox_fis_filtresi->currentText());

        if ( entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE(3).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(4).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }
        else if ( entegre_filtresi EQ FIS_BIRINCI_SIRKET ){
            UNPACK_DOUBLE_ARRAY ( query.VALUE(5).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(6).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }
        else if ( entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE(7).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(8).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }


        double hesap_toplam_borc   = M_HESABIN_BORC_BAKIYESI   [ K9_SON_BAKIYE ];
        double hesap_toplam_alacak = M_HESABIN_ALACAK_BAKIYESI [ K9_SON_BAKIYE ];
...
void MUH_MUHASEBE_FISI::SELECTED_LINE ( int row_number, int row_save_status )
{
    ...
    if ( query.SELECT() NE 0 ) {
        ...
        if ( entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
            UNPACK_DOUBLE_ARRAY ( query.VALUE(2).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(3).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }
        else if ( entegre_filtresi EQ FIS_IKINCI_SIRKET){
            UNPACK_DOUBLE_ARRAY ( query.VALUE(4).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(5).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }
        else {
            UNPACK_DOUBLE_ARRAY ( query.VALUE(0).toString(),M_HESABIN_BORC_BAKIYESI   , K9_ARRAY_SIZE );
            UNPACK_DOUBLE_ARRAY ( query.VALUE(1).toString(),M_HESABIN_ALACAK_BAKIYESI , K9_ARRAY_SIZE );
        }
...
int MUH_MUHASEBE_FISI::LEAVE_RECORD ()
{
    ...
    if ( m_log_kaydi_eklensin_mi EQ true ) {
        ...
        if ( m_kayit_eklendi EQ true) {
            K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_FISLER , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_FISLER , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_FISLER , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
...
bool MUH_MUHASEBE_FISI::KASA_ALACAK_BAKIYE_VERIYOR_MU ( int row_number)
{
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "ent_borc_array,ent_alacak_array,toplam_borc_array,toplam_alacak_array",
    ...
    if ( M_FIS->kaydedilecek_sirket EQ 1 ) {
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_borc_array").toString(),M_HESABIN_BORC_BAKIYESI     ,K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( query.VALUE("ent_alacak_array").toString(),M_HESABIN_ALACAK_BAKIYESI   ,K9_ARRAY_SIZE );
    }
    else {
        UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_borc_array").toString(),M_HESABIN_BORC_BAKIYESI     ,K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( query.VALUE("toplam_alacak_array").toString(),M_HESABIN_ALACAK_BAKIYESI   ,K9_ARRAY_SIZE );
    }



    double kasa_hesabi_toplam_borc    = M_HESABIN_BORC_BAKIYESI   [ K9_SON_BAKIYE ];
    double kasa_hesabi_toplam_alacak  = M_HESABIN_ALACAK_BAKIYESI [ K9_SON_BAKIYE ];
...
void MUH_MUHASEBE_FISI::KDV_AYIRMA ( int p_current_row, int p_row_id, int p_fis_id )
{
    ...
    sql_query.PREPARE_INSERT("muh_fis_satirlari" , "fis_satiri_id" , "fis_id, hesap_id, aciklama, borc_tutari, "
                             "alacak_tutari, order_number, gizli_satir_mi, gizli_satir_id, kaydedilecek_sirket, program_id, modul_id, base_record_id ");
    ...
    sql_query.SET_VALUE      ( ":kaydedilecek_sirket", M_FIS->kaydedilecek_sirket                      );
    ...
    MUH_HESABININ_BAKIYELERINI_ARTTIR ( KDV_AYIRMA_STRUCT->kdv_hesap_id, gun_no, KDV_AYIRMA_STRUCT->kdv_borc_tutari, KDV_AYIRMA_STRUCT->kdv_alacak_tutari, 1 );
    ...
    // entegere olsun mu gececi olrak 1 gonderildi
    MUH_HESABININ_BAKIYELERINI_AZALT ( kdvsi_ayrilan_hesap_id, gun_no, KDV_AYIRMA_STRUCT->kdv_borc_tutari, KDV_AYIRMA_STRUCT->kdv_alacak_tutari, 1 );
