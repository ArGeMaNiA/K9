...
void MUH_YEVMIYE_NO_OLUSTUR_BATCH::RUN_BATCH()
{
    ...
    while ( sql_query.NEXT() EQ true ) {
        ...
        if ( yevmiye_numarasi NE 0 and yev_defterine_basildi EQ true ) {
            continue;
        }
    ...
    K9_LOG_KAYDI_EKLE ( MUHASEBE_MODULU , LOG_MUH_YVM_NO_OLUSTUR,LOG_ISLEM_UPDATE,log_detaylari );
...
QDate MUH_YEVMIYE_NO_OLUSTUR_BATCH::SON_YEVMIYE_TARIHI()
{
    ...
    sql_query.PREPARE_SELECT("muh_fisler" , "fis_tarihi, yevmiye_numarasi, yvm_defterine_basildi",
                             "fis_tarihi <= :fis_tarihi AND kaydedilecek_sirket = :kaydedilecek_sirket");
    ...
    while ( sql_query.NEXT() EQ true ) {
        ...
        if ( yev_defterine_basildi EQ true ) {
...
void MUH_YEVMIYE_NO_OLUSTUR_BATCH::KAPANIS_FISINE_NUMARA_VER()
{
    QDate bitis_tarihi ( MALI_YIL_FIRST_DATE() );
        ...
        sql_query.PREPARE_SELECT("muh_fisler" , "fis_id ,fis_turu" , "fis_tarihi = :bitis_tarihi "
                                 "AND fis_turu = :fis_turu AND kaydedilecek_sirket = :kaydedilecek_sirket");
