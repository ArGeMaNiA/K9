...
int  MUH_FIS_EKLE( MUH_FIS_STRUCT * P_FIS , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_INSERT("muh_fisler" , "fis_id" ,
                         ...
                         "last_order_number , modul_id , program_id , base_fis_id, kaydedilecek_sirket");
...
int  MUH_FIS_SATIRI_EKLE ( MUH_FIS_STRUCT * P_FIS , MUH_FIS_SATIRI_STRUCT * P_FIS_SATIRI , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fisler" , "toplam_borc,toplam_alacak,last_order_number,fis_turu,kasa_hesabi_id,fis_tarihi, "
                         "modul_id , program_id, kaydedilecek_sirket" , "fis_id = :fis_id");
    ...
    int    kaydedilecek_sirket  = query.VALUE ( 8 ).toInt();
    ...
    if ( fis_turu EQ ENUM_TAHSIL_FISI OR fis_turu EQ ENUM_TEDIYE_FISI ) {
        ...
        query.PREPARE_INSERT("muh_fis_satirlari" , "fis_satiri_id" , "fis_id , hesap_id , aciklama , "
                             ...
                             "base_record_id, modul_id, program_id, kaydedilecek_sirket");
        ...
        MUH_HESABININ_BAKIYELERINI_ARTTIR ( kasa_hesabi_id , gun_no , P_FIS_SATIRI->alacak_tutari ,
                                                 P_FIS_SATIRI->borc_tutari, kaydedilecek_sirket , USE_DB);
    ...
    MUH_HESABININ_BAKIYELERINI_ARTTIR ( P_FIS_SATIRI->hesap_id , gun_no ,
                                           P_FIS_SATIRI->borc_tutari , P_FIS_SATIRI->alacak_tutari,
                                           kaydedilecek_sirket , USE_DB);
...
int MUH_FIS_SATIRINI_SIL ( MUH_FIS_STRUCT * P_FIS, int p_fis_satiri_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fisler,muh_fis_satirlari" , "toplam_borc , toplam_alacak , fis_turu , fis_tarihi , kasa_hesabi_id,  "
                         ...
                         "muh_fis_satirlari.kaydedilecek_sirket" , "muh_fisler.fis_id = muh_fis_satirlari.fis_id  AND fis_satiri_id = :fis_satiri_id");
    ...
    int    kaydedilecek_sirket = query.VALUE(12).toInt();
    ...
    if ( fis_turu EQ ENUM_TAHSIL_FISI OR fis_turu EQ ENUM_TEDIYE_FISI ) {
        ...
        if ( gizli_satir_id > 0 ) {
            ...
            MUH_HESABININ_BAKIYELERINI_AZALT (kasa_hesabi_id , gun_no , alacak_tutari , borc_tutari, kaydedilecek_sirket ,USE_DB);
        }
    }
    ...
    MUH_HESABININ_BAKIYELERINI_AZALT ( hesap_id , gun_no , borc_tutari , alacak_tutari, kaydedilecek_sirket , USE_DB);
...
void MUH_FIS_SATIRINI_GUNCELLE ( MUH_FIS_STRUCT * P_FIS , MUH_FIS_SATIRI_STRUCT * P_FIS_SATIRI , int p_fis_satiri_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fis_satirlari,muh_fisler" , "hesap_id , borc_tutari , alacak_tutari,toplam_borc , toplam_alacak , kasa_hesabi_id,"
                         ...
                         "muh_fis_satirlari.modul_id , muh_fis_satirlari.program_id, muh_fis_satirlari.kaydedilecek_sirket",
    ...
    int    modul_id             = query.VALUE ( 10 ).toInt();
    int    program_id           = query.VALUE ( 11 ).toInt();
    int    old_entegre_olsun_mu = query.VALUE ( 12 ).toInt();
    ...
    if ( P_FIS_SATIRI->kaydedilecek_sirket NE old_entegre_olsun_mu ) {
        hesap_bakiyeleri_guncellenecek = true;
    }
    ...
    if ( hesap_bakiyeleri_guncellenecek EQ true ) {
        ...
        MUH_HESABININ_BAKIYELERINI_AZALT ( hesap_id , gun_no , borc_tutari , alacak_tutari , old_entegre_olsun_mu ,USE_DB);

        toplam_borc   = ROUND ( toplam_borc - borc_tutari     );
        toplam_alacak = ROUND ( toplam_alacak - alacak_tutari );

        if ( fis_turu EQ ENUM_TAHSIL_FISI ) {
            MUH_HESABININ_BAKIYELERINI_AZALT ( kasa_hesabi_id , gun_no , borc_tutari , 0.0, old_entegre_olsun_mu , USE_DB);
        }
        else if ( fis_turu EQ ENUM_TEDIYE_FISI ) {
            MUH_HESABININ_BAKIYELERINI_AZALT ( kasa_hesabi_id , gun_no , 0.0 , alacak_tutari, old_entegre_olsun_mu , USE_DB );
        }

        MUH_HESABININ_BAKIYELERINI_ARTTIR ( P_FIS_SATIRI->hesap_id , gun_no , P_FIS_SATIRI->borc_tutari ,
                                                 P_FIS_SATIRI->alacak_tutari, P_FIS_SATIRI->kaydedilecek_sirket , USE_DB);

        toplam_borc   = ROUND ( toplam_borc + P_FIS_SATIRI->borc_tutari     );
        toplam_alacak = ROUND ( toplam_alacak + P_FIS_SATIRI->alacak_tutari  );

        if ( fis_turu EQ ENUM_TAHSIL_FISI ) {
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( kasa_hesabi_id , gun_no , P_FIS_SATIRI->borc_tutari , 0.0, P_FIS_SATIRI->kaydedilecek_sirket ,USE_DB);
        }
        else if ( fis_turu EQ ENUM_TEDIYE_FISI ) {
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( kasa_hesabi_id , gun_no , 0.0 , P_FIS_SATIRI->alacak_tutari , P_FIS_SATIRI->kaydedilecek_sirket,USE_DB );
        }
        ...
        if ( borc_tutari_degisti EQ true ) {
            ...
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( hesap_id , gun_no , borc_tutari_farki , 0.0, old_entegre_olsun_mu , USE_DB);

            toplam_borc = ROUND ( toplam_borc + borc_tutari_farki );

            if ( fis_turu EQ ENUM_TEDIYE_FISI ) {
                MUH_HESABININ_BAKIYELERINI_ARTTIR(kasa_hesabi_id , gun_no , 0.0 , borc_tutari_farki, old_entegre_olsun_mu ,USE_DB);
            }
            ...
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( hesap_id , gun_no , 0.0 , alacak_tutari_farki, old_entegre_olsun_mu ,USE_DB);

            toplam_alacak = ROUND ( toplam_alacak + alacak_tutari_farki );

            if ( fis_turu EQ ENUM_TAHSIL_FISI ) {
                MUH_HESABININ_BAKIYELERINI_ARTTIR ( kasa_hesabi_id , gun_no , alacak_tutari_farki , 0.0, old_entegre_olsun_mu,USE_DB );
            }
        }
    }
    ...
    query.PREPARE_UPDATE("muh_fis_satirlari ","fis_satiri_id",
                         ...
                         "alacak_tutari ,kaydedilecek_sirket , "
                         "base_record_id ",
...
void MUH_FISI_GUNCELLE ( MUH_FIS_STRUCT *P_FIS , int p_fis_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fisler" , "fis_tarihi , kasa_hesabi_id , program_id , modul_id , fis_turu , toplam_borc , "
                         "toplam_alacak, kaydedilecek_sirket" , "fis_id = :fis_id");
    ...
    int         old_entegre_olsun_mu    = query.VALUE ( 7 ).toInt();
    int         new_entegre_olsun_mu    = P_FIS->kaydedilecek_sirket;
    ...
    if ( old_entegre_olsun_mu NE new_entegre_olsun_mu ) {
    ...
    if ( fis_tarihi_degisti_mi EQ true OR entegrasyon_degisti_mi EQ true ) {
        ...
        if ( query.SELECT() NE 0 ) {
            ...
            while ( query.NEXT() EQ true ) {
                ...
                MUH_HESABININ_BAKIYELERINI_AZALT ( hesap_id , old_gun_no , borc_tutari , alacak_tutari, old_entegre_olsun_mu ,USE_DB);

                MUH_HESABININ_BAKIYELERINI_ARTTIR ( hesap_id , gun_no    , borc_tutari , alacak_tutari, new_entegre_olsun_mu ,USE_DB);
            }
        }
    ...
    if ( kasa_hesabi_degisti_mi EQ true ) {
        ...
        if ( fis_turu EQ ENUM_TAHSIL_FISI ) {
            MUH_HESABININ_BAKIYELERINI_AZALT  ( kasa_hesabi_id        , old_gun_no , toplam_borc , 0.0 , old_entegre_olsun_mu ,USE_DB);
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( P_FIS->kasa_hesabi_id , gun_no     , toplam_borc , 0.0 , new_entegre_olsun_mu ,USE_DB);

        }
        else if ( fis_turu EQ ENUM_TEDIYE_FISI ) {
            MUH_HESABININ_BAKIYELERINI_AZALT  ( kasa_hesabi_id        , old_gun_no , 0.0 , toplam_alacak ,  old_entegre_olsun_mu ,USE_DB);
            MUH_HESABININ_BAKIYELERINI_ARTTIR ( P_FIS->kasa_hesabi_id , gun_no     , 0.0 , toplam_borc ,    new_entegre_olsun_mu ,USE_DB);
        }
    ...
    if ( entegrasyon_degisti_mi EQ true ) {
        ...
        query.SET_VALUE ( ":kaydedilecek_sirket", new_entegre_olsun_mu );
        query.SET_VALUE ( ":fis_id",           p_fis_id             );
        query.UPDATE();
    ...
    query.PREPARE_UPDATE ( "muh_fisler ","fis_id",
                           "fis_tarihi , fis_no ,"
                           "aciklama , kasa_hesabi_id ,   "
                           "kaydedilecek_sirket , base_fis_id ",
                           "fis_id = :fis_id " );
...
void MUH_FIS_BILGILERINI_OKU ( MUH_FIS_STRUCT * P_FIS , int p_fis_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fisler" , "fis_no, fis_tarihi, aciklama, toplam_borc, toplam_alacak, "
                         ...
                         "kaydedilecek_sirket,yevmiye_numarasi",
                         "fis_id = :fis_id");
    ...
    P_FIS->kaydedilecek_sirket       = query.VALUE(10).toInt();
    P_FIS->yevmiye_no       = query.VALUE(11).toInt();
}
...
void MUH_FIS_SATIRINI_OKU (MUH_FIS_SATIRI_STRUCT * P_FIS_SATIRI , int p_fis_satiri_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    query.PREPARE_SELECT("muh_fis_satirlari" , "fis_id, hesap_id, aciklama, borc_tutari, alacak_tutari, "
                         "order_number, base_record_id, modul_id, program_id, kaydedilecek_sirket",
                         "fis_satiri_id = :fis_satiri_id");
    ...
    P_FIS_SATIRI->kaydedilecek_sirket     = query.VALUE(9).toInt();
}
