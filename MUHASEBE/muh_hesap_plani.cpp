...
MUH_HESAP_PLANI::MUH_HESAP_PLANI ( QWidget * parent ) : TREE_KERNEL( parent ),m_ui ( new Ui::MUH_HESAP_PLANI )
{
    ...
    m_ent_olanlar_tables        = " ent_borc_array,ent_alacak_array ";
    m_ent_olmayanlar_tables     = " ent_olmayan_borc_array,ent_olmayan_alacak_array ";
    m_tum_hesaplar_tables       = " toplam_borc_array,toplam_alacak_array ";

    SQL_QUERY sql_query      ( DB );

    sql_query.PREPARE_SELECT ( "k9_sabit_degerler" ,"k9_ayraci" );
...
void MUH_HESAP_PLANI::ADD_ITEM (QObject *button, QStringList column_datas)
{
    ...
    query.PREPARE_SELECT("muh_hesaplar" , "tam_hesap_kodu, hesap_id, hesap_seviyesi, parent_id",
                         "hesap_id = :hesap_id");
...
void MUH_HESAP_PLANI::ADD_PARENT_ITEMS ()
{
    ...
    if ( m_entegrasyon_durumu EQ FIS_IKINCI_SIRKET ) {
        column_names  += m_ent_olmayanlar_tables;
    }
    else if ( m_entegrasyon_durumu EQ FIS_BIRINCI_SIRKET ) {
        column_names += m_ent_olanlar_tables;
    }
    else {
    ...
    if ( sql_query.SELECT("tam_hesap_kodu ASC") > 0 ) {
        ...
        while ( sql_query.NEXT() EQ true ) {
           ...
           UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(6).toString()    ,BORC_BAKIYESI     ,K9_ARRAY_SIZE );
           UNPACK_DOUBLE_ARRAY ( sql_query.VALUE(7).toString()    ,ALACAK_BAKIYESI   ,K9_ARRAY_SIZE );

           QStringList hesap;

           hesap << sql_query.VALUE(3).toString() << sql_query.VALUE(4).toString() << alt_hesap_sayisi <<  VIRGUL_EKLE ( QVariant ( BORC_BAKIYESI[K9_SON_BAKIYE ] ).toString() )+" "
                 << VIRGUL_EKLE ( QVariant ( ALACAK_BAKIYESI [ K9_SON_BAKIYE ] ).toString())+" " <<  sql_query.VALUE(0).toString() ;
...
{
    m_entegrasyon_durumu = K9_GET_RAPOR_SIRKET_FILTRESI_ENUM(m_ui->combo_box_sirket_turu->currentText());

    REFRESH_TREE_WIDGET();
}

/**************************************************************************************
                   MUH_HESAP_PLANI::SHOW_REKLAM
***************************************************************************************/

void MUH_HESAP_PLANI::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("MUH_HESAP_PLANI");
}
