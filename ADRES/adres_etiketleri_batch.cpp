...
int ADRES_ETIKETLERI_BATCH::CHECK_VAR ( QObject * object )
{
    if ( object EQ m_ui->combo_box_sablonlar ) {
        ...
        if ( selected_index EQ 0 ) {
            ...
            m_ui->push_button_etiket->setIcon ( QIcon ( QString          ( ":/k9_icons/K9_ICONS/etiketler/%1" ).arg ( sablonlar.at ( selected_index - 1 ).icon_path ) ) );
...
void ADRES_ETIKETLERI_BATCH::RUN_BATCH()
{
    ...
    K9_LOG_KAYDI_EKLE ( ADRES_MODULU, LOG_ADR_ETIKET_YAZDIRMA, -1, log_detaylari);
