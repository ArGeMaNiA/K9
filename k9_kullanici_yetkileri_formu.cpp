#include "yonetim.h"
#include "k9_kullanici_yetkileri_formu_class.h"
#include "ui_k9_kullanici_yetkileri_formu.h"
...
#include "sube_enum.h"
#include "k9_yetki.h"
#include "k9_profil_secme_batch.h"
...
/**************************************************************************************
                   OPEN_K9_KULLANICI_YETKILERI_FORMU
***************************************************************************************/

void OPEN_K9_KULLANICI_YETKILERI_FORMU ( int p_kullanici_id, bool * yetkileri_tazele, QWidget * parent)
{
    K9_KULLANICI_YETKILERI_FORMU * F = new K9_KULLANICI_YETKILERI_FORMU ( p_kullanici_id, yetkileri_tazele, parent );
    F->EXEC( FULL_SCREEN );
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::K9_KULLANICI_YETKILERI_FORMU
***************************************************************************************/

K9_KULLANICI_YETKILERI_FORMU::K9_KULLANICI_YETKILERI_FORMU ( int p_kullanici_id, bool * yetkileri_tazele, QWidget * parent ) : FORM_KERNEL(parent), m_ui(new Ui::K9_KULLANICI_YETKILERI_FORMU)
{
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::SETUP_FORM
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::SETUP_FORM()
{
    SET_HELP_PAGE( "k9_kullanici_yetkileri_formu" );
    SET_PAGE_TITLE( tr ( "KULLANICI YETKİLERİ" )   );

    SET_SETTING_NAME( "K9_KULLANICI_YETKILERI_FORMU" );

    m_ui->reklamViewer->SHOW_REKLAM( "K9-KULLANICI-YETKILERI" );

    K9_FILL_KULLANICININ_CALISTIGI_SIRKETLER( m_ui->yonetim_comboBox_k9_kullancinin_calistigi_sirket );
    ...
    if ( query.SELECT() EQ 0 ) {
        ...
        s_query.PREPARE_INSERT( "k9_kullanici_yetkileri", "yetki_id", "kullanici_id" );
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CLEAR_FORM_MEMBERS
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::CLEAR_FORM_MEMBERS ()
{
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::NEW_RECORD
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::NEW_RECORD()
{
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::GET_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::GET_RECORD ( int p_yetki_id )
{
...
/*************************************************************** SISTEM *****************************************/
    ...
    m_ui->yonetim_combo_box_k9_yetkilendirme->setCurrentIndex                        ( sql_query.VALUE("k9_yetkilendirme_yapabilir_mi").toInt()    );
    m_ui->yonetim_combo_box_k9_onar->setCurrentIndex                                 ( sql_query.VALUE("k9_onar_yapabilir_mi").toInt());
    m_ui->yonetim_combo_box_k9_devir_islemi->setCurrentIndex                         ( sql_query.VALUE("k9_devir_islemi_yapabilir_mi").toInt());
    m_ui->yonetim_combo_box_k9_log_bilgileri->setCurrentIndex                        ( sql_query.VALUE("k9_log_gorebilir_mi").toInt());
    m_ui->yonetim_combo_box_k9_sabit_degerler->setCurrentIndex                       ( sql_query.VALUE("k9_sabit_degerlere_girebilirmi").toInt());
    m_ui->yonetim_combo_box_belge_tasarimcisi->setCurrentIndex                       ( sql_query.VALUE("k9_belge_tasarlayabilir_mi").toInt());
    m_ui->yonetim_comboBox_k9_ent_bilgilerini_degistirebilir_mi->setCurrentIndex     ( sql_query.VALUE("k9_ent_blg_degistirebilir_mi").toInt());
    m_ui->yonetim_comboBox_k9_urettigi_fisleri_grbilir_mi->setCurrentIndex           ( sql_query.VALUE("k9_urettigi_fisleri_grbilir_mi").toInt());

    QString calistigi_sirket = K9_GET_KULLANICININ_CALISTIGI_SIRKET_STRING( sql_query.VALUE("k9_kullanicinin_sirketi" ).toInt() );
    FIND_AND_SET_COMBOBOX_TEXT( m_ui->yonetim_comboBox_k9_kullancinin_calistigi_sirket, calistigi_sirket );

    m_ui->yonetim_combo_box_kullanici_sirket_degistire_bilir_mi->setCurrentIndex     ( sql_query.VALUE("k9_sirket_degistire_bilir_mi" ).toInt());
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::SELECT_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::SELECT_RECORD()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHECK_VAR
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHECK_VAR ( QObject * p_object )
{
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHECK_EMPTY
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHECK_EMPTY ()
{
    return ADAK_OK;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHECK_ADD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHECK_ADD ()
{
    return ADAK_OK;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::ADD_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::ADD_RECORD ()
{
    return -1;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHECK_UPDATE
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHECK_UPDATE ( int p_record_id )
{
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::UPDATE_RECORD
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::UPDATE_RECORD ( int p_yetki_id )
{

    K9_KULLANICI_YETKILERI_STRUCT * K9_KULLANICI_YETKILERI = ( K9_KULLANICI_YETKILERI_STRUCT * ) K9_GET_KULLANICI_YETKILERI();

 /*********************************************************  ADRES ************************************************/

    K9_KULLANICI_YETKILERI->adr_adres_karti_tanimlama_yetkisi        = m_ui->adr_comboBox_adres_karti_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->adr_not_defteri_inceleme_yetkisi         = m_ui->adr_comboBox_not_defterini_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->adr_adres_kayitlari_rapor_alma_yetkisi   = m_ui->adr_comboBox_adres_kayit_raporu->currentIndex();
    K9_KULLANICI_YETKILERI->adr_tel_web_kayitlari_rapor_alma_yetkisi = m_ui->adr_comboBox_tel_web_raporu->currentIndex();
    K9_KULLANICI_YETKILERI->adr_etiket_yazdirma_yetkisi              = m_ui->adr_comboBox_etiket_yazdirabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->adr_fihrist_raporu_alma_yetkisi          = m_ui->adr_comboBox_fihrist_yazdirabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->adr_toplu_mail_gonderme_yetkisi          = m_ui->adr_comboBox_toplu_mail_gonderebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->adr_grup_tanimlama_yetkisi               = m_ui->adr_comboBox_adres_grubu_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->adr_toplu_sms_gonderme_yetkisi           = m_ui->adr_comboBox_toplu_sms_gonderme->currentIndex();
    K9_KULLANICI_YETKILERI->adr_export_yetkisi                       = m_ui->adr_comboBox_adres_export->currentIndex();
    K9_KULLANICI_YETKILERI->adr_import_yetkisi                       = m_ui->adr_comboBox_adres_import->currentIndex();



/************************************************************** BANKA  *****************************************/

    K9_KULLANICI_YETKILERI->bnk_banka_hesabi_acma_yetkisi    = m_ui->bnk_combobox_banka_hesabi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_ent_fisleri_inceleme_yetkisi = m_ui->bnk_combobox_ent_fisleri->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_defter_raporu_alma_yetkisi   = m_ui->bnk_combobox_banka_defteri->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_pos_ekstresi_alma_yetkisi    = m_ui->bnk_combobox_pos_ekstresi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_hareket_fisi_islem_yetkisi   = m_ui->bnk_combobox_bnk_hareket_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_acilis_fisi_islem_yetkisi    = m_ui->bnk_combobox_acilis_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_makro_tanimlama_yetkisi      = m_ui->bnk_combobox_bnk_makro_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_kk_ekstresi_alma_yetkisi     = m_ui->bnk_combobox_kk_ekstresi_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_virman_islem_yetkisi         = m_ui->bnk_combobox_virman_islemi_yapabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->bnk_makro_fisi_islem_yetkisi     = m_ui->bnk_combobox_bnk_makro_fisi->currentIndex();


/******************************************************************* CARI   *************************************/

    K9_KULLANICI_YETKILERI->cari_tahsilat_fisi_islem_yetkisi  = m_ui->car_combobox_tahsilat_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->cari_odeme_fisi_islem_yetkisi     = m_ui->car_combobox_odeme_fisi->currentIndex() ;
    K9_KULLANICI_YETKILERI->cari_hareket_fisi_islem_yetkisi   = m_ui->car_combobox_hareket_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->cari_acilis_fisi_islem_yetkisi    = m_ui->car_combobox_cari_acilis_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->cari_grup_tanimlama_yetkisi       = m_ui->car_combobox_cari_grup->currentIndex();
    K9_KULLANICI_YETKILERI->cari_kart_tanimlama_yetkisi       = m_ui->car_combobox_cari_hesap->currentIndex();
    K9_KULLANICI_YETKILERI->cari_ent_fisleri_inceleme_yetkisi = m_ui->car_combobox_ent_fisleri->currentIndex();
    K9_KULLANICI_YETKILERI->cari_ekstre_alma_yetkisi          = m_ui->car_combobox_cari_hesap_ekstresi->currentIndex();
    K9_KULLANICI_YETKILERI->cari_bakiye_raporu_alma_yetkisi   = m_ui->car_combobox_cari_bakiye_raporu->currentIndex();
    K9_KULLANICI_YETKILERI->cari_virman_islem_yetkisi         = m_ui->car_combobox_virman_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->cari_hesap_hareketleri_gorebilir_mi= m_ui->car_combobox_cari_hesap_hareketleri_grb_mi->currentIndex();


/********************************************************* CEKSENET *****************************************************************/

    K9_KULLANICI_YETKILERI->cek_senet_girebilir_mi            = m_ui->cek_combobox_cek_girisi->currentIndex();
    K9_KULLANICI_YETKILERI->cek_bordrosu_islem_yetkisi        = m_ui->cek_combobox_bordro_girisi->currentIndex();
    K9_KULLANICI_YETKILERI->cek_ent_fisleri_inceleme_yetkisi  = m_ui->cek_combobox_ent_fisleri->currentIndex();
    K9_KULLANICI_YETKILERI->cek_listesi_rapor_alma_yetkisi    = m_ui->cek_combobox_cek_senet_listesi->currentIndex();
    K9_KULLANICI_YETKILERI->cek_mus_cek_senet_arama_yetkisi   = m_ui->cek_combobox_musteri_cek_ve_senetleri->currentIndex();
    K9_KULLANICI_YETKILERI->cek_acilis_bordrosu_islem_yetkisi = m_ui->cek_combobox_acilis_bordrosu_yetkisi->currentIndex();


/***************************************************** DEMIRBAS *****************************************************/

    K9_KULLANICI_YETKILERI->dmr_demirbas_karti_tanimlama_yetkisi    = m_ui->dmr_comboBox_demirbas_karti->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_grup_tanimlama_yetkisi              = m_ui->dmr_comboBox_demirbas_grubu->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_yen_degerleme_fisi_islem_yetkisi    = m_ui->dmr_comboBox_yeniden_degerleme->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_amortisman_fisi_islem_yetkisi       = m_ui->dmr_comboBox_amortisman_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_demirbas_listesi_rapor_alma_yetkisi = m_ui->dmr_comboBox_demirbas_listesi->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_toplu_amortisman_ayirma_yetkisi     = m_ui->dmr_comboBox_toplu_amortisman_ayirma->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_toplu_yeniden_degerleme_yetkisi     = m_ui->dmr_comboBox_toplu_yeniden_degerleme->currentIndex();
    K9_KULLANICI_YETKILERI->dmr_rapor_islem_grubu_tanimlama_yetkisi = m_ui->dmr_comboBox_rapor_grubu_tanimlayabilir_mi->currentIndex();


/*********************************************************** MUHASEBE *******************************************/

    K9_KULLANICI_YETKILERI->muh_muhasebe_fisi_islem_yetkisi       = m_ui->muh_combobox_mhs_fisi_kesebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_ent_fisleri_inceleme_yetkisi      = m_ui->muh_combobox_ent_fisi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hesap_plani_islem_yetkisi         = m_ui->muh_combobox_hesap_planini_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_yvm_no_olusturma_yetkisi          = m_ui->muh_combobox_yvm_no_olusturabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hesap_ekstresi_alma_yetkisi       = m_ui->muh_combobox_hsp_ekstresini_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_mizan_raporu_alma_yetkisi         = m_ui->muh_combobox_mizan_raporunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hesap_plani_rapor_alma_yetkisi    = m_ui->muh_combobox_hesap_plani_raporunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_gelir_tablosu_rapor_alma_yetkisi  = m_ui->muh_combobox_gelir_tablosunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_bilanco_rapor_alma_yetkisi        = m_ui->muh_combobox_bilanco_inceleyebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_yvm_dokumu_alma_yetkisi           = m_ui->muh_combobox_yvm_dokumunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_defteri_kebir_raporu_alma_yetkisi = m_ui->muh_combobox_defteri_kebiri_inceleyebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_fis_dokumu_alma_yetkisi           = m_ui->muh_combobox_fis_dokumunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hatali_fisleri_inceleme_yetkisi   = m_ui->muh_combobox_hatali_fisleri_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_grup_tanimlama_yetkisi            = m_ui->muh_combobox_hesap_grubu->currentIndex();
    K9_KULLANICI_YETKILERI->muh_yansitma_hesabi_tanimlama_yetkisi = m_ui->muh_combobox_yansitma_hesaplari_girebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_yansitma_fisi_olusturma_yetkisi   = m_ui->muh_combobox_yansitma_fisi_olusturabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hesap_ekstresi_nakli_yekun        = m_ui->muh_combobox_hsp_ekstresi_nakli_yekun->currentIndex();
    K9_KULLANICI_YETKILERI->muh_mizan_nakli_yekun                 = m_ui->muh_combobox_mizan_nakli_yekun->currentIndex();
    K9_KULLANICI_YETKILERI->muh_hesap_plani_kopyalama_yetkisi     = m_ui->muh_combobox_hesap_plani_kopyalama->currentIndex();
    K9_KULLANICI_YETKILERI->muh_acilis_fisi_islem_yetkisi         = m_ui->muh_combobox_acilis_fisi_islem_yetkisi->currentIndex();



/***************************************************************** PERSONEL ***************************************/

    K9_KULLANICI_YETKILERI->prs_sicil_karti_tanimlama_yetkisi        = m_ui->prs_comboBox_prs_sicil_karti_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->prs_ek_odenek_tanimlama_yetkisi          = m_ui->prs_comboBox_ek_odenek_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->prs_ek_kesinti_tanimlama_yetkisi         = m_ui->prs_comboBox_ek_kesinti_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->prs_sabit_deger_guncelleme_yetkisi       = m_ui->prs_comboBox_sabit_degerler->currentIndex();
    K9_KULLANICI_YETKILERI->prs_maas_bordrosu_alma_yetkisi           = m_ui->prs_combo_box_maas_bordrosu_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_maas_bordrosu_iptal_yetkisi          = m_ui->prs_combo_box_maas_bordrosu_iptal_edebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_maas_bordrosu_raporu_alma_yetkisi    = m_ui->prs_combo_box_maas_bordrosu_raporu_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_ihbar_ve_kidem_tazminati_raporu_alma_yetkisi = m_ui->prs_combo_box_ihbar_ve_kidem_tazminati_raporu_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_isten_ayrilma_islem_yetkisi          = m_ui->prs_comboBox_isten_ayrilma_islemi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_bordro_inceleme_yetkisi              = m_ui->prs_combo_box_bordro_inceleme_yapabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_personel_raporu_alma_yetkisi         = m_ui->prs_combo_box_personel_raporu_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_e_bildirge_alma_yetkisi              = m_ui->prs_comboBox_e_bildirge_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prs_grup_tanimlama_yetkisi               = m_ui->prs_comboBox_grup_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->prs_isten_ayrilan_personel_arama_yetkisi = m_ui->prs_comboBox_isten_ayrilan_arama->currentIndex();
    K9_KULLANICI_YETKILERI->prs_eksik_gun_nedenleri_giris_yetkisi    = m_ui->prs_comboBox_eksik_gun_nedenleri->currentIndex();
    K9_KULLANICI_YETKILERI->prs_ucret_pusulasi_raporu_alma_yetkisi   = m_ui->prs_comboBox_ucret_pusulasi->currentIndex();

/*************************************************** SUBE / DEPO / UNITE  ****************************************/

    K9_KULLANICI_YETKILERI->sub_sube_tanimlama_yetkisi           = m_ui->sub_combobox_sube_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sub_depo_tanimlama_yetkisi           = m_ui->sub_comboBox_depo_kartini_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sub_depo_islem_yetkisi               = SUBE_GET_DEPO_ISLEM_YETKISI_ENUM ( m_ui->sub_comboBox_depo_islem_yetkisi->currentText());
    K9_KULLANICI_YETKILERI->sub_sube_islem_yetkisi               = SUBE_GET_ISLEM_YETKISI_ENUM ( m_ui->sub_comboBox_sube_islem_yetkisi->currentText());
    K9_KULLANICI_YETKILERI->sub_ent_hesaplari_guncelleme_yetkisi = m_ui->sub_comboBox_sube_ent_hesaplari->currentIndex();


/********************************************************* STOK *************************************************/

    K9_KULLANICI_YETKILERI->stok_urun_karti_tanimlama_yetkisi   = m_ui->stk_comboBox_urun_kartini_gorebilir_mi->currentIndex() ;
    K9_KULLANICI_YETKILERI->stok_hizmet_karti_tanimlama_yetkisi = m_ui->stk_combobox_hizmet_tanimlama->currentIndex();
    K9_KULLANICI_YETKILERI->stok_grup_tanimlama_yetkisi         = m_ui->stk_comboBox_grup_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->stok_fisi_islem_yetkisi             = m_ui->stk_comboBox_stok_fisine_girebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->stok_urun_listesi_alma_yetkisi      = m_ui->stk_comboBox_urun_listesi->currentIndex();
    K9_KULLANICI_YETKILERI->stok_miktar_raporu_alma_yetkisi     = m_ui->stk_comboBox_stok_miktarlari->currentIndex();
    K9_KULLANICI_YETKILERI->stok_ekstresi_alma_yetkisi          = m_ui->stk_comboBox_stok_ekstresi->currentIndex();
    K9_KULLANICI_YETKILERI->stok_urun_hareketlerini_gorebilir_mi= m_ui->stk_comboBox_stok_urun_hareketlerini_grb_mi->currentIndex();


/*********************************************    FATURA          **************************************************/

    K9_KULLANICI_YETKILERI->fat_alis_faturasi_kesebilir_mi   = m_ui->fat_combobox_alis_faturasi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_sts_faturasi_kesebilir_mi    = m_ui->fat_combobox_satis_faturasi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_elle      = m_ui->fat_combobox_elle_irs_faturalastirma->currentIndex();
    K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_oto       = m_ui->fat_combobox_oto_irs_faturalastirma_->currentIndex();
    K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_toplu     = m_ui->fat_combobox_toplu_irs_faturalastirma->currentIndex();
    K9_KULLANICI_YETKILERI->fat_satir_iskontosu_yapabilir_mi = m_ui->fat_comboBox_satir_iskontosu_yapabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_toplu_iskonto_yapabilir_mi   = m_ui->fat_comboBox_toplu_iskonto_yapabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_hizmet_ekstresi_alma_yetkisi = m_ui->fat_comboBox_hizmet_ekstre_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_ba_bs_listesi_alma_yetkisi   = m_ui->fat_comboBox_ba_bs_alabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->fat_fatura_listesi_alma_yetkisi  = m_ui->fat_comboBox_fatura_listesi->currentIndex();


/******************************************   İRSALİYE *************************************************************/

    K9_KULLANICI_YETKILERI->irs_alis_irsaliyesi_islem_yetkisi = m_ui->irs_combobox_alis_irsaliyesi->currentIndex();
    K9_KULLANICI_YETKILERI->irs_sts_irsaliyesi_islem_yetkisi  = m_ui->irs_combobox_satis_irsaliyesi->currentIndex();
    K9_KULLANICI_YETKILERI->irs_fat_irsaliyelestirme_elle     = m_ui->irs_combobox_elle_fatura_irsaliyelestirme->currentIndex();
    K9_KULLANICI_YETKILERI->irs_fat_irsaliyelestirme_oto      = m_ui->irs_combobox_oto_fatura_irsaliyelestirme->currentIndex();
    K9_KULLANICI_YETKILERI->irs_satir_iskontosu_yapabilir_mi  = m_ui->irs_comboBox_satir_iskontosu_yapabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->irs_listesi_gorebilir_mi          = m_ui->irs_comboBox_irsaliye_listesi_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->irs_toplu_iskonto_yapabilir_mi    = m_ui->irs_comboBox_toplu_iskonto_yapabilir_mi->currentIndex();


/******************************************************** ISLETME ****************************************************/

    K9_KULLANICI_YETKILERI->isl_isletme_defteri_rapor_alma_yetkisi = m_ui->isl_combobox_isletme_defteri_raporunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->isl_kdv_raporu_alma_yetkisi            = m_ui->isl_combobox_kdv_raporunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->isl_fisi_islem_yetkisi                 = m_ui->isl_combobox_fis_kesebilir_mi->currentIndex();


/******************************************************** SM_MAKBUZU ****************************************************/

    K9_KULLANICI_YETKILERI->smm_makbuzu_islem_yetkisi              = m_ui->comboBox_sm_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->smm_hizmet_tanimlama_yetkisi           = m_ui->comboBox_smm_hizmet_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->smm_tahsil_edildi_odendi_secili_gelsin = m_ui->comboBox_smm_tahsil_edildi_odendi->currentIndex();

/******************************************************** DOVIZ ****************************************************/

    K9_KULLANICI_YETKILERI->dvz_dovizleri_inceleme_yetkisi = m_ui->dvz_comboBox_doviz_kurlari_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->dvz_doviz_tanimlama_yetkisi    = m_ui->dvz_comboBox_doviz_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->dvz_kur_guncelleme_yetkisi     = m_ui->dvz_comboBox_kur_guncelleyebilir_mi->currentIndex();

/********************************************       PERAKENDE      ************************************************/

    K9_KULLANICI_YETKILERI->prk_fis_girebilir_mi          = m_ui->prk_comboBox_fis_girebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prk_gdr_tur_tanimlayabilir_mi = m_ui->prk_comboBox_gider_turu_tanimlayabilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->prk_kasa_raporu_alabilir_mi   = m_ui->prk_comboBox_kasa_raporu->currentIndex();


/************************************************************* SISTEM **********************************************/

    K9_KULLANICI_YETKILERI->sis_blg_formuna_girebilir_mi        = m_ui->sistem_comboBox_bilgisayarlar_formunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sis_kul_formuna_girebilir_mi        = m_ui->sistem_comboBox_kullanicilar_formunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sis_veritabani_formuna_girebilir_mi = m_ui->sistem_comboBox_firmalar_formunu_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sis_yonetim_log_inceleme_yetkisi    = m_ui->sistem_comboBox_007_yonetim_bilgilerini_gorebilir_mi->currentIndex();
    K9_KULLANICI_YETKILERI->sis_mail_srv_tanimlayabilirmi       = m_ui->sistem_combo_box_posta_sunucusu_tanimlayabilirmi->currentIndex();
    K9_KULLANICI_YETKILERI->sis_sms_srv_tanimlayabilirmi        = m_ui->sistem_combo_box_sms_sunucusu_tanimlayabilirmi->currentIndex() ;


    K9_KULLANICI_YETKILERI->ynt_k9_yetkilendirme_yapabilir_mi   = m_ui->yonetim_combo_box_k9_yetkilendirme->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_onar_yetkisi                 = m_ui->yonetim_combo_box_k9_onar->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_devir_islemi_yapabilir_mi    = m_ui->yonetim_combo_box_k9_devir_islemi->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_log_inceleme_yetkisi         = m_ui->yonetim_combo_box_k9_log_bilgileri->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_sabit_degerler_guncelleme_yetkisi = m_ui->yonetim_combo_box_k9_sabit_degerler->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_belge_tasarlama_yetkisi      = m_ui->yonetim_combo_box_belge_tasarimcisi->currentIndex();
    K9_KULLANICI_YETKILERI->ynt_k9_ent_bilgilerini_degistirme_yetkisi = m_ui->yonetim_comboBox_k9_ent_bilgilerini_degistirebilir_mi->currentIndex() ;

    K9_KULLANICI_YETKILERI->ynt_k9_kullanicinin_calistigi_sirket         = K9_GET_KULLANICININ_CALISTIGI_SIRKET_ENUM( m_ui->yonetim_comboBox_k9_kullancinin_calistigi_sirket->currentText() );
    K9_KULLANICI_YETKILERI->ynt_k9_kullanici_sirket_degistire_bilir_mi = m_ui->yonetim_combo_box_kullanici_sirket_degistire_bilir_mi->currentIndex();

    K9_KULLANICI_YETKILERI->ynt_k9_urettigi_fisleri_grbilir_mi  = m_ui->yonetim_comboBox_k9_urettigi_fisleri_grbilir_mi->currentIndex();
    ...
    if ( m_ui->check_box_muhasebe->isChecked() EQ true ) {
        ...
        K9_KULLANICI_YETKILERI->k9_muh_kullanabilir_mi = 0;
    }
    ...
    if ( m_ui->check_box_isletme->isChecked() EQ true ) {
        ...
        K9_KULLANICI_YETKILERI->k9_isl_kullanabilir_mi = 0;
    }
    ...
    if ( m_ui->check_box_adres->isChecked() EQ true ) {
        ...
        K9_KULLANICI_YETKILERI->k9_adr_kullanabilir_mi = 0;
    }
    ...
    if ( m_ui->check_box_doviz->isChecked() EQ true ) {
        ...
        K9_KULLANICI_YETKILERI->k9_dvz_kullanabilir_mi = 0;
    }
    ...
    if (m_ui->check_box_sm_makbuzu->isChecked() EQ true ){
        ...
        K9_KULLANICI_YETKILERI->k9_smm_kullanabilir_mi = 0;
    }
    if ( m_ui->check_box_faiz->isChecked() EQ true ) {
        K9_KULLANICI_YETKILERI->k9_faiz_kullanabilir_mi = 1;
    }
    else {
        K9_KULLANICI_YETKILERI->k9_faiz_kullanabilir_mi = 0;
    }
    ...
    if ( m_ui->check_box_perakende->isChecked() EQ true ) {
        ...
        K9_KULLANICI_YETKILERI->k9_prk_kullanabilir_mi = 0;
    }
    ...
    sql_query.PREPARE_SELECT ( "k9_kullanici_yetkileri", "kullanici_id","yetki_id = :yetki_id");
    ...
    K9_LOG_KAYDI_EKLE (K9_YONETIM , LOG_K9_YETKILENDIRME , -1 , kullanici_kodu_adi);


}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHECK_DELETE
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHECK_DELETE ( int record_id )
{
    Q_UNUSED(record_id);
    return ADAK_OK;
}
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::DELETE_RECORD
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::DELETE_RECORD ( int record_id )
{
    Q_UNUSED ( record_id );
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::FIND_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::FIND_RECORD ()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::FIND_FIRST_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::FIND_FIRST_RECORD ()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::FIND_LAST_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::FIND_LAST_RECORD ()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::FIND_NEXT_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::FIND_NEXT_RECORD ()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::FIND_PREV_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::FIND_PREV_RECORD ()
{
    return 0;
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::LOCK_RECORD
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::LOCK_RECORD ( int p_yetki_id )
{
    return DB->LOCK_ROW( "yetki_id","k9_kullanici_yetkileri", QString( "yetki_id = %1" ).arg( p_yetki_id ) );
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::UNLOCK_RECORD
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::UNLOCK_RECORD(int p_yetki_id )
{
    DB->UNLOCK_ROW( "yetki_id","k9_kullanici_yetkileri", QString( "yetki_id = %1" ).arg( p_yetki_id  ) );
}

/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::TUM_COMBOXLARI_DEGISTIR
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::TUM_COMBOXLARI_DEGISTIR ( int value )
{
/*************************************************** ADRES  **************************************************/
...
/*********************************************************** SISTEM **************************************************/
    ...
    m_ui->yonetim_combo_box_k9_yetkilendirme->setCurrentIndex                          ( value );
    m_ui->yonetim_combo_box_k9_devir_islemi->setCurrentIndex                           ( value );
    m_ui->yonetim_combo_box_belge_tasarimcisi->setCurrentIndex                         ( value );
    m_ui->yonetim_combo_box_k9_log_bilgileri->setCurrentIndex                          ( value );
    m_ui->yonetim_combo_box_k9_onar->setCurrentIndex                                   ( value );
    m_ui->yonetim_combo_box_k9_sabit_degerler->setCurrentIndex                         ( value );
    m_ui->yonetim_comboBox_k9_ent_bilgilerini_degistirebilir_mi->setCurrentIndex       ( value );
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::EKRANI_DUZENLE
***************************************************************************************/

void K9_KULLANICI_YETKILERI_FORMU::EKRANI_DUZENLE()
{
    ...
    for ( int i = tab_size; i >= 0 ; i-- ) {
        ...
        m_ui->yonetim_combo_box_k9_devir_islemi->setHidden(true);
        m_ui->label_ynt_devir_islemi->setHidden(true);

        m_ui->yonetim_combo_box_k9_onar->setHidden(true);
        m_ui->label_ynt_k9_onar->setHidden(true);
...
/**************************************************************************************
                   K9_KULLANICI_YETKILERI_FORMU::CHANGER_BUTTON_CLICKED
***************************************************************************************/

int K9_KULLANICI_YETKILERI_FORMU::CHANGER_BUTTON_CLICKED ( QAbstractButton * p_button )
{
    if ( p_button EQ m_ui->pushButton_profil_secin ) {
           bool degisiklik_yapildimi = false;

           OPEN_K9_PROFIL_SECME( ENUM_K9_KULLANICI_YETKILERI, NULL, &degisiklik_yapildimi );

           K9_KULLANICI_YETKILERI_STRUCT * K9_KULLANICI_YETKILERI = ( K9_KULLANICI_YETKILERI_STRUCT * ) K9_GET_KULLANICI_YETKILERI();

           if ( degisiklik_yapildimi EQ true ) {

               m_ui->check_box_muhasebe->setChecked        ( QVariant( K9_KULLANICI_YETKILERI->k9_muh_kullanabilir_mi).toBool() );
               m_ui->check_box_isletme->setChecked         ( QVariant( K9_KULLANICI_YETKILERI->k9_isl_kullanabilir_mi).toBool() );
               m_ui->check_box_adres->setChecked           ( QVariant( K9_KULLANICI_YETKILERI->k9_adr_kullanabilir_mi).toBool() );
               m_ui->check_box_doviz->setChecked           ( QVariant( K9_KULLANICI_YETKILERI->k9_dvz_kullanabilir_mi).toBool() );
               m_ui->check_box_cari->setChecked            ( QVariant( K9_KULLANICI_YETKILERI->k9_car_kullanabilir_mi).toBool() );
               m_ui->check_box_cek->setChecked             ( QVariant( K9_KULLANICI_YETKILERI->k9_cek_kullanabilir_mi).toBool() );
               m_ui->check_box_banka->setChecked           ( QVariant( K9_KULLANICI_YETKILERI->k9_bnk_kullanabilir_mi).toBool() );
               m_ui->check_box_sube->setChecked            ( QVariant( K9_KULLANICI_YETKILERI->k9_sub_kullanabilir_mi).toBool() );
               m_ui->check_box_fatura->setChecked          ( QVariant( K9_KULLANICI_YETKILERI->k9_fat_kullanabilir_mi).toBool() );
               m_ui->check_box_irsaliye->setChecked        ( QVariant( K9_KULLANICI_YETKILERI->k9_irs_kullanabilir_mi).toBool() );
               m_ui->check_box_stok->setChecked            ( QVariant( K9_KULLANICI_YETKILERI->k9_stk_kullanabilir_mi).toBool() );
               m_ui->check_box_personel->setChecked        ( QVariant( K9_KULLANICI_YETKILERI->k9_prs_kullanabilir_mi).toBool() );
               m_ui->check_box_demirbas->setChecked        ( QVariant( K9_KULLANICI_YETKILERI->k9_dmr_kullanabilir_mi).toBool() );
               m_ui->check_box_sm_makbuzu->setChecked      ( QVariant( K9_KULLANICI_YETKILERI->k9_smm_kullanabilir_mi).toBool() );
               m_ui->check_box_faiz->setChecked            ( QVariant( K9_KULLANICI_YETKILERI->k9_faiz_kullanabilir_mi).toBool() );
               m_ui->check_box_perakende->setChecked       ( QVariant( K9_KULLANICI_YETKILERI->k9_prk_kullanabilir_mi).toBool() );
