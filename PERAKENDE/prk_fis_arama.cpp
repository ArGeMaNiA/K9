...
void PRK_FIS_ARAMA::SETUP_FORM()
{
    ...
    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ 1 ) {
        m_ui->comboBox_fis_filtresi->setCurrentIndex(m_ui->comboBox_fis_filtresi->findText(K9_GET_FIS_FILTRESI_STRING(FIS_BIRINCI_SIRKET)));
    }
    else {
        m_ui->comboBox_fis_filtresi->setCurrentIndex(m_ui->comboBox_fis_filtresi->findText(K9_GET_FIS_FILTRESI_STRING(FIS_HER_IKI_SIRKET)));
    }

    m_ui->frame_fis_filtresi->setHidden(true);
...
void PRK_FIS_ARAMA::SEARCH ()
{
    ...
    if ( entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        ...
        query.SET_VALUE ( ":kaydedilecek_sirket", 1 );
    }
    else if ( entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
        query.AND_EKLE  ( "kaydedilecek_sirket = :kaydedilecek_sirket" );
        query.SET_VALUE ( ":kaydedilecek_sirket", 0 );
    }
