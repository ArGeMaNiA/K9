...
void PRK_PERAKENDE_FISI::SETUP_FORM()
{
    ...
    query.PREPARE_SELECT("k9_sabit_degerler" , "ontanimli_kdv_orani,kdv_oranlari_array,ind_kdv_hesap_id_array, hes_kdv_hesap_id_array ",
...
void PRK_PERAKENDE_FISI::CLEAR_FORM_MEMBERS()
{
    ...
    K9_CLEAR_ENT_DETAYLARI_STRUCT(M_ENT_DETAYLARI);
...
int PRK_PERAKENDE_FISI::GET_FIS_RECORD ( int record_id )
{
    ...
    query.PREPARE_SELECT("prk_fisler" ,
                         ...
                         "kdv_dahil_toplam, modul_id , program_id, kaydedilecek_sirket,"
    ...
    M_FIS->kaydedilecek_sirket       = query.VALUE(9).toInt();
    M_FIS->muh_fis_id       = query.VALUE(10).toInt();
    ...
    while ( query.NEXT() ) {
        ...
        combobox_tah_odm_sekli->setCurrentIndex(combobox_tah_odm_sekli->findText(K9_GET_TAH_ODM_SEKLI_STRING(tah_odm_sekli)));
...
void PRK_PERAKENDE_FISI::SET_LINE_DEFAULTS ( int row_number )
{
    ...
    K9_FILL_KDV_ORANLARI_COMBOBOX(combobox_kdv_orani);
...
int PRK_PERAKENDE_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
    ...
    if ( object EQ m_ui->searchEdit_sube_kodu ) {
        ...
        m_entegre_filtresi = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBox_fis_filtresi->currentText());
...
int PRK_PERAKENDE_FISI::CHECK_LINE_VAR ( int row_number, QObject * object )
{
    ...
    if ( ( object EQ commaedit_kdv_haric_tutar) OR  ( object EQ commaedit_kdv_dahil_tutar )
        ...
        if ( K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tah_odm_sekli->currentText()) EQ ENUM_NAKIT  ) {
            if ( M_FIS->kaydedilecek_sirket EQ 1 ) {
                ...
                query.PREPARE_SELECT("k9_sabit_degerler" , "max_nakit_giris_cikis_tutari" ,
                                     "sabit_deger_id = :sabit_deger_id");
                query.SET_VALUE(":sabit_deger_id" , 1);
        ...
        int tah_odm_sekli = K9_GET_TAH_ODM_SEKLI_ENUM(combobox_tah_odm_sekli->currentText());
    ...
    switch ( tah_odm_sekli ) {
        case ENUM_HAVALE:
        {
            ...
            if ( lineedit_hesap_no_id->text().toInt() EQ 0 ) {
                if ( OPEN_K9_EFT_BATCH(-1 , ENUM_HAVALE , fis_satiri_id ,DVZ_GET_TEMEL_PARA_BIRIMI_ID(),
        ...
        case ENUM_EFT   :
        {
            ...
            if ( lineedit_hesap_no_id->text().toInt() EQ 0 ) {
                if ( OPEN_K9_EFT_BATCH(-1 , ENUM_EFT,fis_satiri_id ,DVZ_GET_TEMEL_PARA_BIRIMI_ID(),
                                       M_ISLEM_DETAYLARI , PERAKENDE_MODULU , this) EQ false ){
        ...
        case ENUM_KREDI_KARTI:
        {
            ...
            if ( m_gider_fisi_mi EQ 1 ) {
                if ( lineedit_pos_kk_id->text().toInt() EQ 0 ) {
                    if ( OPEN_K9_KK_ODEME_BATCH(fis_satiri_id , M_ISLEM_DETAYLARI ,
                ...
                if ( lineedit_pos_kk_id->text().toInt() EQ 0 ) {
                    if ( OPEN_K9_KK_TAHSILAT_BATCH(fis_satiri_id , M_ISLEM_DETAYLARI , PERAKENDE_MODULU,
                                                   this) EQ false ) {
                        return ADAK_FAIL;
                    }
...
void PRK_PERAKENDE_FISI::SAVER_BUTTON_CLICKED(QAbstractButton *button, int record_id)
{
    ...
    if ( button EQ m_ui->pushButton_yazdir ) {
        ...
        K9_URETILEN_FIS_SEC( K9_PROGRAMI, PERAKENDE_MODULU, record_id, this );
    }
}
...
int PRK_PERAKENDE_FISI::CHANGER_BUTTON_CLICKED(QAbstractButton *button)
{
    if ( button EQ m_ui->pushButton_detaylar ) {
        ...
        if ( record_id > 0 ) {
            int muh_fis_id = MUH_GET_ENT_FIS_ID(K9_PROGRAMI , PERAKENDE_MODULU,
                                                record_id);
            M_ENT_DETAYLARI->muh_fis_no        = K9_GET_FIS_NO(MUHASEBE_MODULU , muh_fis_id);

        }
        OPEN_K9_ENT_DETAYLARI_BATCH ( M_ENT_DETAYLARI ,true, this);
    ...
    switch ( tah_odm_sekli ) {
        ...
        case ENUM_KREDI_KARTI:
        {
            ...
            if ( m_gider_fisi_mi EQ 1 ) {
                ...
                if ( OPEN_K9_KK_TAHSILAT_BATCH(fis_satiri_id , M_ISLEM_DETAYLARI,
                                               PERAKENDE_MODULU,this) EQ true ) {
...
void PRK_PERAKENDE_FISI::SET_KASA_MEVCUDU()
{
    ...
    if ( M_FIS->kaydedilecek_sirket EQ 1 ) {
        UNPACK_DOUBLE_ARRAY(MUHASEBE_HESABI->ent_borc_array   , borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(MUHASEBE_HESABI->ent_alacak_array , alacak_array , K9_ARRAY_SIZE);
    }
    else {
        UNPACK_DOUBLE_ARRAY(MUHASEBE_HESABI->toplam_borc_array   , borc_array , K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(MUHASEBE_HESABI->toplam_alacak_array , alacak_array , K9_ARRAY_SIZE);
    }

    double toplam_borc   = borc_array[K9_SON_BAKIYE];
    double toplam_alacak = alacak_array[K9_SON_BAKIYE];
...
/**************************************************************************************
                   PRK_PERAKENDE_FISI::SHOW_REKLAM
***************************************************************************************/

void PRK_PERAKENDE_FISI::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("PRK_PERAKENDE_FISI");
}
