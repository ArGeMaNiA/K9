...
void PRS_MAAS_BORDROSU_IPTAL_BATCH::RUN_BATCH()
{
    ...
    if ( PRS_VERILEN_DONEM_BORDROSU_KESILMIS_MI ( m_ay, m_yil, m_sube_id ) EQ ADAK_FAIL ) {
        ...
        MSG_WARNING(  K9_GET_AYLAR_STRING ( m_ay ) + " " + QVariant(m_yil).toString() +
                               tr(" Dönemine Ait Bordro Hazırlanmamıştır <br> ""Lütfen Yeniden Dönem Seçiniz !.."), m_ui->combo_box_aylar );
        return;
    }

    int secim = MSG_YES_NO_CANCEL( K9_GET_AYLAR_STRING ( m_ay ) + " " + QVariant(m_yil).toString() +
    ...
    int cari_fis_id = CARI_GET_ENT_FIS_ID( K9_PROGRAMI , PERSONEL_MODULU , bordro_id);
...
void PRS_MAAS_BORDROSU_IPTAL_BATCH::BORDRO_IPTAL_ET  ( int p_bordro_id )
{
    ...
    log_detaylari = K9_GET_AYLAR_STRING ( m_ay ) + " " + QVariant( m_yil ).toString() +
                    " Dönemine Ait Bordro İptal Edilmiştir ";

    K9_LOG_KAYDI_EKLE( PERSONEL_MODULU, LOG_PRS_BORDROLAR, -1 , log_detaylari );
    ...
    if ( query.SELECT("bordro_donemi_ay DESC", 0, 1 ) NE 0 ) {
        ...
        if ( enson_alinan_bordro_ayi > secilen_ay ) {
            MSG_WARNING( tr(" <b>%1</b> Ayına Bordro alındığı için Bordro İptal edilemez. \n "
                            "Bordro İptal etmek için önce %1' ayını iptal etlemisiniz.").arg( K9_GET_AYLAR_STRING( enson_alinan_bordro_ayi )), m_ui->combo_box_aylar );
            return ADAK_FAIL;
        }
    }
