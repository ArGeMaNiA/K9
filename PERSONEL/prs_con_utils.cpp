...
void PRS_ENTEGRASYON_FISLERINI_OLUSTUR ( int p_bordro_id , int p_program_id , ADAK_SQL * P_GELEN_DB)
{
    ...
    double ssk_isveren_taban_deger          = 0.00;
    double ssk_isveren_tavan_deger          = 0.00;

    double prs_sgdp_primi_isci_yuzdesi      = 0.00;
    ...
    query.PREPARE_SELECT( "prs_sabit_degerler ",
                          ...
                          "ssk_isveren_taban_deger, ssk_isveren_tavan_deger, muh_fis_hesaplari_birlestir, "
                          "prs_sgdp_primi_isci_yuzdesi, prs_sgdp_primi_isveren_yuzdesi, tehlike_sinifi, isy_5510_nolu_kanun_yuzdesi ",
                          "sabit_degerler_id =:sabit_degerler_id" );
    ...
    if ( query.SELECT() NE 0 ) {
        ...
        ssk_isveren_taban_deger             = query.VALUE( 2 ).toDouble();
        ssk_isveren_tavan_deger             = query.VALUE( 3 ).toDouble();
        muh_fis_hesaplari_birlestir         = query.VALUE( 4 ).toInt();
        prs_sgdp_primi_isci_yuzdesi         = query.VALUE( 5 ).toDouble();
        prs_sgdp_primi_isveren_yuzdesi      = query.VALUE( 6 ).toDouble();
        tehlike_sinifi                      = query.VALUE( 7 ).toInt();
        isy_5510_nolu_kanun_yuzdesi         = query.VALUE( 8 ).toDouble();
    }
    ...
    while ( query.NEXT() EQ true ) {
        ...
        if ( muh_fis_hesaplari_birlestir EQ ENUM_BORDRO_DETAYLARI ) {
            ...
            MUH_FIS_SATIRI->aciklama      = QString ( " %1  %2 dönemi Şube Giderleri Tutarı." ).arg( QString::number( bordro_donemi_yil ) )
                                                                                               .arg( K9_GET_AYLAR_STRING( bordro_donemi_ay ) );
        ...
        CARI_FIS_SATIRI->kaydedilecek_sirket              = 1;
...
void PRS_BORDRO_TOPLAMLARI_MUH_ENT ( int p_bordro_id, int p_muh_fis_id, int p_sube_id, ADAK_SQL * P_GELEN_DB )
{
    ...
    while ( query.NEXT() EQ true ) {
          ...
          if ( s_query.SELECT() NE 0 ) {
              ...
              ssk_isveren_taban_deger         = s_query.VALUE( 2 ).toDouble();
              ssk_isveren_tavan_deger         = s_query.VALUE( 3 ).toDouble();
              prs_sgdp_primi_isci_yuzdesi     = s_query.VALUE( 4 ).toDouble();
              prs_sgdp_primi_isveren_yuzdesi  = s_query.VALUE( 5 ).toDouble();
              tehlike_sinifi                  = s_query.VALUE( 6 ).toInt();
              isy_5510_nolu_kanun_yuzdesi     = s_query.VALUE( 7 ).toDouble();
          }
...
void PRS_KIDEM_IHBAR_TAZMINATI_ENTEGRASTONU  ( PRS_KIDEM_IHBAR_ENT_STRUCT  * P_STRUCT , ADAK_SQL * P_GELEN_DB )
{
    ...
    MUH_FIS->kaydedilecek_sirket       = P_STRUCT->kaydedilecek_sirket;
    ...
    CARI_FIS->fis_turu              = ENUM_CARI_HAREKET_FISI;
    CARI_FIS->kaydedilecek_sirket            = P_STRUCT->kaydedilecek_sirket;
    CARI_FIS->muh_fis_id            = muh_fis_id;
    ...
    MUH_FIS_SATIRI->kaydedilecek_sirket          = P_STRUCT->kaydedilecek_sirket;
    ...
    CARI_FIS_SATIRI->base_record_id          = 0;
    CARI_FIS_SATIRI->kaydedilecek_sirket              = P_STRUCT->kaydedilecek_sirket;
...
void PRS_CLEAR_KIDEM_IHBAR_ENT_STRUCT ( PRS_KIDEM_IHBAR_ENT_STRUCT * P_STRUCT )
{
    ...
    P_STRUCT->modul_id                       = 0;
    P_STRUCT->kaydedilecek_sirket                     = 1;
...
QString PRS_GET_BORDRO_DONEMI(int p_bordro_id)
{
    ...
    return K9_GET_AYLAR_STRING( query.VALUE( 0 ).toInt() );
}
