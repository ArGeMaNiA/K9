...
void PRS_MAAS_BORDROSU_BATCH::RUN_BATCH()
{
    ...
    int ay  =  K9_GET_AYLAR_ENUM( m_ui->combo_box_aylar->currentText() );
    ...
    if ( PRS_VERILEN_DONEM_BORDROSU_KESILMIS_MI (  ay,  yil, m_sube_id ) EQ ADAK_OK ) {
        MSG_WARNING( K9_GET_AYLAR_STRING( ay ) + " " + QVariant(yil).toString() +
    ...
    PRS_ENTEGRASYON_FISLERINI_OLUSTUR( bordro_id , K9_PROGRAMI );
...
void PRS_MAAS_BORDROSU_BATCH::BORDRO_BILGILERINI_DB_YAZ (PRS_BORDRO_STRUCT * BORDRO_BILGILERI )
{
   ...
   log_detaylari = K9_GET_AYLAR_STRING( BORDRO_BILGILERI->bordro_donemi_ay) + " " + QVariant(BORDRO_BILGILERI->bordro_donemi_yil).toString() +
                   " Bordrosu Kesilmiştir.";

   K9_LOG_KAYDI_EKLE(PERSONEL_MODULU , LOG_PRS_BORDROLAR , LOG_ISLEM_ADD ,log_detaylari);
...
{
    // hangi ay ?
    int secilen_ay = K9_GET_AYLAR_ENUM( m_ui->combo_box_aylar->currentText() );
    ...
    if ( query.SELECT("bordro_donemi_ay DESC", 0, 1 ) NE 0 ) {
        ...
        if ( enson_alinan_bordro_ayi > secilen_ay ) {
            MSG_WARNING( tr(" <b>%1</b> Ayına Bordro alındığı için bu Ay için Bordro alamazsınız. \n "
                            "Bordro almak  için %1' ayını iptal etlemisiniz.").arg( K9_GET_AYLAR_STRING( enson_alinan_bordro_ayi )), m_ui->combo_box_aylar );
            return ADAK_FAIL;
        }
    }
