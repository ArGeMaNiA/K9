...
void PRS_UCRET_PUSULASI_BATCH::SETUP_FORM()
{
    ...
    K9_FILL_COMBOBOX_AYLAR( m_ui->comboBox_bordro_ay );

    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_bordro_ay, K9_GET_AYLAR_STRING( QDate::currentDate().month() ) );
...
void PRS_UCRET_PUSULASI_BATCH::RUN_BATCH ()
{
    ...
    M_UCRET_PUSULASI_VAR->bordro_ay         = K9_GET_AYLAR_ENUM( m_ui->comboBox_bordro_ay->currentText() );
    M_UCRET_PUSULASI_VAR->bordro_yil        = m_ui->comboBox_bordro_yil->currentText().toInt();
    M_UCRET_PUSULASI_VAR->personel_list     = GRP_GRUBA_EKLI_KAYITLARI_BUL ( K9_PROGRAMI, PERSONEL_MODULU, m_grup_idleri );
...
int PRS_UCRET_PUSULASI_BATCH::CHANGER_BUTTON_CLICKED(QAbstractButton *p_button)
{
    if ( p_button EQ m_ui->pushButton_gruplar ) {
        if ( OPEN_GRUP_SECIM ( K9_PROGRAMI,PERSONEL_MODULU , &m_grup_idleri, DB, this ) EQ true ) {
