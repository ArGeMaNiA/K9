...
void  BNK_VIRMAN_FORMU::SETUP_FORM ()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "BANKA-VIRMAN-FORMU" );
...
int BNK_VIRMAN_FORMU::ADD_RECORD ()
{
    ...
    if ( K9_GET_KAYDEDILECEK_SIRKET_ENUM(m_ui->comboBox_sirket_turu->currentText()) EQ SIRKET_1 ) {
        kaydedilecek_sirket = SIRKET_1;
    }
    else {
        kaydedilecek_sirket = SIRKET_2;
    }
    ...
    VIRMAN_STRUCT->kaydedilecek_sirket                   = kaydedilecek_sirket;
...
void BNK_VIRMAN_FORMU::UPDATE_RECORD ( int record_id )
{
    ...
    if ( m_gon_hesabi_doviz_id NE m_alici_hesabi_doviz_id) {
        if(m_gon_hesabi_doviz_id EQ DVZ_GET_TEMEL_PARA_BIRIMI_ID() ) {
            ...
            para_birim_id   = m_gon_hesabi_doviz_id;
        }
        else if( m_alici_hesabi_doviz_id EQ DVZ_GET_TEMEL_PARA_BIRIMI_ID() ) {
            islem_tutari    = m_ui->commaEdit_alan_tutar->GET_DOUBLE();
            para_birim_id   = m_alici_hesabi_doviz_id;
        }
        else {
    ...
    QStringList def_row_list    =   GET_BNK_DEFTER_FIS_ROW_ID_LIST( K9_PROGRAMI, BANKA_MODULU, def_fis_id );
    ...
    int kaydedilecek_sirket = SIRKET_2;
    ...
    query.PREPARE_UPDATE ( "bnk_virman_fisler","fis_id",
                           "aciklama, fis_tarihi, kaydedilecek_sirket, toplam_hesaba_giren, toplam_hesaptan_cikan ",
                           "fis_id = :fis_id " );
    ...
    query.PREPARE_INSERT (  "bnk_virman_fis_satirlari","fis_satiri_id" ,"fis_id, aciklama, gonderen_islem_tutari, alici_islem_tutari,   "
                            ...
                            "program_id, modul_id, kaydedilecek_sirket, gonderen_hesap_no_id, alici_hesap_no_id,     "
...
void BNK_VIRMAN_FORMU::SAVER_BUTTON_CLICKED(QAbstractButton *button, int record_id)
{
    if ( button EQ m_ui->pushButton_urettigi_fisler ) {
        int def_fis_id = GET_BNK_DEFTER_FIS_ID( K9_PROGRAMI, BANKA_MODULU, record_id );
                                   ...
                                   bool p_base_fis_satiri_eklenecek , ADAK_SQL * P_GELEN_DB)
{
    ...
    MUH_FIS->kaydedilecek_sirket   = P_VIRMAN_STRUCT->kaydedilecek_sirket;
    ...
    if ( virman_fis_id EQ 0 ) {
        query.PREPARE_INSERT("bnk_virman_fisler","fis_id",
                             ...
                             "modul_id, base_fis_id, kaydedilecek_sirket,                            "
        ...
        query.SET_VALUE( ":kaydedilecek_sirket"                      , P_VIRMAN_STRUCT->kaydedilecek_sirket           );
    ...
    DEF_FIS->program_id                         = K9_PROGRAMI;
    DEF_FIS->base_fis_id                        = virman_fis_id;
    DEF_FIS->hesap_no_id                        = P_VIRMAN_STRUCT->gonderen_hesap_no_id;
    DEF_FIS->kaydedilecek_sirket                         = P_VIRMAN_STRUCT->kaydedilecek_sirket;
    ...
    DEF_FIS_2->program_id                         = K9_PROGRAMI;
    DEF_FIS_2->base_fis_id                        = virman_fis_id;
    DEF_FIS_2->hesap_no_id                        = P_VIRMAN_STRUCT->alici_hesap_no_id;
    DEF_FIS_2->kaydedilecek_sirket                   = P_VIRMAN_STRUCT->kaydedilecek_sirket;
    ...
    DEF_SATIRI->program_id        = K9_PROGRAMI;
    DEF_SATIRI->modul_id          = BANKA_MODULU;
    DEF_SATIRI->base_record_id    = satir_id;
    DEF_SATIRI->kaydedilecek_sirket  = P_VIRMAN_STRUCT->kaydedilecek_sirket;
...
void    CLEAR_BNK_VIRMAN_FIS_SATIRI_STRUCT ( BNK_VIRMAN_FIS_AND_SATIRI_STRUCT * P_VIRMAN_STRUCT  )
{
    ...
    P_VIRMAN_STRUCT->fis_no                     = 0;
    P_VIRMAN_STRUCT->kaydedilecek_sirket        =-1;
