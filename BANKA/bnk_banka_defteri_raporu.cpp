...
QString BNK_BANKA_DEFTERI_RAPORU::GET_HEADER()
{
   ...
   string_list << K9_GET_FIRMA_ISMI() << "<b>BANKA DEFTERİ<b>" << GET_REPORT_TOP_RIGHT_HEADER();
...
void BNK_BANKA_DEFTERI_RAPORU::PRINT_NAKLI_YEKUN()
{
    ...
    if ( BNK_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
        ...
        UNPACK_DOUBLE_ARRAY(select_query.VALUE("ent_olmayan_borc_array").toString(),toplam_borc_array,K9_ARRAY_SIZE);
        UNPACK_DOUBLE_ARRAY(select_query.VALUE("ent_olmayan_alacak_array").toString(),toplam_alacak_array,K9_ARRAY_SIZE);
    }
