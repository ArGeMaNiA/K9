...
m_ui ( new Ui::BNK_MAKRO_FISI )
{
    ...
    M_ENT_DETAYLARI     = new K9_ENT_DETAYLARI_STRUCT;
...
void BNK_MAKRO_FISI::SETUP_FORM ()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM("BANKA-MAKRO-FISI");
    ...
    if( K9_KULLANICININ_CALISTIGI_SIRKET() EQ 1 ){
        ...
        m_kaydedilecek_sirket = SIRKET_1;
    }
    else {
        m_ent_filtresi_turu =  FIS_HER_IKI_SIRKET;
        m_kaydedilecek_sirket = SIRKET_2;
    }
    ...
    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket, K9_GET_KAYDEDILECEK_SIRKET_STRING( m_kaydedilecek_sirket ));
...
{    
    ...
    sql_query.PREPARE_SELECT ( "bnk_makro_fisler","fis_no, fis_tarihi, hesap_no_id, aciklama, "
                               "toplam_makro_tutari, kaydedilecek_sirket, muh_fis_id","fis_id = :fis_id" );
    ...
    m_muh_fis_id            = sql_query.VALUE( 6 ).toInt();
    m_kaydedilecek_sirket   = sql_query.VALUE( 5 ).toInt();
...
int BNK_MAKRO_FISI::CHECK_FIS_FORM_VAR ( QObject * object )
{
    ...
    if( object EQ m_ui->comboBox_fis_filtresi )
    {
    ...
    else if ( object EQ m_ui->combo_box_banka_hesap_kodu  ) {
        ...
        m_kaydedilecek_sirket = K9_GET_KAYDEDILECEK_SIRKET_ENUM( m_ui->comboBox_sirket->currentText() );
    }
...
int BNK_MAKRO_FISI::ADD_FIS_RECORD()
{
    ...
    M_FIS->kaydedilecek_sirket = m_kaydedilecek_sirket;
...
void BNK_MAKRO_FISI::UPDATE_FIS_RECORD ( int fis_id )
{
    ...
    int def_fis_id  = GET_BNK_DEFTER_FIS_ID( K9_PROGRAMI, BANKA_MODULU, fis_id );
        ...
        while ( s_query.NEXT() EQ true ) {
            ...
            switch ( hesap_turu ) {
                ...
                case ENUM_BNK_KREDI_KARTI_HESABI: {
                        ...
                        DEF_FIS->kaydedilecek_sirket= m_kaydedilecek_sirket;
                ...
                case ENUM_BNK_KASA_HESABI: {
                        ...
                        int muh_fis_id  = MUH_GET_ENT_FIS_ID( K9_PROGRAMI, BANKA_MODULU, fis_id );
                        ...
                        MUH_FIS->kaydedilecek_sirket   = m_kaydedilecek_sirket;
                ...
                case ENUM_BNK_CARI_HESAP: {
                        ...
                        int cari_fis_id = CARI_GET_ENT_FIS_ID( K9_PROGRAMI, BANKA_MODULU, def_fis_id );

                        CARI_FIS_BILGILERINI_OKU( CARI_FIS, cari_fis_id );

                        CARI_FIS->kaydedilecek_sirket = m_kaydedilecek_sirket;
    ...
    query.PREPARE_UPDATE("bnk_makro_fisler","fis_id","fis_no ,fis_tarihi,"
                                "hesap_no_id , aciklama , kaydedilecek_sirket ",
                                "fis_id = :fis_id");
    ...
    query.SET_VALUE ( ":kaydedilecek_sirket" , m_kaydedilecek_sirket                       );
    query.SET_VALUE ( ":fis_id"           , fis_id                                         );

    query.UPDATE();

    query.PREPARE_SELECT ( "bnk_makro_fisler","fis_tarihi, hesap_no_id, aciklama, "
                               "kaydedilecek_sirket, toplam_makro_tutari","fis_id = :fis_id");
...
//    int         hesap_no_id         = sql_query.VALUE(1).toInt();
//    QString     aciklama            = sql_query.VALUE(2).toString();
//    int         kaydedilecek_sirket    = sql_query.VALUE(3).toInt();
    double      makro_tutari            = query.VALUE(4).toDouble();
    ...
    if ( object EQ combobox_makro_turu ) {
        ...
        switch ( hesap_turu ) {
            ...
            case ENUM_BNK_CARI_HESAP        : {
                    ...
                    select_query.PREPARE_SELECT("bnk_makro_satirlari","hesaba_etkisi",
                                                "hesap_turu = :hesap_turu AND makro_id = :makro_id");
                    ...
                    if ( select_query.SELECT() NE 0 ) {
                        select_query.NEXT();
                        hesaba_etkisi = select_query.VALUE(0).toInt();
                    }
...
{  
    ...
    M_FIS_SATIRI->kaydedilecek_sirket   = m_kaydedilecek_sirket;
...
void BNK_MAKRO_FISI::UPDATE_LINE ( int fis_id, int row_number )
{
    ...
    select_query.PREPARE_SELECT ( "bnk_makro_fis_satirlari, bnk_makro_fisler","fis_tarihi, bnk_makro_fisler.aciklama, makro_tutari, otomatik_odeme_tutari, hesap_turu, "
                                  "hesap_id, doviz_kuru, parite, bnk_makro_fisler.kaydedilecek_sirket ",
    ...
    int     old_entegre_olsun_mu      = select_query.VALUE( 8 ).toInt();
    ...
    if ( old_fis_tarihi NE m_ui->adakDate_fis_tarihi->QDATE() OR
         ...
         old_parite NE commaedit_parite->GET_DOUBLE() OR
         old_entegre_olsun_mu NE m_kaydedilecek_sirket ) {
...
int BNK_MAKRO_FISI::LEAVE_RECORD()
{
    if ( m_log_kaydi_eklensin_mi EQ true ) {
        ...
        if ( m_kayit_eklendi EQ true ) {
            K9_LOG_KAYDI_EKLE ( BANKA_MODULU , LOG_BNK_FISLER , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE ( BANKA_MODULU , LOG_BNK_FISLER , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE ( BANKA_MODULU , LOG_BNK_FISLER , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
...
void BNK_MAKRO_FISI::LINE_SEARCH_EDIT_CLICKED ( int row_number,QWidget * widget, QLineEdit * line_edit )
{
    ...
    switch ( hesap_turu ) {
        case ENUM_BNK_CARI_HESAP :
        {
            int cari_hesap_id = OPEN_CARI_KART_ARAMA( "", SAHIS_ARTI_FIRMA,this,true, K9_PROGRAMI, CARI_MODULU );
...
void BNK_MAKRO_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_record_id )
{
    if( p_button EQ m_ui->buton_yazdir ) {
        ...
        int def_fis_id = GET_BNK_DEFTER_FIS_ID( K9_PROGRAMI, BANKA_MODULU, p_record_id );
        K9_URETILEN_FIS_SEC( K9_PROGRAMI, BANKA_MODULU, def_fis_id, this );
    }
}
...
int BNK_MAKRO_FISI::CHANGER_BUTTON_CLICKED ( QAbstractButton * p_button )
{
    ...
    int pre_entegre_oldu_mu = m_kaydedilecek_sirket;
    if ( p_button EQ m_ui->pushButton_detaylar ) {
        OPEN_K9_ENT_DETAYLARI_BATCH ( M_ENT_DETAYLARI, false,this );
        if( pre_entegre_oldu_mu NE m_kaydedilecek_sirket ){
            degismis_mi = true;
        }
...
int MAKRO_ADD_LINE( MAKRO_SATIR_STRUCT * P_FIS_SATIRI, int p_fis_satiri_id , bool base_fis_satiri_eklenecek , ADAK_SQL *P_GELEN_DB )
{
    ...
    double      temel_para_birimi_tutari = 0.00;
    ...
    if ( cari_hesap_id NE 0 ) {
        cari_para_birimi_tutari = CARI_GET_PARA_BIRIMI_TUTARI(cari_para_birimi_id,P_FIS_SATIRI->banka_doviz_id,
                                                             P_FIS_SATIRI->doviz_kuru ,P_FIS_SATIRI->parite,
                                                             P_FIS_SATIRI->islem_tutari);
    }
    ...
    if ( base_fis_satiri_eklenecek EQ true ) {
        query.PREPARE_INSERT("bnk_makro_fis_satirlari","fis_satiri_id" ,
                             ...
                             "doviz_kuru, parite, kaydedilecek_sirket, "
    ...
    int     banka_fark_satiri_id        = -1;
    int     banka_fark_satiri_hesaba_etkisi   = -1;
        ...
        switch ( makro_hesap_turu ) {
            ...
            case ENUM_BNK_CARI_HESAP: {
                    ...
                    if ( car_fis_id EQ 0 ) { // banka hareket fisi inin bir cari hesap yok ise olusturulur
                        ...
                        CARI_FIS->base_fis_id           = def_fis_id;
                        CARI_FIS->kaydedilecek_sirket   = P_FIS_SATIRI->kaydedilecek_sirket;
                    ...
                    CARI_FIS_SATIRI->kaydedilecek_sirket     = P_FIS_SATIRI->kaydedilecek_sirket;
    ...
    if ( banka_satirlari_eklenecek_mi EQ true ) {
        ...
        double doviz_kuru   = P_FIS_SATIRI->doviz_kuru;
        ...
        DEF_FIS_SATIRI->kaydedilecek_sirket = P_FIS_SATIRI->kaydedilecek_sirket;
        ...
        if ( bnk_ara_toplam_borc > 0.00  ) {
            ...
            doviz_kuru  = ROUND( temel_ara_toplam_hesaptan_cikan / bnk_ara_toplam_borc, -4 );
        ...
        if ( bnk_ara_toplam_alacak > 0.00 ) {
            ...
            doviz_kuru  = ROUND( temel_ara_toplam_hesaba_giren / bnk_ara_toplam_alacak, -4 );
        ...
        banka_fark_satiri_id            = fis_satiri_id;
        banka_fark_satiri_hesaba_etkisi = hesaba_etkisi;
...
int  MAKRO_ADD_FIS ( MAKRO_FIS_STRUCT * P_FIS, int p_fis_id , bool base_fis_eklenecek  ,ADAK_SQL * P_GELEN_DB)
{
    ...
    if ( base_fis_eklenecek EQ true  ) {
        insert_query.PREPARE_INSERT("bnk_makro_fisler","fis_id","fis_no,fis_tarihi,                                                                "
                                    ...
                                    "kaydedilecek_sirket, muh_fis_id, def_fis_id, toplam_hesaba_giren, toplam_hesaptan_cikan ");
        ...
        insert_query.SET_VALUE ( ":kaydedilecek_sirket"     ,  P_FIS->kaydedilecek_sirket  );
    ...
    if (  P_FIS->def_fis_id EQ 0 ) {
        ...
        DEF_FIS->kaydedilecek_sirket                = P_FIS->kaydedilecek_sirket;
...
void  CLEAR_MAKRO_FIS_SATIRI( MAKRO_SATIR_STRUCT * P_FIS_SATIRI )
{
    ...
    P_FIS_SATIRI->kaydedilecek_sirket      = 0;
