...
void BNK_HAREKET_FISI::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM("BNK - BANKA_HAREKET");
    ...
    K9_KULLANICI_YETKILERI_STRUCT * K9_KUL_YETKILERI = ( K9_KULLANICI_YETKILERI_STRUCT * ) K9_GET_KULLANICI_YETKILERI();

    m_entegrasyon_hesaplarini_girebilir_mi           = K9_KUL_YETKILERI->ynt_k9_ent_bilgilerini_degistirme_yetkisi;
...
int BNK_HAREKET_FISI::FIND_FIS_RECORD()
{
    ...
    if( m_modul_id EQ 0 ) {
        where_str.append( " AND program_id != " + QString::number( K9_PROGRAMI ) );
    }
    else {
...
void BNK_HAREKET_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_record_id )
{
    if( p_button EQ m_ui->buton_yazdir ){
        ...
        K9_URETILEN_FIS_SEC( M_FIS->program_id, M_FIS->modul_id, M_FIS->def_fis_id, this );
    }
}
