...
void  BNK_ACILIS_FORMU::SETUP_FORM ()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "BANKA-ACILIS-FISI" );
    ...
    FIND_AND_SET_COMBOBOX_TEXT(m_ui->comboBox_sirket , K9_GET_KAYDEDILECEK_SIRKET_STRING(m_ent_filtresi_turu));
...
{  
    ...
    int sirket_turu = K9_GET_KAYDEDILECEK_SIRKET_ENUM( m_ui->comboBox_sirket->currentText() );

    if ( sirket_turu EQ SIRKET_1 ) {
        BNK_HRK_FIS->kaydedilecek_sirket = SIRKET_1;
    }
    else {
        BNK_HRK_FIS->kaydedilecek_sirket = SIRKET_2;
    }
    ...
    BNK_HRK_FIS_SATIRI->kaydedilecek_sirket  = BNK_HRK_FIS->kaydedilecek_sirket;
