...
QStringList BNK_POS_EKSTRESI::GET_POS_ISLEMLERI()
{
    ...
    if ( BNK_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        where_str += QString ( " AND kaydedilecek_sirket = 1 ");
    }
    else if ( BNK_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ){
        where_str += QString ( " AND kaydedilecek_sirket = 0 ");
    }
...
void BNK_POS_EKSTRESI::PRINT_NAKLI_YEKUN()
{
    ...
    if ( BNK_RV->entegre_filtresi EQ FIS_BIRINCI_SIRKET ) {
        UNPACK_DOUBLE_ARRAY(query.VALUE("ent_kk_tahsilat_borc_array").toString() , toplam_borc_array , K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY(query.VALUE("ent_kk_tahsilat_alacak_array").toString() , toplam_alacak_array , K9_ARRAY_SIZE );
    }
    else if ( BNK_RV->entegre_filtresi EQ FIS_IKINCI_SIRKET ) {
        UNPACK_DOUBLE_ARRAY(query.VALUE("ent_olm_kk_tahsilat_borc_array").toString() , toplam_borc_array , K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY(query.VALUE("ent_olm_kk_thslat_alacak_array").toString() , toplam_alacak_array , K9_ARRAY_SIZE );
    }
    else if ( BNK_RV->entegre_filtresi EQ FIS_HER_IKI_SIRKET ) {
        UNPACK_DOUBLE_ARRAY(query.VALUE("toplam_kk_tahsilat_borc_array").toString() , toplam_borc_array , K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY(query.VALUE("toplam_kk_thslat_alacak_array").toString() , toplam_alacak_array , K9_ARRAY_SIZE );
    }
