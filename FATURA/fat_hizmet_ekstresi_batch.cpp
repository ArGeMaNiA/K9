...
int FAT_HIZMET_EKSTRESI_BATCH::CHECK_VAR ( QObject * object )
{
    if ( object EQ m_ui->searchEdit_bas_hizmet_kodu OR object EQ m_ui->searchEdit_bts_hizmet_kodu OR
        ...
        m_entegrasyon_durumu = K9_GET_RAPOR_SIRKET_FILTRESI_ENUM (m_ui->comboBox_sirket_filtresi->currentText());
    }
...
int FAT_HIZMET_EKSTRESI_BATCH::CHECK_RUN ()
{
    ...
    if ( K9_MALI_YIL_TARIH_ARALIGI_KONTROLU( m_ui->adakDatebas_tarihi, m_ui->adakDate_bitis_tarihi ) NE ADAK_OK ) {
        return ADAK_FAIL;
    }
