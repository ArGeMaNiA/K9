...
void FAT_FATURA_LISTESI_BATCH::SETUP_FORM()
{
    ...
    FIND_AND_SET_COMBOBOX_TEXT( m_ui->comboBox_sirket_filtresi, K9_GET_RAPOR_SIRKET_FILTRESI_STRING( m_ent_filtresi_turu ));
...
{
    if( object EQ m_ui->comboBox_sirket_filtresi ) {

        m_ent_filtresi_turu = K9_GET_RAPOR_SIRKET_FILTRESI_ENUM(m_ui->comboBox_sirket_filtresi->currentText());
    }
...
{
    if ( K9_MALI_YIL_TARIH_ARALIGI_KONTROLU( m_ui->adakDate_bas_tarihi, m_ui->adakDate_bts_tarihi ) NE ADAK_OK ) {
        return ADAK_FAIL;
    }
    if ( m_ui->checkBox_cari_hesap->isChecked() EQ true ) {
        if ( m_ui->searchEdit_cari_hesap_kodu->GET_TEXT().isEmpty() EQ true ) {
