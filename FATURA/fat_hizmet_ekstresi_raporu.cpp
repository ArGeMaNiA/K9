...
void FAT_HIZMET_EKSTRESI_RAPORU::GET_HIZMET_ISLEMLERI(int hizmet_id, QString hizmet_kodu_adi)
{
    ...
    if ( M_FAT_RV->entegre_filtresi_turu EQ FIS_BIRINCI_SIRKET ) {
        query.AND_EKLE  ( "fat_faturalar.kaydedilecek_sirket = :kaydedilecek_sirket " );
        query.SET_VALUE ( ":kaydedilecek_sirket", 1 );
    }
    else if ( M_FAT_RV->entegre_filtresi_turu EQ FIS_IKINCI_SIRKET ) {
        query.AND_EKLE  ( "fat_faturalar.kaydedilecek_sirket = :kaydedilecek_sirket " );
...
void FAT_HIZMET_EKSTRESI_RAPORU::PRINT_NAKLI_YEKUN(int hizmet_id)
{
    ...
    if ( M_FAT_RV->entegre_filtresi_turu EQ FIS_BIRINCI_SIRKET ) {
        where_str += QString ( " AND fat_fatura_satirlari.kaydedilecek_sirket = :kaydedilecek_sirket ");
