...
void FAT_TOPLU_FATURALASTIR_ARAMA::SETUP_FORM()
{
    ...
    query.PREPARE_SELECT ( "k9_sabit_degerler","irs_faturalandirilacak_gun" ,"sabit_deger_id = :sabit_deger_id" );
...
int FAT_TOPLU_FATURALASTIR_ARAMA::CHECK_VAR ( QObject * object )
{
    ...
    if ( object EQ m_ui->searchEdit_cari_hesap_kodu ) {
        ...
        OPEN_GRUP_SECIM( K9_PROGRAMI, CARI_MODULU, &m_grup_idleri, DB , this, true );
...
void FAT_TOPLU_FATURALASTIR_ARAMA::SEARCH()
{
    ...
    if ( m_ui->checkBox_cari_hesap_kodu->isChecked() EQ true ) {
        ...
        QList<int> cari_hesap_id_list = GRP_GRUBA_EKLI_KAYITLARI_BUL ( K9_PROGRAMI, CARI_MODULU, m_grup_idleri );
...
void FAT_TOPLU_FATURALASTIR_ARAMA::FATURALASTIR( int p_irsaliye_fis_id )
{
    ...
    FATURA_FISI->program_id                     = K9_PROGRAMI;
    FATURA_FISI->fatura_irsaliyelestirildi_mi   = 1;
    FATURA_FISI->kaydedilecek_sirket            = K9_KULLANICININ_CALISTIGI_SIRKET();
    ...
    for ( int i = 0 ; i < irs_str_id_list.size() ; i++ ) {
        ...
        FATURA_FIS_SATIRI->kaydedilecek_sirket              = STK_FIS_SATIRI->kaydedilecek_sirket;
    ...
    K9_LOG_KAYDI_EKLE ( FATURA_MODULU , LOG_FAT_TOPLU_FATURALASTIRMA , -1, log_detaylari );
...
void FAT_TOPLU_FATURALASTIR_ARAMA::SEARCH_EDIT_CLICKED ( QWidget * widget , QLineEdit * line_edit )
{
    if ( widget EQ m_ui->searchEdit_cari_hesap_kodu ) {
        int cari_hesap_id = OPEN_CARI_KART_ARAMA ( line_edit->text(), SAHIS_ARTI_FIRMA, this, false , K9_PROGRAMI, CARI_MODULU );
