...
void FAT_FATURA_FISI::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "FATURA-FISI" );
    ...
    query.PREPARE_SELECT("k9_sabit_degerler" , "irs_faturalandirilacak_gun,ontanimli_kdv_orani",
    ...
    M_FIS->acik_kapali_fatura  = K9_GET_ACIK_KAPALI_FATURA_SABIT_DEGER();
...
int FAT_FATURA_FISI::GET_FIS_RECORD ( int p_fis_id )
{
    ...
    M_FATURA_DETAYLARI->muh_fis_no  = K9_GET_FIS_NO ( CARI_MODULU,     cari_fis_id );
    M_FATURA_DETAYLARI->cari_fis_no = K9_GET_FIS_NO ( MUHASEBE_MODULU, muh_fis_id  );
...
void FAT_FATURA_FISI::SET_LINE_DEFAULTS ( int p_row_number )
{
    ...
    button_fatura_satiri_secimi->setIcon( QPixmap( ":/k9_icons/K9_ICONS/fatura.png" ));
    button_irsaliye_satiri_secimi->setIcon( QPixmap( ":/k9_icons/K9_ICONS/irsaliye.png" ));

    button_seri_parti_kodu->setIconSize( QSize( 80, 20 ));
    button_seri_parti_kodu->setIcon( QPixmap( ":/k9_icons/K9_ICONS/stok_seri_numaralari.png" ));
...
{ 
    if( p_object EQ m_ui->comboBox_fis_filtresi ) {
    ...
    else if( p_object EQ m_ui->comboBox_sirket ) {
...
int FAT_FATURA_FISI::FIND_FIS_RECORD()
{
    ...
    if ( kaydedilecek_sirket NE -1 ) {
        select_query.AND_EKLE("kaydedilecek_sirket = :kaydedilecek_sirket");
        select_query.SET_VALUE(":kaydedilecek_sirket" , kaydedilecek_sirket);
    }
...
int FAT_FATURA_FISI::CHECK_LINE_VAR ( int p_row_number, QObject * p_object )
{
    ...
    if ( p_object EQ searchEdit_urun_kodu ) {
        ...
        int urun_para_birim_id = URUN->urun_satis_doviz_id;
        if ( M_FIS->fatura_alis_satis_turu EQ ENUM_ALIS_IRSALIYESI_FATURASI ) {
            urun_para_birim_id = URUN->urun_alis_doviz_id;
        }
...
void FAT_FATURA_FISI::ADD_LINE ( int p_fis_id, int p_row_number )
{
    ...
    M_FIS_SATIRI->program_id                        = K9_PROGRAMI;
...
void FAT_FATURA_FISI::SEARCH_EDIT_CLICKED(QWidget * widget, QLineEdit * line_edit)
{
    ...
    if ( widget EQ m_ui->searchEdit_cari_hesap_kodu ) {
        int cari_hesap_id = OPEN_CARI_KART_ARAMA( line_edit->text(), SAHIS_ARTI_FIRMA,this,true , K9_PROGRAMI, CARI_MODULU );
...
void FAT_FATURA_FISI::SELECTED_LINE ( int p_row_number, int p_row_save_status )
{
    ...
    double urun_birim_fiyati                       = commaEdit_iskonto_sonrasi_birim_fiyat->GET_DOUBLE();
    double kdv_orani                               = comboBox_kdv_orani->currentText().toDouble();
    ...
    double vergi_haric_birim_fiyat                 = 0.00;
    double otv_dahil_birim_fiyat                   = 0.00;
    double kdv_tutari_birim_fiyat                  = 0.00;
    double otv_tutari_birim_fiyat                  = 0.00;
    double makbuz_para_birimi_cinsinden_birim_fiyat = 0.00;

    if ( comboBox_kdv_dahil_mi->currentIndex() EQ 0 ) {

        otv_tutari_birim_fiyat                  = ROUND ( urun_birim_fiyati     * ( otv_orani / 100 ));
        otv_dahil_birim_fiyat                   = ROUND ( urun_birim_fiyati     * ( 1 + ( otv_orani / 100 ) ) );
        kdv_tutari_birim_fiyat                  = ROUND ( otv_dahil_birim_fiyat * ( kdv_orani / 100 ) );
        kdv_dahil_birim_fiyat                   = ROUND ( otv_dahil_birim_fiyat + kdv_tutari_birim_fiyat  );
        ...
        otv_dahil_birim_fiyat                   = urun_birim_fiyati / ( 1 + ( kdv_orani / 100 ) );
        kdv_tutari_birim_fiyat                  = urun_birim_fiyati - otv_dahil_birim_fiyat;
        vergi_haric_birim_fiyat                 = otv_dahil_birim_fiyat / ( 1 + ( otv_orani / 100 ) );
        otv_tutari_birim_fiyat                  = otv_dahil_birim_fiyat - vergi_haric_birim_fiyat;
        makbuz_para_birimi_cinsinden_birim_fiyat = commaEdit_vergi_haric_tutar->GET_DOUBLE();
...
int FAT_FATURA_FISI::LEAVE_RECORD()
{
    if ( m_log_kaydi_eklenecek_mi EQ true ) {
        ...
        if ( m_kayit_eklendi EQ true ) {
            K9_LOG_KAYDI_EKLE (FATURA_MODULU , LOG_FAT_FATURALAR , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE (FATURA_MODULU , LOG_FAT_FATURALAR , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE (FATURA_MODULU , LOG_FAT_FATURALAR , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
...
void FAT_FATURA_FISI::FATURA_EKRANINI_DUZENLE ()
{
    ...
    K9_KULLANICI_YETKILERI_STRUCT * KULLANICI_YETKILERI = ( K9_KULLANICI_YETKILERI_STRUCT * ) K9_GET_KULLANICI_YETKILERI();
...
void FAT_FATURA_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_fis_id )
{
    ...
    if ( p_button EQ m_ui->button_irsaliye_secimi ) {
        ...
        K9_URETILEN_FIS_SEC( M_FIS->program_id, M_FIS->modul_id, p_fis_id, this );
    }
}
