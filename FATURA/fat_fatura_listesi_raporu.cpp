...
void FAT_FATURA_LISTESI_RAPORU::FILL_RAPOR_SATIRLARI()
{
    ...
    if ( M_FAT_RV->entegre_filtresi_turu EQ FIS_IKINCI_SIRKET ) {
        query.AND_EKLE  ( "kaydedilecek_sirket = :kaydedilecek_sirket" );
    ...
    while ( query.NEXT() ) {
        ...
        int fatura_irsaliyelestirildi_mi = query.VALUE("fatura_irsaliyelestirildi_mi").toInt();

        if ( fatura_irsaliyelestirildi_mi EQ 1 ) {
            fatura_durumu = ENUM_IRSALIYELENMIS;
        }
        else {
            fatura_durumu = ENUM_IRSALIYELENMEMIS;
        }
        int fatura_turu = query.VALUE("fatura_turu").toInt();

        if ( fatura_turu EQ ENUM_FAT_HIZMET_ALIS_FATURASI OR fatura_turu EQ ENUM_FAT_HIZMET_SATIS_FATURASI ) {
            fatura_durumu = -1;
        }
