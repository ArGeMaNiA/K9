...
int FAT_FATURA_ARAMA::CHECK_VAR ( QObject * p_object )
{
    if ( p_object EQ m_ui->comboBox_fis_filtresi ) {
        ...
        m_entegre_filtresi_turu = K9_GET_FIS_FILTRESI_ENUM( m_ui->comboBox_fis_filtresi->currentText() );

    }
    else if ( ( p_object EQ m_ui->adakDate_ilk_tarih ) OR ( p_object EQ m_ui->adakDate_son_tarih) ) {
...
void FAT_FATURA_ARAMA::SEARCH_EDIT_CLICKED( QWidget *widget, QLineEdit *line_edit )
{
    ...
    if ( widget EQ m_ui->searchEdit_cari_hesap_kodu ) {
        m_cari_hesap_id = OPEN_CARI_KART_ARAMA ( "", SAHIS_ARTI_FIRMA , this, 0, K9_PROGRAMI, CARI_MODULU );// SAHIS + FIRMA
