...
{
    static struct MENU_STRUCT K9_MENU_STRUCT[] = {
...
MENU_STRUCT * YONETIM_GET_MENU_STRUCT()
{
    ...
    static struct MENU_STRUCT YONETIM_MENU_STRUCT[ 26 ] = {
            { MENU_YNT_K9_SABIT_DEGERLER      , QObject::tr ("Parametreler")                   , "SIS4" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_BELGE_TASARIMCISI   , QObject::tr ("Belge Tasarımcısı")              , "SIS5" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_LOG_BILGILERI       , QObject::tr ("K9 Log Bilgisi Arama")           , "SIS6" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_EMAIL_LOGLARI       , QObject::tr ("E-Mail Log Bilgileri" )          , "SIS7" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_DEVIR_ISLEMI        , QObject::tr ("Devir İşlemi")                   , "SIS8" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_ONAR                , QObject::tr ("K9 Onar")                        , "SIS9" , "" , "" , false , 1 , 0 },
            { MENU_YNT_K9_PROFIL_SECME        , QObject::tr ("K9 Profil Seçme")                , "SIS10", "" , "" , false , 1 , 0 },
            { MENU_SEPERATOR                  , QObject::tr ("")                               , ""     , "" , "" , false , 1 , 0 }
...
{
    switch ( enum_id ) {
        case MENU_YNT_K9_DEVIR_ISLEMI :
            OPEN_K9_DEVIR_ISLEMI_BATCH ( parent );
            break;

        case MENU_YNT_K9_ONAR :
            OPEN_K9_ONAR_BATCH ( parent );
            break;

        case MENU_YNT_K9_BELGE_TASARIMCISI :
            //ADAK_INFO( "Belge Tasarımcısı bu versiyonda kapalıdır. \nBir sonraki versiyonda yeni hali ile devreye girecektir...", NULL, NULL );
            OPEN_TASARIM_MENU_BATCH (  k9_belge_struct, ADAK_BELGE_COUNT, parent );
            break;

        case MENU_YNT_K9_SABIT_DEGERLER :
            OPEN_K9_SABIT_DEGERLER_FORMU ( parent );
            break;

        case MENU_YNT_K9_LOG_BILGILERI  :
            OPEN_K9_LOG_KAYDI_ARAMA ( parent );
            break;

        case MENU_YNT_K9_EMAIL_LOGLARI :
            OPEN_ADRES_EMAIL_LOG_BATCH ( parent );
            break;

        case MENU_YNT_K9_PROFIL_SECME : 
	    {
                bool degisiklik_yapildimi = false;
                OPEN_K9_PROFIL_SECME( ENUM_K9_BASLANGIC, parent, &degisiklik_yapildimi );
            }
            break;

        default:
            SISTEM_ITEM_CLICKED  ( enum_id , parent, &OPEN_K9_KULLANICI_YETKILERI_FORMU );
            break;
    }
}
