...
extern struct K9_KULLANICI_YETKILERI_STRUCT * K9_KULLANICI_YETKILERI;

/**************************************************************************************
                   OPEN_K9_ANA_MENU
***************************************************************************************/

void OPEN_K9_ANA_MENU ( QWidget * p_parent )
{
    K9_ANA_MENU * w = new K9_ANA_MENU(p_parent);
    w->showMaximized();
    ADAK_CREATE_TEMP_FILE (NULL);
    w->SHOW_REKLAM ("K9_ANA_MENU");
...
/**************************************************************************************
                   K9_ANA_MENU::K9_ANA_MENU
***************************************************************************************/

K9_ANA_MENU::K9_ANA_MENU(QWidget * parent) : SEKME_MENU_KERNEL(K9_PROGRAMI,parent)
{
    ...
    query.PREPARE_SELECT( "k9_sabit_degerler", "program_profil_id", "sabit_deger_id = :sabit_deger_id" );
    ...
    if ( profil_id EQ 0 ) {
        ...
        query.PREPARE_UPDATE( "k9_sabit_degerler", "sabit_deger_id", "program_profil_id", "sabit_deger_id = :sabit_deger_id");
...
/**************************************************************************************
                   K9_ANA_MENU::PREPARE_PROGRAM_FOR_USER
***************************************************************************************/

void K9_ANA_MENU::PREPARE_PROGRAM_FOR_USER()
{
    KULLANICI_YETKILERINI_AYARLA ();
}

/**************************************************************************************
                   K9_ANA_MENU::KULLANICI_YETKILERINI_AYARLA
***************************************************************************************/

void K9_ANA_MENU::KULLANICI_YETKILERINI_AYARLA()
{
    ...
    if ( K9_KULLANICI_YETKILERI->ynt_k9_yetkilendirme_yapabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_KULLANICI_YETKILERI);
    }

    if ( K9_KULLANICI_YETKILERI->ynt_k9_log_inceleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_LOG_BILGILERI );
    }
    if ( K9_KULLANICI_YETKILERI->ynt_k9_email_log_inceleme_yetkisi EQ 1 ){
         SHOW_MENU_ITEM( MENU_YNT_K9_EMAIL_LOGLARI );
    }
    if ( K9_KULLANICI_YETKILERI->adr_adres_karti_tanimlama_yetkisi EQ 1 ) {
    ...
    K9_GET_SISTEM_KULLANICI_YETKILERI( KULLANICI_ID(), SISTEM_MENUSU );
    ...
    if ( KULLANICI_ID () EQ 1 ) {
        ...
        SHOW_MENU_ITEM ( MENU_YONETIM                    );
        SHOW_MENU_ITEM ( MENU_YNT_K9_ONAR                );
    ...
    if ( K9_KULLANICI_YETKILERI->adr_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_GRUPLAR );
    }

    if ( K9_KULLANICI_YETKILERI->adr_etiket_yazdirma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_ETIKETLERI );
    }

    if ( K9_KULLANICI_YETKILERI->adr_toplu_mail_gonderme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_TOPLU_MAIL_GONDERME );
    }

    if ( K9_KULLANICI_YETKILERI->adr_toplu_sms_gonderme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_TOPLU_SMS_GONDERME );
    }

    if ( K9_KULLANICI_YETKILERI->adr_adres_kayitlari_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_KAYITLARI );
    }
    if ( K9_KULLANICI_YETKILERI->adr_tel_web_kayitlari_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_TEL_WEB_KAYITLARI);
    }

    if ( K9_KULLANICI_YETKILERI->adr_fihrist_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_FIHRIST  );
    }

    if ( K9_KULLANICI_YETKILERI->adr_export_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_ADR_EXPORT );
    }

    if ( K9_KULLANICI_YETKILERI->adr_import_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADR_ISLEMLER_HEADER);
    ...
    if ( K9_KULLANICI_YETKILERI->bnk_hareket_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_FISLER_HEADER );
        SHOW_MENU_ITEM ( MENU_BNK_HAREKET_FISI );
        SHOW_MENU_ITEM ( MENU_BNK_PARA_YAT_CEKME_FISI );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_acilis_fisi_islem_yetkisi  EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_FISLER_HEADER );
        SHOW_MENU_ITEM ( MENU_BNK_ACILIS_FISI );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_banka_hesabi_acma_yetkisi  EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_ISLEMLER_HEADER );
        SHOW_MENU_ITEM ( MENU_BNK_HESAPLARI );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_ent_fisleri_inceleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_ENT_FISLER_HEADER );
        SHOW_MENU_ITEM ( MENU_BNK_ENT_FISLERI       );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_defter_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_RAPORLAR_HEADER   );
        SHOW_MENU_ITEM ( MENU_BNK_DEFTERI           );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_pos_ekstresi_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_RAPORLAR_HEADER   );
        SHOW_MENU_ITEM ( MENU_BNK_POS_EKSTRESI      );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_makro_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_ISLEMLER_HEADER   );
        SHOW_MENU_ITEM ( MENU_BNK_MAKRO_TANIMLAMA   );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_makro_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BNK_FISLER_HEADER     );
        SHOW_MENU_ITEM ( MENU_BNK_MAKRO_FISI        );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_kk_ekstresi_alma_yetkisi EQ  1 ){
        SHOW_MENU_ITEM ( MENU_BNK_RAPORLAR_HEADER   );
        SHOW_MENU_ITEM ( MENU_BNK_KK_EKSTRESI       );
    }

    if ( K9_KULLANICI_YETKILERI->bnk_virman_islem_yetkisi EQ  1 ){
        SHOW_MENU_ITEM ( MENU_BNK_FISLER_HEADER     );
        SHOW_MENU_ITEM ( MENU_BNK_VIRMAN_FORMU      );
    }

    //CARI
    if ( K9_KULLANICI_YETKILERI->cari_tahsilat_fisi_islem_yetkisi EQ 1 ) {    
        SHOW_MENU_ITEM ( MENU_CARI_FISLER_HEADER    );
        SHOW_MENU_ITEM ( MENU_CARI_TAHSILAT_FISI    );
    }
    if ( K9_KULLANICI_YETKILERI->cari_odeme_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_FISLER_HEADER    );
        SHOW_MENU_ITEM ( MENU_CARI_ODEME_FISI       );
    }
    if ( K9_KULLANICI_YETKILERI->cari_hareket_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_FISLER_HEADER    );
        SHOW_MENU_ITEM ( MENU_CARI_HAREKET_FISI     );
    }
    if ( K9_KULLANICI_YETKILERI->cari_acilis_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_FISLER_HEADER    );
        SHOW_MENU_ITEM ( MENU_CARI_ACILIS_FISI      );
    }
    if ( K9_KULLANICI_YETKILERI->cari_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_KARTLAR_HEADER   );
        SHOW_MENU_ITEM ( MENU_CARI_GRUPLAR          );
    }
    if ( K9_KULLANICI_YETKILERI->cari_kart_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_KARTLAR_HEADER   );
        SHOW_MENU_ITEM ( MENU_CARI_FIRMA_KARTI      );
        SHOW_MENU_ITEM ( MENU_CARI_SAHIS_KARTI      );
    }
    if ( K9_KULLANICI_YETKILERI->cari_bakiye_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_RAPORLAR_HEADER  );
        SHOW_MENU_ITEM ( MENU_CARI_BAKIYE_RAPORU    );
    }
    if ( K9_KULLANICI_YETKILERI->cari_ekstre_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_RAPORLAR_HEADER  );
        SHOW_MENU_ITEM ( MENU_CARI_HESAP_EKSTRESI   );
    }
    if ( K9_KULLANICI_YETKILERI->cari_virman_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI_VIRMAN_FISI      );
    }
    if ( K9_KULLANICI_YETKILERI->cari_ent_fisleri_inceleme_yetkisi EQ 1 ) {
    ...
    //CEK
    if ( K9_KULLANICI_YETKILERI->cek_senet_girebilir_mi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->cek_mus_cek_senet_arama_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->cek_listesi_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CEK_RAPORLAR_HEADER );
        SHOW_MENU_ITEM( MENU_CEK_SENET_LISTESI );
    }

    if ( K9_KULLANICI_YETKILERI->cek_bordrosu_islem_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->cek_acilis_bordrosu_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CEK_ACILIS_BORDROSU_HEADER );
        SHOW_MENU_ITEM ( MENU_CEK_ACILIS_BORDROSU );

    }

    if ( K9_KULLANICI_YETKILERI->cek_ent_fisleri_inceleme_yetkisi EQ 1 ) {
    ...
    //DEMIRBAS
    if ( K9_KULLANICI_YETKILERI->dmr_amortisman_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_DMR_FISLER_HEADER);
        SHOW_MENU_ITEM ( MENU_DMR_AMORTISMAN_FISI);

    }
    if ( K9_KULLANICI_YETKILERI->dmr_toplu_amortisman_ayirma_yetkisi EQ 1){
        SHOW_MENU_ITEM ( MENU_DMR_ISLEMLER_HEADER );
        SHOW_MENU_ITEM ( MENU_DMR_TOPLU_AMORTISMAN );
    }

    if ( K9_KULLANICI_YETKILERI->dmr_yen_degerleme_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_DMR_FISLER_HEADER);
        SHOW_MENU_ITEM ( MENU_DMR_YEN_DEGERLEME_FISI );
    }

    if ( K9_KULLANICI_YETKILERI->dmr_toplu_yeniden_degerleme_yetkisi EQ 1) {
         SHOW_MENU_ITEM ( MENU_DMR_ISLEMLER_HEADER );
         SHOW_MENU_ITEM ( MENU_DMR_TOPLU_YEN_DEGERLEME );
    }
    if ( K9_KULLANICI_YETKILERI->dmr_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_DMR_TANIMLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_DMR_GRUPLARI );
    }
    if ( K9_KULLANICI_YETKILERI->dmr_rapor_islem_grubu_tanimlama_yetkisi EQ 1 ){
        SHOW_MENU_ITEM ( MENU_DMR_TANIMLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_DMR_RAPOR_ISLEM_GRUPLARI);
    }
    if ( K9_KULLANICI_YETKILERI->dmr_demirbas_karti_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_DMR_TANIMLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_DMR_KAYDI);
        SHOW_MENU_ITEM ( MENU_DMR_SATISI);
    }
    if ( K9_KULLANICI_YETKILERI->dmr_demirbas_listesi_rapor_alma_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->dvz_dovizleri_inceleme_yetkisi EQ 1 ){
        SHOW_MENU_ITEM ( MENU_DVZ_DOVIZLER );
    }
    if ( K9_KULLANICI_YETKILERI->dvz_kur_guncelleme_yetkisi EQ 1 ){
        SHOW_MENU_ITEM ( MENU_DVZ_KURLARI_GUNCELLE  );
    }
    if  ( K9_KULLANICI_YETKILERI->dvz_doviz_tanimlama_yetkisi EQ 1){
        SHOW_MENU_ITEM ( MENU_DVZ_TANIMLARI   );
    }

    //FAIZ YETKILENDIRME YOK
    if ( K9_KULLANICI_YETKILERI->k9_faiz_kullanabilir_mi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->fat_alis_faturasi_kesebilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_FAT_ALIS_FAT_HEADER);
        SHOW_MENU_ITEM ( MENU_FAT_ALIS_FATURALARI );
        SHOW_MENU_ITEM ( MENU_FAT_SATISTAN_IADE   );
    }

    if ( K9_KULLANICI_YETKILERI->fat_sts_faturasi_kesebilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_FAT_SATIS_FAT_HEADER );
        SHOW_MENU_ITEM ( MENU_FAT_SATIS_FATURALARI );
        SHOW_MENU_ITEM ( MENU_FAT_ALISTAN_IADE     );
    }

    if ( K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_toplu EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_FAT_TOPLU_IRS_FATURALASTIRMA );
    }
    if ( K9_KULLANICI_YETKILERI->fat_hizmet_ekstresi_alma_yetkisi EQ 1 ) {

        SHOW_MENU_ITEM ( MENU_FAT_HIZMET_EKSTRESI );
    }

    if ( K9_KULLANICI_YETKILERI->fat_ba_bs_listesi_alma_yetkisi  EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_FAT_RAPORLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_FAT_BA_BS_FORMU );
    }

    if ( K9_KULLANICI_YETKILERI->fat_fatura_listesi_alma_yetkisi EQ 1 ){
        SHOW_MENU_ITEM ( MENU_FAT_RAPORLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_FAT_LISTESI );
    }

    if ( K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_elle EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->fat_irs_faturalastirma_oto EQ 1 ) {
    ...
    //İRSALİYE
    if ( K9_KULLANICI_YETKILERI->irs_alis_irsaliyesi_islem_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->irs_sts_irsaliyesi_islem_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->irs_fat_irsaliyelestirme_oto EQ 1 ) {
    ...
    //MUHASEBE
    if ( K9_KULLANICI_YETKILERI->muh_muhasebe_fisi_islem_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->muh_acilis_fisi_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_FISLER_HEADER );
        SHOW_MENU_ITEM ( MENU_MUH_ACILIS_FISI  );
    }

    if ( K9_KULLANICI_YETKILERI->muh_hesap_plani_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM (MENU_MUH_HESAPLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_HESAP_PLANI );
    }

    if ( K9_KULLANICI_YETKILERI->muh_hesap_ekstresi_alma_yetkisi EQ 1 ) {
         SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
         SHOW_MENU_ITEM ( MENU_MUH_HESAP_EKSTRESI );
    }

    if ( K9_KULLANICI_YETKILERI->muh_mizan_raporu_alma_yetkisi EQ 1 ) {
         SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
         SHOW_MENU_ITEM ( MENU_MUH_MIZAN );
    }

    if ( K9_KULLANICI_YETKILERI->muh_hesap_plani_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_HESAP_PLANI_RAPORU );
    }

    if ( K9_KULLANICI_YETKILERI->muh_bilanco_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_BILANCO );
    }

    if ( K9_KULLANICI_YETKILERI->muh_defteri_kebir_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_DEFTERI_KEBIR );
    }

    if ( K9_KULLANICI_YETKILERI->muh_gelir_tablosu_rapor_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_GELIR_TABLOSU );
    }

    if ( K9_KULLANICI_YETKILERI->muh_yvm_dokumu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_YEVMIYE_DOKUMU );
    }

    if ( K9_KULLANICI_YETKILERI->muh_hatali_fisleri_inceleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_HATALI_FISLER );
    }

    if ( K9_KULLANICI_YETKILERI->muh_fis_dokumu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_TOPLU_FIS_DOKUMU );
    }

    if ( K9_KULLANICI_YETKILERI->muh_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM (MENU_MUH_HESAPLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_HESAP_GRUPLARI );
    }

    if ( K9_KULLANICI_YETKILERI->muh_yansitma_fisi_olusturma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_YANSITMA_FISI_OLUSTUR );
    }

    if ( K9_KULLANICI_YETKILERI->muh_yvm_no_olusturma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_YVM_NO_OLUSTUR );
    }

    if ( K9_KULLANICI_YETKILERI->muh_yansitma_hesabi_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_YANSITMA_HESAPLARI);
    }

    if ( K9_KULLANICI_YETKILERI->muh_hesap_plani_kopyalama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_MUH_HESAP_PLANI_KOPYALA );
    }

    if ( K9_KULLANICI_YETKILERI->muh_ent_fisleri_inceleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUH_ENT_FISLERI_HEADER );
        SHOW_MENU_ITEM ( MENU_MUH_ENT_FISLERI );
    }

    //PERAKENDE
    if ( K9_KULLANICI_YETKILERI->prk_fis_girebilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRK_FISLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRK_ALIS_FISI );
        SHOW_MENU_ITEM ( MENU_PRK_SATIS_FISI);
    }
    if ( K9_KULLANICI_YETKILERI->prk_gdr_tur_tanimlayabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRK_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRK_GIDER_TURLERI );
        SHOW_MENU_ITEM ( MENU_PRK_GELIR_TURLERI );
    }
    if ( K9_KULLANICI_YETKILERI->prk_kasa_raporu_alabilir_mi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->prs_eksik_gun_nedenleri_giris_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_EKSIK_GUN_NEDENLERI);
    }

    if ( K9_KULLANICI_YETKILERI->prs_ucret_pusulasi_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_UCRET_PUSULASI );
    }

    if ( K9_KULLANICI_YETKILERI->prs_sicil_karti_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_SICIL_KARTI_TANIMLAMA );
    }

    if ( K9_KULLANICI_YETKILERI->prs_ek_odenek_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_EK_ODENEK_TANIMLAMA );
    }

    if ( K9_KULLANICI_YETKILERI->prs_maas_bordrosu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_MAAS_BORDROSU );
    }

    if ( K9_KULLANICI_YETKILERI-> prs_ek_kesinti_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_EK_KESINTI_TANIMLAMA );
    }

    if ( K9_KULLANICI_YETKILERI->prs_sabit_deger_guncelleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_SABIT_DEGERLER );
    }

    if ( K9_KULLANICI_YETKILERI->prs_maas_bordrosu_iptal_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_MAAS_BORDROSU_IPTAL );
    }

    if ( K9_KULLANICI_YETKILERI->prs_personel_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_PERSONEL_RAPORU );
    }

    if ( K9_KULLANICI_YETKILERI->prs_isten_ayrilan_personel_arama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_ISTEN_AYRILAN_ARAMA );
    }

    if ( K9_KULLANICI_YETKILERI->prs_maas_bordrosu_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_MAAS_BORDROSU_RAPORU );
    }

    if ( K9_KULLANICI_YETKILERI->prs_isten_ayrilma_islem_yetkisi EQ 1 ) {
         SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
         SHOW_MENU_ITEM ( MENU_PRS_ISTEN_AYRILMA );
    }

    if ( K9_KULLANICI_YETKILERI->prs_e_bildirge_alma_yetkisi EQ 1) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_EBILDIRGE);
    }

    if ( K9_KULLANICI_YETKILERI->prs_ihbar_ve_kidem_tazminati_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_IHBAR_VE_KIDEM_TAZMINATI_RAPORU );
    }

    if ( K9_KULLANICI_YETKILERI->prs_bordro_inceleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_ISLEMLER_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_BORDRO_INCELEME );
    }

    if ( K9_KULLANICI_YETKILERI->prs_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PRS_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_PRS_GRUPLAR );
    }

    if ( K9_KULLANICI_YETKILERI->smm_makbuzu_islem_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_SMM_FISLER_HEADER );
        SHOW_MENU_ITEM ( MENU_SMM_MAKBUZ_ALIS);
        SHOW_MENU_ITEM ( MENU_SMM_MAKBUZ_SATIS);
    }

    if ( K9_KULLANICI_YETKILERI->smm_hizmet_tanimlama_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->stok_grup_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_TANIMLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_STOK_GRUPLARI        );
    }

    if ( K9_KULLANICI_YETKILERI->stok_hizmet_karti_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_TANIMLAR_HEADER );
        SHOW_MENU_ITEM ( MENU_STOK_HIZMET_KARTI    );
    }

    if ( K9_KULLANICI_YETKILERI->stok_fisi_islem_yetkisi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->stok_urun_karti_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_TANIMLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_STOK_URUN_KARTI );
    }

    if ( K9_KULLANICI_YETKILERI->stok_ekstresi_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_STOK_EKSTRESI );
    }
    if ( K9_KULLANICI_YETKILERI->stok_miktar_raporu_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_RAPORLAR_HEADER);
        SHOW_MENU_ITEM ( MENU_STOK_MIKTARLARI_RAPORU );
    }
    if ( K9_KULLANICI_YETKILERI->stok_urun_listesi_alma_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK_RAPORLAR_HEADER );
    ...
    //SUBE
    if ( K9_KULLANICI_YETKILERI->sub_sube_tanimlama_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_SUBE_SUBELER        );
        SHOW_MENU_ITEM ( MENU_SUBE_ORGANIZASYON_SEMASI );
    }

    //MODUL YETKILERI
    if ( K9_KULLANICI_YETKILERI->k9_smm_kullanabilir_mi EQ 1 ){
        SHOW_MENU_ITEM(MENU_SMM_MAKBUZ);
    }

    if ( K9_KULLANICI_YETKILERI->k9_muh_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_MUHASEBE );
    }

    if ( K9_KULLANICI_YETKILERI->k9_isl_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ISLETME );
    }

    if ( K9_KULLANICI_YETKILERI->k9_adr_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_ADRES );
    }

    if ( K9_KULLANICI_YETKILERI->k9_dvz_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_DOVIZ );
    }

    if ( K9_KULLANICI_YETKILERI->k9_car_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CARI );
    }

    if ( K9_KULLANICI_YETKILERI->k9_cek_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_CEK );
    }

    if ( K9_KULLANICI_YETKILERI->k9_bnk_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_BANKA );
    }

    if ( K9_KULLANICI_YETKILERI->k9_sub_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_SUBE );
    }

    if ( K9_KULLANICI_YETKILERI->k9_fat_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_FATURA );
    }

    if ( K9_KULLANICI_YETKILERI->k9_irs_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_IRSALIYE );
    }

    if ( K9_KULLANICI_YETKILERI->k9_stk_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_STOK );
    }

    if ( K9_KULLANICI_YETKILERI->k9_prs_kullanabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_PERSONEL );
    }

    if ( K9_KULLANICI_YETKILERI->k9_dmr_kullanabilir_mi EQ 1 ) {
    ...
    if ( K9_KULLANICI_YETKILERI->k9_prk_kullanabilir_mi EQ 1 ) {
    ...
    // YETKI YAZILCAK MI
    SHOW_MENU_ITEM( MENU_YNT_K9_PROFIL_SECME );

    if ( K9_KULLANICI_YETKILERI->ynt_k9_onar_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_ONAR);
    }

    if ( K9_KULLANICI_YETKILERI->ynt_k9_devir_islemi_yapabilir_mi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_DEVIR_ISLEMI);
    }
    ...
    if ( K9_KULLANICI_YETKILERI->ynt_k9_sabit_degerler_guncelleme_yetkisi EQ 1 ) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_SABIT_DEGERLER);
    }

    if ( K9_KULLANICI_YETKILERI->ynt_k9_belge_tasarlama_yetkisi EQ 1) {
        SHOW_MENU_ITEM ( MENU_YNT_K9_BELGE_TASARIMCISI );
    }
...
/**************************************************************************************
                   K9_ANA_MENU::MENU_ITEM_CLICKED
***************************************************************************************/

MENU_STRUCT * K9_ANA_MENU::GET_MENU_STRUCT(int parent_enum_id)
{
...
void K9_ANA_MENU::MENU_ITEM_CLICKED (int parent_item_enum, int menu_item_enum)
{
    switch ( parent_item_enum ) {
        ...
        default:
            if (menu_item_enum EQ MENU_HATA_ISTEK ) {
                ...
                OPEN_K9_KUL_KITAPCIGI_VE_YARDIM_SAYFASI_BATCH( 1, this );
            }
            else if ( menu_item_enum EQ MENU_SIK_SORUNLAN_SORUNLAR ) {
                OPEN_K9_KUL_KITAPCIGI_VE_YARDIM_SAYFASI_BATCH( 0, this );
            }
