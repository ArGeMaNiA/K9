...
//gonderilmelidir.Struct da bu degerler float page_left_space = left_margin iken , float page_top_space = top_margin dir. Bottom ve right margin daima 0 dir cunku
//etiket duzgun ayarlanmissa zaten etiket tam olarak sayfaya oturacagindan bottom ve right margin direk olarak hesaplanir.
//
//Dikkat structdaki float margin_right,float margin_bottom degerleri,sayfadaki her bir etiket arasindaki degerlerdir.Printer marginleri
...
protected:
    //etiketin boyutlari
    void SET_ETIKET_SIZE(float etiket_width,float etiket_height);

    //etiketin sagdan ve alt tarafindan birakilacak bosluk
    void SET_ETIKET_MARGIN(float margin_right,float margin_bottom);

    //Sayfanin en ust ve sag tarafindan birakilacak bosluk
    //Bu opsiyon kullanilirsa yazici margin 0 lanmalidir.
    void SET_PAGE_SPACE(float top_space,float left_space);
...
private:
    float p_etiket_width;
    float p_etiket_height;
    float p_page_left_space;
    float p_page_top_space;
    float p_margin_right;
    float p_margin_bottom;
