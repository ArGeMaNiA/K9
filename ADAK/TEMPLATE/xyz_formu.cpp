...
int XYZ_FORMU::CHECK_VAR (QObject * object)
{
    if ( object EQ m_ui->combobox_para_birimi ) {
        SQL_QUERY query(DB);
        query.PREPARE_SELECT ( "k9_para_birimleri","para_birimi_adi","para_birimi_kodu=:para_birimi_kodu" );
...
void XYZ_FORMU::UPDATE_RECORD ( int record_id )
{
    ...
    query.PREPARE_UPDATE ( "k9_sabit_degerler","id_column","firma_ismi,yetkili,firma_adresi,stok_ayraci,personel_ayraci" );
...
{
    return DB->LOCK_ROW ( "firma_bilgileri_id", "k9_sabit_degerler", QString ( "firma_bilgileri_id = %1").arg ( record_id ) );
}
...
{
    DB->UNLOCK_ROW ( "firma_bilgileri_id", "k9_sabit_degerler", QString ( "firma_bilgileri_id = %1").arg ( record_id ) );
}
