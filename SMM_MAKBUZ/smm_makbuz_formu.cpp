...
void  SMM_MAKBUZ_FORMU::NEW_RECORD ()
{
    ...
    M_MAKBUZ->program_id    = K9_PROGRAMI;
    M_MAKBUZ->modul_id      = SMM_MODULU;
    M_MAKBUZ->makbuz_turu   = m_makbuz_turu;

    M_MAKBUZ->kaydedilecek_sirket = K9_KULLANICININ_CALISTIGI_SIRKET();

    if ( m_makbuz_turu EQ SMM_SATIS_MAKBUZU ) {
        QString sm_sicil_numarasi = K9_GET_SM_SICIL_NUMARASI();
...
int SMM_MAKBUZ_FORMU::CHECK_VAR (QObject * object)
{
    if ( object EQ m_ui->searchEdit_cari_hesap_kodu ) {
        ...
        int tah_odm_sekli = K9_GET_TAH_ODM_SEKLI_ENUM(m_ui->comboBox_tah_odm_sekli->currentText());
        ...
        M_MAKBUZ->kaydedilecek_sirket = K9_GET_KAYDEDILECEK_SIRKET_ENUM( m_ui->comboBox_sirket->currentText() );
    }
...
int SMM_MAKBUZ_FORMU::ADD_RECORD ()
{
    ...
    M_MAKBUZ->tah_odm_sekli               = K9_GET_TAH_ODM_SEKLI_ENUM(m_ui->comboBox_tah_odm_sekli->currentText());
...
int SMM_MAKBUZ_FORMU::CHANGER_BUTTON_CLICKED(QAbstractButton *button)
{
    ...
    if ( button EQ m_ui->pushButton_detaylar ) {
        ...
        if ( record_id > 0 ) {
            int muh_fis_id = MUH_GET_ENT_FIS_ID(K9_PROGRAMI , SMM_MODULU,
                                                record_id);
            M_ENT_DETAYLARI->muh_fis_no = K9_GET_FIS_NO(MUHASEBE_MODULU , muh_fis_id);

            int cari_fis_id = CARI_GET_ENT_FIS_ID(K9_PROGRAMI , SMM_MODULU,
                                                  record_id);

            M_ENT_DETAYLARI->cari_fis_no = K9_GET_FIS_NO(CARI_MODULU , cari_fis_id);
        }

        OPEN_SMM_ENT_DETAYLARI_BATCH(M_ENT_DETAYLARI , this);


        if ( M_ENT_DETAYLARI->kaydedilecek_sirket NE M_MAKBUZ->kaydedilecek_sirket ) {
...
void SMM_MAKBUZ_FORMU::SAVER_BUTTON_CLICKED(QAbstractButton *button, int record_id)
{
    if ( button EQ m_ui->pushButton_yazdir ) {
        ...
        K9_URETILEN_FIS_SEC( K9_PROGRAMI, SMM_MODULU, record_id, this );
    }
}

/**************************************************************************************
                   SMM_MAKBUZ_FORMU::SHOW_REKLAM
***************************************************************************************/

void SMM_MAKBUZ_FORMU::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("SMM_MAKBUZ_FORMU");
}
