...
void  DMR_DEMIRBAS_KARTI::NEW_RECORD()
{
    ...
    select_query.PREPARE_SELECT ( "k9_sabit_degerler","dmr_amor_yntemi_degisebilir_mi, dmr_amortisman_ayirma_yontemi,"
...
int DMR_DEMIRBAS_KARTI::GET_RECORD ( int record_id )
{
    ...
    m_ui->textEdit_rapor_isl_gruplari->setText ( GRP_KAYDIN_GRUPLARINI_BUL ( K9_PROGRAMI , DEMIRBAS_GRUP_MODULU, record_id, &m_rpr_isl_grubu_idler, new QStringList, new QStringList ) );
...
int DMR_DEMIRBAS_KARTI::ADD_RECORD ()
{
    ...
    GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI,DEMIRBAS_GRUP_MODULU, demirbas_id , &m_rpr_isl_grubu_idler);

    if ( m_grup_id > 0 ) {
        QList<int> grup_id_list ;
        grup_id_list << m_grup_id;

        GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , DEMIRBAS_MODULU , demirbas_id , &grup_id_list);
    }
    ...
    K9_LOG_KAYDI_EKLE ( DEMIRBAS_MODULU , LOG_DMR_DEMIRBASLAR , LOG_ISLEM_ADD , log_detaylari );
...
void DMR_DEMIRBAS_KARTI::UPDATE_RECORD ( int record_id )
{
    ...
    GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI,DEMIRBAS_GRUP_MODULU, record_id , &m_rpr_isl_grubu_idler);

    if ( m_grup_id > 0 ) {
        QList<int> grup_id_list ;
        grup_id_list << m_grup_id;

        GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , DEMIRBAS_MODULU , record_id , &grup_id_list);
    }
    ...
    K9_LOG_KAYDI_EKLE ( DEMIRBAS_MODULU , LOG_DMR_DEMIRBASLAR , LOG_ISLEM_UPDATE , log_detaylari );
...
void DMR_DEMIRBAS_KARTI::DELETE_RECORD ( int record_id )
{
    ...
    GRP_KAYDIN_GRUPLARINI_SIL(K9_PROGRAMI,DEMIRBAS_GRUP_MODULU,record_id);

    QString log_detaylari = QObject::tr ( "Demirbaş Kodu / Adı : " );
    log_detaylari.append ( demirbas_kodu + "  " + demirbas_adi );

    K9_LOG_KAYDI_EKLE  ( DEMIRBAS_MODULU , LOG_DMR_DEMIRBASLAR , LOG_ISLEM_DELETE , log_detaylari );
}
...
void DMR_DEMIRBAS_KARTI::SEARCH_EDIT_CLICKED ( QWidget * widget,QLineEdit * line_edit )
{
    ...
    if ( widget EQ m_ui->searchedit_muh_hesap_kodu ) {
        ...
        m_cari_hesap_id = OPEN_CARI_KART_ARAMA("",SAHIS_ARTI_FIRMA ,this,1,K9_PROGRAMI,PERSONEL_MODULU,QObject::tr("PERSONEL ARAMA"));
...
int DMR_DEMIRBAS_KARTI::CHANGER_BUTTON_CLICKED(QAbstractButton *button)
{
    if ( button EQ m_ui->pushButton_guplar ) {
        if ( OPEN_GRUP_SECIM ( K9_PROGRAMI,DEMIRBAS_GRUP_MODULU, &m_rpr_isl_grubu_idler, DB, this ) EQ true ) {
