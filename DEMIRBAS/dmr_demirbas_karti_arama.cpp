...
int DMR_DEMIRBAS_KARTI_ARAMA::CHECK_VAR ( QObject * object )
{
    if ( object EQ m_ui->checkbox_demirbas_kodu ) {
        ...
        OPEN_GRUP_SECIM(K9_PROGRAMI , DEMIRBAS_GRUP_MODULU, &m_grup_idler_list , DB,this);
...
void DMR_DEMIRBAS_KARTI_ARAMA::SEARCH()
{
    ...
    if ( m_ui->checkbox_gruplar->isChecked() EQ true ) {
        gruplardaki_kayit_listesi = GRP_GRUBA_EKLI_KAYITLARI_BUL( K9_PROGRAMI, DEMIRBAS_GRUP_MODULU,m_grup_idler_list );
    }
