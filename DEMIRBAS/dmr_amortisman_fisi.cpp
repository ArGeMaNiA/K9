...
#include "dmr_amortsiman_fisi_class.h"
...
int DMR_AMORTISMAN_FISI::LEAVE_RECORD()
{
    if ( m_log_kaydi_eklenecek_mi EQ true ) {
        ...
        if ( m_kayit_eklendi EQ true ) {
            K9_LOG_KAYDI_EKLE (DEMIRBAS_MODULU , LOG_DMR_FISLER , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE (DEMIRBAS_MODULU , LOG_DMR_FISLER , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE (DEMIRBAS_MODULU , LOG_DMR_FISLER , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
        DB->COMMIT_TRANSACTION();
