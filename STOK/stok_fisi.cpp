...
void STOK_FISI::SETUP_FORM()
{
    ...
    m_ui->reklamViewer->SHOW_REKLAM( "STOK-FISI" );
...
void STOK_FISI::SET_LINE_DEFAULTS ( int p_row_number )
{
    ...
    K9_FILL_OTV_ORANLARI_COMBOBOX      ( comboBox_otv_orani   );
    ...
    button_fat_satiri_secimi->setIcon( QPixmap( ":/k9_icons/K9_ICONS/fatura.png" ));
    button_iade_irsaliye_satiri_secimi->setIcon( QPixmap( ":/k9_icons/K9_ICONS/irsaliye.png" ));

    pushButton_seri_no_parti_kodu->setIconSize( QSize( 80, 20 ));
    pushButton_seri_no_parti_kodu->setIcon( QPixmap( ":/k9_icons/K9_ICONS/stok_seri_numaralari.png" ));
...
int STOK_FISI::CHECK_FIS_FORM_VAR ( QObject * p_object )
{
    ...
    else if ( p_object EQ m_ui->comboBox_vergi_dairesi ) {
...
void STOK_FISI::UPDATE_FIS_RECORD ( int p_fis_id )
{
    ...
    int     old_entegre_olsun_mu    =   query.VALUE ( "kaydedilecek_sirket" ).toInt();
    int     new_entegre_olsun_mu    =   M_FIS->kaydedilecek_sirket;
    ...
    if ( m_fis_tarihi_degistirildi_mi EQ true ) {
        ...
        STK_IRSALIYE_TARIH_DEGISIKLIGINE_GORE_STOK_GIRIS_CIKISLARINI_DUZENLE ( p_fis_id, old_fis_tarihi, new_fis_tarihi,
                                                                               ...
                                                                               old_entegre_olsun_mu, new_entegre_olsun_mu );
    ...
    query.SET_VALUE ( ":kaydedilecek_sirket"            , M_FIS->kaydedilecek_sirket                                        );
...
int STOK_FISI::CHECK_ADD_LINE ( int p_fis_id, int p_row_number )
{
    ...
    if ( M_FIS->fis_turu EQ ENUM_ACILIS_FISI ) {
        gun_no = 0;
    }
...
void STOK_FISI::SEARCH_EDIT_CLICKED ( QWidget * p_widget, QLineEdit * p_lineEdit)
{
    ...
    if ( p_widget EQ m_ui->searchEdit_depo_kodu OR p_widget EQ m_ui->searchEdit_alan_depo_kodu ) {
        ...
        int cari_hesap_id = OPEN_CARI_KART_ARAMA ( p_lineEdit->text(), SAHIS_ARTI_FIRMA, this, 1 , K9_PROGRAMI, CARI_MODULU );
...
void STOK_FISI::SAVER_BUTTON_CLICKED ( QAbstractButton * p_button, int p_irsaliye_id )
{
    ...
    if ( p_button EQ m_ui->button_fatura_secimi ) {
        ...
        for ( int i = 0; i < fis_satiri_id_list.size(); i++ ) {
            ...
            IRSALIYE_SATIR_BILGILERI->program_id                        = K9_PROGRAMI;
            IRSALIYE_SATIR_BILGILERI->kaydedilecek_sirket               = FAT_FIS_SATIRI.kaydedilecek_sirket;
        ...
        if ( OPEN_STOK_DETAYLAR_BATCH ( M_STOK_DETAYLARI , this) EQ true ) {
            ...
            if ( M_STOK_DETAYLARI->muh_hesap_id NE old_muh_hesap_id ) {
                ...
                query.PREPARE_SELECT("stk_fis_satirlari , stk_fisler",
                                     "fis_satiri_id , muh_fis_id, irsaliye_tutari, urun_id ",
                                     "stk_fisler.fis_id = :fis_id ANd stk_fisler.fis_id = stk_fis_satirlari.fis_id ");
                query.SET_VALUE(":fis_id" , p_irsaliye_id);

                if ( query.SELECT() NE 0 ) {

                    double irsaliye_tutari = 0.0;

                    while ( query.NEXT() ) {
                        muh_fis_id      = query.VALUE( "muh_fis_id" ).toInt();
                        fis_satiri_id   = query.VALUE( "fis_satiri_id" ).toInt();
                        irsaliye_tutari = query.VALUE( "irsaliye_tutari").toDouble();
                        urun_id         = query.VALUE( "urun_id").toInt();
                        ...
                        if ( M_FIS->fis_turu EQ ENUM_STK_SARF_FIRE_FISI ) {
                            ...
                            if ( muh_fis_satirlari_silindi_mi EQ false ) {
                                ...
                                QStringList muh_row_list = MUH_GET_ENT_ROW_ID_LIST( K9_PROGRAMI, M_FIS->modul_id, fis_satiri_id );
                            ...
                            STK_FIS_SATIRI->kaydedilecek_sirket = M_FIS->kaydedilecek_sirket;
            ...
            query.PREPARE_UPDATE("stk_fisler","fis_id", "kdv_muaf_mi ,kaydedilecek_sirket , "
        ...
        K9_URETILEN_FIS_SEC( M_FIS->program_id, M_FIS->modul_id, p_irsaliye_id, this );
    }
...
int STOK_FISI::LEAVE_RECORD ()
{
    ...
    if ( m_log_kaydi_eklenecek_mi EQ true) {
        ...
        if ( m_kayit_eklendi EQ true ) {
            K9_LOG_KAYDI_EKLE (modul , log_turu , LOG_ISLEM_ADD , m_log_detaylari);
        }
        if ( m_kayit_silindi EQ true ) {
            K9_LOG_KAYDI_EKLE (modul , log_turu , LOG_ISLEM_DELETE , m_log_detaylari);
        }
        if ( m_kayit_eklendi EQ false AND m_kayit_silindi EQ false ) {
            K9_LOG_KAYDI_EKLE (modul , log_turu , LOG_ISLEM_UPDATE , m_log_detaylari);
        }
