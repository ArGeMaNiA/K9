...
void  STOK_URUN_KARTI_FORMU::SETUP_FORM()
{
    ...
    K9_FILL_OTV_ORANLARI_COMBOBOX ( m_ui->comboBox_alis_otv_orani );

    K9_FILL_KDV_ORANLARI_COMBOBOX ( m_ui->comboBox_satis_kdv_orani );
    K9_FILL_OTV_ORANLARI_COMBOBOX ( m_ui->comboBox_satis_otv_orani );
...
int STOK_URUN_KARTI_FORMU::GET_RECORD ( int p_urun_id )
{
    ...
    f_query.PREPARE_SELECT("stk_urunler","urun_kodu, urun_adi, barkod_numarasi,"
                             ...
                             "ent_stoga_giren_array, ent_stoktan_cikan_array, fotograf, satis_kdv_orani, satis_otv_orani ",
                             "urun_id = :urun_id");
    ...
    m_ui->textEdit_gruplar->setText(GRP_KAYDIN_GRUPLARINI_BUL ( K9_PROGRAMI , STOK_MODULU , p_urun_id, &m_grup_idleri, &grup_adlari ,&grup_kodlari ));
    ...
    m_ui->comboBox_satis_kdv_orani->setCurrentIndex                     ( m_ui->comboBox_satis_kdv_orani->findText(f_query.VALUE(20).toString()));
    m_ui->comboBox_satis_otv_orani->setCurrentIndex                     ( m_ui->comboBox_satis_otv_orani->findText(f_query.VALUE(21).toString()));
    ...
    QString ent_stoga_giren_miktar_array    = f_query.VALUE("ent_stoga_giren_array").toString();
    QString ent_stoktan_cikan_miktar_array  = f_query.VALUE("ent_stoktan_cikan_array").toString();

    double STOGA_GIREN_MIKTAR   [K9_ARRAY_SIZE];
    double STOKTAN_CIKAN_MIKTAR [K9_ARRAY_SIZE];

    double ENT_STOGA_GIREN_MIKTAR   [K9_ARRAY_SIZE];
    double ENT_STOKTAN_CIKAN_MIKTAR [K9_ARRAY_SIZE];

    double stok_miktari = 0.00;
    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ SIRKET_1 ) {
        UNPACK_DOUBLE_ARRAY ( ent_stoga_giren_miktar_array,   ENT_STOGA_GIREN_MIKTAR,   K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( ent_stoktan_cikan_miktar_array, ENT_STOKTAN_CIKAN_MIKTAR, K9_ARRAY_SIZE );

        stok_miktari    = ENT_STOGA_GIREN_MIKTAR[K9_SON_BAKIYE] - ENT_STOKTAN_CIKAN_MIKTAR[K9_SON_BAKIYE];
    }
    else {
        UNPACK_DOUBLE_ARRAY ( stoga_giren_miktar_array,   STOGA_GIREN_MIKTAR,   K9_ARRAY_SIZE );
        UNPACK_DOUBLE_ARRAY ( stoktan_cikan_miktar_array, STOKTAN_CIKAN_MIKTAR, K9_ARRAY_SIZE );

        stok_miktari    = STOGA_GIREN_MIKTAR[K9_SON_BAKIYE] - STOKTAN_CIKAN_MIKTAR[K9_SON_BAKIYE];
    }
...
int STOK_URUN_KARTI_FORMU::ADD_RECORD ()
{
    ...
    GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , STOK_MODULU , urun_id, &m_grup_idleri);

    QString log_detaylari = "Ürün Kodu / Adı : " + m_ui->lineEdit_urun_kodu->text() + "  " +
                            m_ui->lineEdit_urun_adi->text();
    K9_LOG_KAYDI_EKLE(STOK_MODULU , LOG_STK_URUNLER , LOG_ISLEM_ADD , log_detaylari);
...
void STOK_URUN_KARTI_FORMU::UPDATE_RECORD ( int p_urun_id )
{
    ...
    GRP_KAYDI_GRUPLARA_EKLE(K9_PROGRAMI , STOK_MODULU , p_urun_id, &m_grup_idleri);

    QString log_detaylari = "Ürün Kodu / Adı : " + m_ui->lineEdit_urun_kodu->text() + "  " +
                            m_ui->lineEdit_urun_adi->text();
    K9_LOG_KAYDI_EKLE(STOK_MODULU , LOG_STK_URUNLER , LOG_ISLEM_UPDATE , log_detaylari);
...
void STOK_URUN_KARTI_FORMU::DELETE_RECORD ( int p_urun_id )
{
    ...
    GRP_KAYDIN_GRUPLARINI_SIL(K9_PROGRAMI , STOK_MODULU , p_urun_id);

    QString log_detaylari = "Ürün Kodu / Adı : " + urun_kodu + "  " + urun_adi;
    K9_LOG_KAYDI_EKLE(STOK_MODULU , LOG_STK_URUNLER , LOG_ISLEM_DELETE , log_detaylari);
}
...
int STOK_URUN_KARTI_FORMU::CHANGER_BUTTON_CLICKED(QAbstractButton *button)
{
    ...
    if ( button EQ m_ui->pushButton_gruplar ) {
        if ( OPEN_GRUP_SECIM( K9_PROGRAMI,STOK_MODULU , &m_grup_idleri , DB , this ) EQ true ) {
            QStringList grup_adlari;
            QStringList grup_kodlari;
...
/**************************************************************************************
                   STOK_URUN_KARTI_FORMU::SHOW_REKLAM
***************************************************************************************/

void STOK_URUN_KARTI_FORMU::SHOW_REKLAM()
{
       m_ui->reklamViewer->SHOW_REKLAM("STOK_URUN_KARTI_FORMU");
}
