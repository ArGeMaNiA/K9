...
void STOK_EKSTRESI_RAPORU::FILL_URUN_ISLEMLERI(int urun_id ,QString urun_kodu_adi)
{
    ...
    if ( M_STOK_RV->ent_filtresi EQ FIS_BIRINCI_SIRKET ) {
        query.AND_EKLE("stk_fisler.kaydedilecek_sirket = :kaydedilecek_sirket");
        query.SET_VALUE(":kaydedilecek_sirket" , SIRKET_1);
    }
    else if ( M_STOK_RV->ent_filtresi EQ FIS_IKINCI_SIRKET ) {
        query.AND_EKLE("stk_fisler.kaydedilecek_sirket = :kaydedilecek_sirket");
        query.SET_VALUE(":kaydedilecek_sirket" , SIRKET_2);
    }
