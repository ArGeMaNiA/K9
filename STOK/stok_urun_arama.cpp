...
void STOK_URUN_ARAMA::SETUP_FORM()
{
    ...
    K9_FILL_COMBOBOX_FIS_FILTRESI(m_ui->comboBox_sirket);

    if ( K9_KULLANICININ_CALISTIGI_SIRKET() EQ SIRKET_1 ) {
        m_ui->comboBox_sirket->setCurrentIndex(m_ui->comboBox_sirket->findText(K9_GET_FIS_FILTRESI_STRING(FIS_BIRINCI_SIRKET)));
    }
    else {
        m_ui->comboBox_sirket->setCurrentIndex(m_ui->comboBox_sirket->findText(K9_GET_FIS_FILTRESI_STRING(FIS_HER_IKI_SIRKET)));
    }
...
void STOK_URUN_ARAMA::SEARCH()
{
    ...
    m_entegrasyon_durumu = K9_GET_FIS_FILTRESI_ENUM(m_ui->comboBox_sirket->currentText());

    if ( m_entegrasyon_durumu EQ FIS_IKINCI_SIRKET ) {
        stk_girenler_table_name     = "ent_olmayan_stoga_giren_array";
        stk_cikanlar_table_name     = "ent_olmayan_stktan_cikan_array";
    }
    else if ( m_entegrasyon_durumu EQ FIS_BIRINCI_SIRKET ) {

        stk_girenler_table_name     = "ent_stoga_giren_array";
        stk_cikanlar_table_name     = "ent_stoktan_cikan_array";
    }
    else {
    ...
    if ( m_ui->checkBox_gruplar->isChecked() EQ true ) {
         QList<int> gruptaki_urun_or_hizmet_id_list = GRP_GRUBA_EKLI_KAYITLARI_BUL ( K9_PROGRAMI , STOK_MODULU , m_grup_idleri );
    ...
    double GIREN_URUN_MIKTARI   [K9_ARRAY_SIZE];
    double CIKAN_URUN_MIKTARI   [K9_ARRAY_SIZE];
    ...
    while ( sql_query.NEXT() EQ true ) {
        ...
        if ( m_urun_hizmet_karti_turu NE ENUM_HIZMET_KARTI ) {
            ...
            if ( query.SELECT() NE 0 ) {
                ...
                UNPACK_DOUBLE_ARRAY(query.VALUE(0).toString() , GIREN_URUN_MIKTARI,K9_ARRAY_SIZE);
                UNPACK_DOUBLE_ARRAY(query.VALUE(1).toString() , CIKAN_URUN_MIKTARI,K9_ARRAY_SIZE);

                urun_miktari = GIREN_URUN_MIKTARI[K9_SON_BAKIYE] - CIKAN_URUN_MIKTARI[K9_SON_BAKIYE];
            }
        }
        ...
        button_hareketler->setIcon(QPixmap(":/k9_icons/K9_ICONS/cari_hareketler.png"));
